...
if (!function_exists('nsx_doDecode')) {
    function nsx_doDecode($string, $key='NSX')
    {
        ...
        $keyLen = strlen($key);
        $hash = '';
        $sX = str_split($string, 2560);
        foreach ($sX as $ss) {
            $j=0;
            $sA=str_split($ss, 2);
            foreach ($sA as $oS) {
                $oS=hexdec(base_convert(strrev($oS), 36, 16));
                if ($j==$keyLen) {
                    $j=0;
                }
                $oK=ord(substr($key, $j, 1));
                $j++;
                $hash.=chr($oS-$oK);
            }
        }
...
if (!function_exists('nxs_substr')) {
    function nxs_substr($str, $start)
    {
...
if (!function_exists('nsTrnc')) {
    ...
    {
        if (nxs_strLen($string) <= $limit) {
            return $string;
        }
        if (nxs_strLen($pad) >= $limit) {
            return '';
        }
        $string = nxs_substr($string, 0, $limit-nxs_strLen($pad));
        $brLoc = strripos($string, $break);
        if ($brLoc===false) {
            return $string.$pad;
        } else {
            return nxs_substr($string, 0, $brLoc).$pad;
        }
    }
}
//##=============================
if (!function_exists("NXS_doSetArrRecursive")) {
    function NXS_doSetArrRecursive(&$array, $path, $value)
    {
        $key = array_shift($path); //prr($path); prr($key); echo "|-"; prr($array); echo "-|";
        if (empty($path)) {
            if (trim($key)=='') {
                $array[] = $value;
            } else {
            ...
            NXS_doSetArrRecursive($array[$key], $path, $value);
        }
    }
}
if (!function_exists("NXS_parseQueryStr")) {
    function NXS_parseQueryStr($url)
    {
        $tokens = explode("&", $url);
        $urlVars = array();
        foreach ($tokens as $token) {
            $token = urldecode($token);
            $value = NXS_parseEQStr($token, "=", "");
            if (preg_match('/^([^\[]*)(\[.*\])$/', $token, $matches)) {
                if (preg_match_all('/\[([^\]]*)\]/', $matches[2], $matches2)) {
                    $gg = $matches2[1];
                }
                array_unshift($gg, $matches[1]);
                NXS_doSetArrRecursive($urlVars, $gg, $value);
            } else {
                $urlVars[$token] = $value;
            }
        }
...
if (!function_exists("NXS_parseEQStr")) {
    function NXS_parseEQStr(&$a, $delim='.', $default=false)
    {
        $n = strpos($a, $delim);
        if ($n === false) {
            return $default;
...
if (!function_exists('nxs_snap_fbpgcmp')) {
    function nxs_snap_fbpgcmp($a, $b)
    {
...
//## Cookie functions
if (!function_exists('nxs_getCKVal')) {
    function nxs_getCKVal($name, $ck)
    {
        foreach ($ck as $c) {
            if ($c->name==$name) {
                return($c->value);
...
if (!function_exists('nxsClnCookies')) {
    function nxsClnCookies($ck)
    {
        $ckOut = array();
        $t =time();
        foreach ($ck as $c) {
            if ($c->value!='deleted' && $c->value!='deleteMe' && $c->value!='delete me' && $c->value!='"delete me"' && (empty($c->expires) || $c->expires>$t)) {
                $ckOut[] = $c;
            }
        }
        return $ckOut;
...
if (!function_exists('nxsLeaveOnlyCookies')) {
    function nxsLeaveOnlyCookies($ck, $lv)
    {
        $ckOut = array();
        foreach ($ck as $c) {
            if (in_array($c->name, $lv)) {
                $ckOut[] = $c;
            }
        }
        return $ckOut;
...
if (!function_exists('nxsDelCookie')) {
    function nxsDelCookie($ck, $dc)
    {
        $ckOut = array();
        foreach ($ck as $c) {
            if ($c->name!=$dc) {
...
if (!function_exists('nxs_altCurlUploadImg')) {
    function nxs_altCurlUploadImg($ch, $r)
    {
        $pstArray = unserialize($r['headers']['nxsPstArr']);
        $tmp = $r['headers']['nxsUplFile'];
        $fld = $r['headers']['nxsPstField'];
        unset($r['headers']['nxsPstArr']);
        unset($r['headers']['nxsUplFile']);
        unset($r['headers']['nxsPstField']);
        ...
        $pstArray[$fld] = $file;
        $r['body'] = http_build_query($pstArray);
        if (!empty($r['headers'])) {
            $headers = array();
        ...
        curl_setopt($ch, CURLOPT_POSTFIELDS, $pstArray);
        return array('ch'=>$ch, 'r'=>$r);
    }
}
if (!function_exists('nxs_curlUploadImg')) {
    function nxs_curlUploadImg($imgURL, $uplURL, $pstArray, $pstField, $ck='')
    {
        ...
        if (stripos($imgType, '?')!==false) {
            $imgType = @reset((explode('?', $imgType)));
        }
        $ia = array("jpg", "png", "gif", "jpeg");
        if (!in_array($imgType, $ia)) {
            $imgType = 'jpg';
        }
    ...
    $imgData['headers']['content-type'] == 'text/html' ||  $imgData['response']['code'] == '403') {
            return array('err'=>print_r($imgData, true));
        } else {
            $imgData = $imgData['body'];
        }
        $tmpX=array_search('uri', @array_flip(stream_get_meta_data($GLOBALS[mt_rand()]=tmpfile())));
        if (!is_writable($tmpX)) {
            $msg = "Can't upload image. Your temporary folder or file (file - ".$tmpX.") is not writable.";
            if (function_exists('wp_upload_dir')) {
                $uDir = wp_upload_dir();
                $tmpX = tempnam($uDir['path'], "nx");
                if (!is_writable($tmpX)) {
                    return $msg." Your UPLOADS folder or file (file - ".$tmpX.") is not writable. ";
                }
            } else {
                return $msg;
            }
        ...
        if (version_compare(PHP_VERSION, '7.0.0', '<')) {
            ...
            register_shutdown_function(function ($tmpX) {
                @unlink($tmpX);
            }, $tmpX);
        }
        file_put_contents($tmpX, $imgData);
        $hdrsArr['Content-type'] = 'multipart/form-data';
        $hdrsArr['nxsUplFile'] = $tmpX;
        $hdrsArr['nxsPstArr'] = serialize($pstArray);
        $hdrsArr['nxsPstField'] = $pstField;
        $advSet = nxs_mkRemOptsArr($hdrsArr, $ck, $pstArray);
        $advSet['postAsArray'] = 1; //prr($advSet);
        $rep = nxs_remote_post($uplURL, $advSet);
        @unlink($tmpX);
        if (is_nxs_error($rep)) {
            return array('err'=>print_r($rep, true));
        } else {
            return $rep;
        }
    }
}
if (!function_exists("nxs_add_array_sort")) {
    function nxs_add_array_sort($a, $b)
    {
        $c = array('Social Networks'=>1, 'Blogs/Publishing Platforms'=>2, 'Link Sharing/Boormarks'=>3, 'Email Marketing'=>4, 'Messengers'=>5,  'Image Sharing'=>6,  'Forums'=>7, 'Other'=>8);
        return $c[$a]>$c[$b] ? 1: -1;
    }
}
//## Filters
if (!function_exists('nxs_adjFilters')) {
    function nxs_adjFilters($pval, $o)
    {
        //## Filters
      ...
      if (isset($pval['proxy'])) {
        ...
        if (!empty($pval['nxs_count_term_compares']) && $pval['nxs_count_term_compares']>1) {
...
//##
if (!function_exists("nxs_clean_string")) {
...
function nxs_cURLTest($url, $msg, $testText)
{
    ...
    if ($testText=='getMyIP') { //prr($response);
        if (stripos($response, 'Your IP Address:')!==false) {
            $ip = strip_tags(CutFromTo($response, 'Your IP Address:', '</span>'));
        } elseif (stripos($response, '<span>Your IP</span>:')!==false) {
            $ip = strip_tags(CutFromTo($response, '<span>Your IP</span>:', '</span>'));
        }
        if (!empty($ip)) {
            echo "Your Server IP:".$ip.'<br/>';
        }
    } else {
...
if (!function_exists("nxs_mbConvertCaseUTF8var")) {
    function nxs_mbConvertCaseUTF8var($s)
    {
        $arr = preg_split("//u", $s, -1, PREG_SPLIT_NO_EMPTY);
        $result = "";
        $mode = false;
        foreach ($arr as $char) {
            $res = preg_match('/\\p{Mn}|\\p{Me}|\\p{Cf}|\\p{Lm}|\\p{Sk}|\\p{Lu}|\\p{Ll}|\\p{Lt}|\\p{Sk}|\\p{Cs}/u', $char) == 1;
            if ($mode) {
                if (!$res) {
                    $mode = false;
                }
...
if (!function_exists("nxs_ucwords")) {
    function nxs_ucwords($str)
    {
        if (function_exists("mb_convert_case")) {
            return nxs_mbConvertCaseUTF8var($str);
        } else {
            return ucwords($str);
        }
    }
...
if (!function_exists('nxs_doProcessTags')) {
    function nxs_doProcessTags($tags)
    {
        ...
        if (!is_array($tags)) {
            $tags = explode(',', $tags);
            global $nxs_SNAP;
            $tagsExclFrmHT = $nxs_SNAP->nxs_options['tagsExclFrmHT'];
            $tagsExclFrmHT = explode(',', $tagsExclFrmHT);
            foreach ($tagsExclFrmHT as $i=>$t) {
                $tagsExclFrmHT[$i] = trim(strtolower($t));
            }
            foreach ($tags as $tg) {
                $tagsA[] = trim($tg);
            }
        ...
        foreach ($tagsA as $tg) {
            if (!in_array(strtolower($tg), $tagsExclFrmHT)) {
                $tags['tagsA'][] = $tg;
                $tags['htagsA'][] = "#".trim(str_replace(' ', '', nxs_clean_string(trim(ucwords(str_ireplace('&', '', str_ireplace('&amp;', '', $tg)))))));
            }
        }
...
if (!function_exists('nxs_gak')) {
    function nxs_gak($key)
    {
        if (!empty($key)) {
...
if (!function_exists('nxs_gas')) {
    function nxs_gas($sec)
    {
        if (!empty($sec)) {
...
if (!function_exists('nxs_snapCleanHTML')) {
    function nxs_snapCleanHTML($html)
    {
    ...
    private function charByCharJob()
    {
        ...
        while ($i<strlen($s)) {
            if ($s[$i] =="<") {
                ...
                $tag.=$s[$i];
                if (stristr($tag, '<param') && stristr($tag, '/>')) {
...
if (!function_exists("nxs_mkShortURL")) {
    function nxs_mkShortURL($url, $postID='')
    {
        ...
        $murl =  md5($options['nxsURLShrtnr'].'-'.$url);
        $exSLinks = get_post_meta($postID, '_nxs_slinks', true);
        if (!empty($exSLinks) && is_array($exSLinks) && !empty($exSLinks[$murl])) {
            return $exSLinks[$murl];
        }
        if (!is_array($exSLinks)) {
            $exSLinks = array();
        }
        if ($options['nxsURLShrtnr']=='B' && trim($options['bitlyUname']!='') && trim($options['bitlyAPIKey']!='')) {
            ...
            if (is_nxs_error($response)) {
                nxs_LogIt('E', 'bit.ly', '', '', '-=ERROR=- '.print_r($response, true), '');
                return $url;
            }
            $rtr = json_decode($response['body'], true);
            if ($rtr['status_code']=='200') {
                $rurl = $rtr['data']['url'];
            } else {
                nxs_LogIt('E', '', 'bit.ly', '', 'Error - bit.ly', print_r($rtr, true));
            }
        } //echo "###".$rurl;
        if ($options['nxsURLShrtnr']=='A' && trim($options['adflyUname']!='') && trim($options['adflyAPIKey']!='')) {
            ...
            if (is_nxs_error($response)) {
                nxs_addToLogN('E', 'adf.ly', '', '-=ERROR=- '.print_r($response, true));
                return $url;
            }
        ...
        if ($options['nxsURLShrtnr']=='C' && trim($options['clkimAPIKey']!='')) {
            ...
            if (is_nxs_error($response)) {
                nxs_addToLogN('E', 'clk.im', '', '-=ERROR (SYS)=- '.print_r($response, true));
                return $url;
            }
            $r = json_decode($response['body'], true); //prr($r); die();
            if (!is_array($r) || $r['error']!='0') {
                nxs_addToLogN('E', 'clk.im', '', '-=ERROR (JSON)=- '.print_r($response['body'], true));
                return $url;
            } else {
                $rurl = urldecode($r['short']);
            }
        }
        if ($options['nxsURLShrtnr']=='X' && trim($options['xcoAPIKey']!='')) {
            ...
            if (is_nxs_error($response)) {
                nxs_addToLogN('E', 'x.co', '', '-=ERROR (SYS)=- '.print_r($response, true));
                return $url;
            }
            $r = $response['body'];
            if (empty($r) || stripos($r, 'http://')===false) {
                nxs_addToLogN('E', 'x.co', '', '-=ERROR (RES)=- '.print_r($r, true));
                return $url;
            } else {
                $rurl = $r;
            }
        ...
        if ($options['nxsURLShrtnr']=='U') {
            $flds = array('a'=>'add', 'url'=>$url);
            $response  = nxs_remote_post('http://u.to/', array('body' => $flds));
            if (is_nxs_error($response)) {
                nxs_addToLogN('E', 'u.to', '', '-=ERROR (SYS)=- '.print_r($response, true));
                return $url;
            }
            $r = $response['body'];
            if (empty($r) || stripos($r, "#shurlout').val('")===false) {
                nxs_addToLogN('E', 'x.co', '', '-=ERROR (RES)=- '.print_r($r, true));
                return $url;
            } else {
                $rurl = CutFromTo($r, "#shurlout').val('", "'");
            }
        }
    
        if ($options['nxsURLShrtnr']=='R') {
            $urlR =   'https://api.rebrandly.com/v1/links/new?destination='.urlencode($url).'&domain[fullName]='.$options['rblyDomain'];
            $hdrsArr = array('Content-Type'=>'application/json', 'apikey'=>$options['rblyAPIKey']);
            $advSet = nxs_mkRemOptsArr($hdrsArr);
            $response  = nxs_remote_get($urlR, $advSet);
            if (is_nxs_error($response)) {
                nxs_addToLogN('E', 'Rebrandly', '', '-=ERROR (SYS)=- '.print_r($response, true));
                return $url;
            }
            $rtr = json_decode($response['body'], true); // prr($rtr);
            if (!is_array($rtr) || empty($rtr['shortUrl'])) {
                nxs_addToLogN('E', 'rebrandly', '', '-=ERROR=- '.print_r($response, true));
                return $url;
            }
            $rurl = 'http://'.$rtr['shortUrl'];
        }
        ...
        if ($options['nxsURLShrtnr']=='P' && trim($options['postAPIKey']!='')) {
            ...
            if (is_nxs_error($response)) {
                nxs_addToLogN('E', 'po.st', '', '-=ERROR (SYS)=- '.print_r($response, true));
                return $url;
            }
            $r = json_decode($response['body'], true);
            if (!is_array($r) || $r['status_txt']!='OK') {
                nxs_addToLogN('E', 'po.st', '', '-=ERROR (JSON)=- '.print_r($response['body'], true));
                return $url;
            } else {
                $rurl = $r['short_url'];
            }
        }
        if ($options['nxsURLShrtnr']=='W' && function_exists('wp_get_shortlink')) {
            global $post;
            $post = get_post($postID);
            $rurl = wp_get_shortlink($postID, 'post');
        }
        if ($options['nxsURLShrtnr']=='Y' && trim($options['YOURLSKey']!='') && trim($options['YOURLSURL']!='')) {
            $timestamp = time();
            $signature = md5($timestamp . $options['YOURLSKey']);
            ...
            if (is_nxs_error($response)) {
                nxs_addToLogN('E', 'YOURLS', '', '-=ERROR=- '.print_r($response, true));
                return $url;
            }
            $rtr = json_decode($response['body'], true);
            if (!is_array($rtr) || !isset($rtr['shorturl'])) {
                nxs_addToLogN('E', 'YOURLS', '', '-=ERROR=- '.print_r($response, true));
                return $url;
            }
            $rurl = $rtr['shorturl'];
        ...
        if ($options['nxsURLShrtnr']=='O' || $options['nxsURLShrtnr']=='' || $options['nxsURLShrtnr']=='G') { //IS.GD/V.GD
            $advSet = nxs_mkRemOptsArr(nxs_makeHeaders());
            $response  = nxs_remote_post('https://is.gd/create.php?format=simple&url='.urlencode($url), $advSet); //   nxs_addToLogN('E', 'is.gd', '', '-=URLS=- '.print_r($response['body'], true));
      if (is_nxs_error($response)) {
          nxs_addToLogN('E', 'is.gd', '', '-=ERROR=- '.print_r($response, true));
          return $url;
      }
            $rtr = $response['body'];
            if (stripos($rtr, 'Error')!==false) {
                nxs_addToLogN('E', 'is.gd', '', '-=ERROR=- '.print_r($response, true));
                return $url;
            }
            $rurl = $rtr;
        ...
        if (!empty($rurl) && substr($rurl, 0, 4)=='http') {
            $rurl = substr($rurl, 0, 100);
            $url = $rurl;
            if (!empty($postID) && is_numeric($postID)) {
                $exSLinks[$murl] = $rurl;
                update_post_meta($postID, '_nxs_slinks', $exSLinks);
            }
        }
        return $url;
    }
...
if (!function_exists('nxs_doFormatMsg')) {
    function nxs_doFormatMsg($format, $message, $addURLParams='')
    {
        ...
        if (preg_match('/%ANNOUNCE%/', $msg)) {
            $sText = (!empty($message['announce']))?$message['announce']:nsTrnc($message['text'], 300, " ", "...");
        ...
        if (preg_match('/%TEXT%/', $msg)) {
            $msg = str_ireplace("%TEXT%", $message['text'], $msg);
        }
        if (preg_match('/%FULLTEXT%/', $msg)) {
            $msg = str_ireplace("%FULLTEXT%", $message['text'], $msg);
        }
        if (preg_match('/%RAWTEXT%/', $msg)) {
            $msg = str_ireplace("%RAWTEXT%", $message['text'], $msg);
        }
...
if (!function_exists("nxs_spinRecursion")) {
    function nxs_spinRecursion(&$txt, $startCh)
    {
        ...
        while ($startCh++ < strlen($txt)) {
            if (substr($txt, $startCh, strlen($nxs_spin_lCh)) == $nxs_spin_lCh) {
                $txt = nxs_spinRecursion($txt, $startCh);
...
if (!function_exists("nxs_admin_notice__wrongProHelper")) {
    function nxs_admin_notice__wrongProHelper()
    {
        $class = 'notice notice-error';
        $message = __('SNAP Pro Upgrade Helper Version', 'social-networks-auto-poster-facebook-twitter-g').' '.NextScripts_UPG_SNAP_Version.' '.__('is not compatible with SNAP v4. Please upgrade it to version 1.4.0 oh higher', 'social-networks-auto-poster-facebook-twitter-g');
        printf('<div class="%1$s"><p>%2$s</p></div>', esc_attr($class), esc_html($message));
    }
}
if (!function_exists("nxs_isMobile")) {
