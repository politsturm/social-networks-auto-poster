    ...
    class nxs_Http
    {
        ...
        public function request($url, $args = array())
        {
            ...
            if (empty($url) || empty($arrURL['scheme'])) {
                return new nxs_Error('http_request_failed', 'A valid URL was not provided.');
            }
            ...
            if (nxs_Http_Encoding::is_available()) {
                $r['headers']['Accept-Encoding'] = nxs_Http_Encoding::accept_encoding();
            }
                ...
                if (!empty($r['usearray']) && is_array($r['body'])) {
                    ...
                    if (is_array($r['body']) || is_object($r['body'])) {
                        ...
                        if (! isset($r['headers']['Content-Type'])) {
                            $r['headers']['Content-Type'] = 'application/x-www-form-urlencoded; charset=' . ($r['charset']!==''?$r['charset']:'utf-8');
                        }
                    }
                    if ('' === $r['body']) {
                        $r['body'] = null;
        ...
        public function sendReq($url, $type='GET', $args = array())
        {
        ...
        { // prr($headers);
            ...
            foreach ((array) $headers as $tempheader) {
                ...
                if ('set-cookie' == $key) {
                    $cookies[] = new nxs_Http_Cookie($value);
                }
        ...
        public static function buildCookieHeader(&$r)
        {
            if (! empty($r['cookies'])) {
                ...
                foreach ((array) $r['cookies'] as $cookie) {
                    if (is_object($cookie) && method_exists($cookie, 'getHeaderValue')) {
        ...
        public function curl_request($url, $args = array())
        {
            ...
            //## NXS
            if (isset($r['proxy'])) {
                if (isset($r['proxy']) && isset($r['proxy']['proxy'])) {
                    $prx = explode(':', $r['proxy']['proxy']);
                } else {
                    $prx = false;
                }
                if (isset($r['proxy']) && isset($r['proxy']['up'])) {
                    $pru = explode(':', $r['proxy']['up']);
                } else {
                    $pru = false;
                }
            ...
            default: curl_setopt($handle, CURLOPT_CUSTOMREQUEST, $r['method']); if (! is_null($r['body'])) {
            ...
            if (!empty($args['postAsArray'])) {
                $tt = nxs_altCurlUploadImg($handle, $r);
                $handle = $tt['ch'];
                $r = $tt['r'];
            }
            ...
            if (0 == strlen($theResponse) && empty($theHeaders['headers'])) {
                if ($curl_error = curl_error($handle)) {
                    return new nxs_Error('http_request_failed', $curl_error);
                }
                if (in_array(curl_getinfo($handle, CURLINFO_HTTP_CODE), array( 301, 302 ))) {
                    return new nxs_Error('http_request_failed', 'Too many redirects.');
                }
            }
            $this->headers = '';
            $response = array();
            ...
            if (true === $r['decompress'] && true === nxs_Http_Encoding::should_decode($theHeaders['headers'])) {
                $theBody = nxs_Http_Encoding::decompress($theBody);
            }
...
if (!class_exists('nxs_HTTP_Proxy')) {
    class nxs_HTTP_Proxy
    {
        ...
        {
            if (isset($this->nxs_PROXY_HOST)) {
                return $this->nxs_PROXY_HOST;
            }
            return '';
        }
        public function port()
        {
            if (isset($this->nxs_PROXY_PORT)) {
                return $this->nxs_PROXY_PORT;
            }
            return '';
        }
        public function username()
        {
            if (isset($this->nxs_PROXY_USERNAME)) {
                return $this->nxs_PROXY_USERNAME;
            }
            return '';
        }
        public function password()
        {
            if (isset($this->nxs_PROXY_PASSWORD)) {
                return $this->nxs_PROXY_PASSWORD;
            }
            return '';
        }
        ...
        public function send_through_proxy($uri)
        {
            ...
            if ($check['host'] == 'localhost' || (isset($_SERVER['SERVER_NAME']) && $check['host'] == $_SERVER['SERVER_NAME'])) {
                return false;
            }
            return true;
        }
...
if (!class_exists('nxs_Http_Encoding')) {
    class nxs_Http_Encoding
    {
        ...
        public static function decompress($compressed, $length = null)
        {
            ...
            if (false !== ($decompressed = nxs_Http_Encoding::compatible_gzinflate($compressed))) {
                return $decompressed;
            }
...
if (!function_exists("is_nxs_error")) {
    ...
    {
        if (is_object($thing) && (is_a($thing, 'nxs_Error') ||  is_a($thing, 'wp_Error'))) {
            return true;
        }
        return false;
    }
...
if (!function_exists("nxs_getHttpStatusDesc")) {
    function nxs_getHttpStatusDesc($code)
    {
      ...
      507 => 'Insufficient Storage',510 => 'Not Extended'
        ...
        if (isset($httpRetCodes[$code])) {
            return $httpRetCodes[$code];
        } else {
            return '';
        }
    }
...
if (!function_exists("nxs_makeHeaders")) {
    function nxs_makeHeaders($ref='', $org='', $type='GET', $aj=false)
    {
        $hdrsArr = array();
...
if (!function_exists("nxs_getNXSHeaders")) {
    function nxs_getNXSHeaders($ref='', $post=false)
    {
        return nxs_makeHeaders($ref, '', $post?'POST':'GET');
    }
...
if (!function_exists("nxs_mkRemOptsArr")) {
    function nxs_mkRemOptsArr($hdrsArr, $ck='', $flds='', $p='', $rdr=0, $timt=45, $sslverify = false)
    {
        ...
        if (empty($hdrsArr)) {
            $hdrsArr = nxs_makeHeaders('http://'.$_SERVER['HTTP_HOST']);
        }
        $a = array('headers' => $hdrsArr, 'httpversion' => '1.1', 'timeout' => $timt, 'redirection' => $rdr, 'sslverify'=>$sslverify, 'user-agent'=>$ua);
        if (!empty($flds)) {
            $a['body'] = $flds;
        }
        if (!empty($p)) {
