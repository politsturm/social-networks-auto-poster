...
if (!function_exists("doConnectToWaNeLo")) {
  ...
  if ($contents['url']=='http://wanelo.com/users/sign_in') {
...
if (!function_exists("doGetBoardsFromWaNeLo")) {
    function doGetBoardsFromWaNeLo()
    {
        ...
        foreach ($txta as $txti) {
            if (stripos($txti, "'followers-count'")!==false) {
  ...
  if (stripos($contents, 'blocked this')!==false) {
      ...
      return "WaNeLo ERROR: 'The Source is blocked'. Please see https://support.WaNeLo.com/entries/21436306-why-is-my-pin-or-site-blocked-for-spam-or-inappropriate-content/ for more info | WaNeLo Message:".$retText;
        ...
        if (stripos($contents, 'Oops')!==false && stripos($contents, '<body>')!==false) {
            return 'WaNeLo ERROR MESSAGE : '.trim(str_replace(array("\r\n", "\r", "\n"), " | ", strip_tags(CutFromTo($contents, '<body>', '</body>'))));
        }
        if (stripos($contents, 'pinSuccess')!==false) {
            ...
            return "Somethig is Wrong - WaNeLo Returned Error 502";
        }
    }
}
...
if (!function_exists("nxs_getWLHeaders")) {
    function nxs_getWLHeaders($ref, $post=false, $xhr=false)
    {
        ...
        if (is_array($post)) {
            $hdrsArr = array_merge($hdrsArr, $post);
        }
        if ($xhr) {
...
if (!function_exists("doConnectToWaNeLo2")) {
    function doConnectToWaNeLo2($email, $pass)
    {
        ...
        $response = nxs_remote_get('http://wanelo.com/users/sign_in', array( 'headers' => $hdrsArr));
        if (is_wp_error($response)) {
            return "Something wrong".print_r($response);
        }
        $contents = $response['body'];
        ...
        if ($response['headers']['location']=='http://wanelo.com/users/sign_in' || $response['headers']['location']=='https://wanelo.com/users/sign_in') {
            return 'Incorrect Username/Password ';
        }
  ...
  $hdrsArr = nxs_getWLHeaders('http://wanelo.com/', false, $nxs_gTkn);
        ...
        $user = $jsn['user'];
        if ($user=='null' || !is_array($user)) {
            return "Invalid login - no user.".print_r($response);
        }
...
if (!function_exists("doPostToWaNeLo2")) {
    function doPostToWaNeLo2($msg, $imgURL, $lnk, $boardID, $title = '', $price='', $via='')
    {
        ...
        $contents = $response['body'];
        $jsn = json_decode($contents, true);
        if (is_array($jsn) && isset($jsn['user']) && isset($jsn['user']['id'])) {
            $userID = $jsn['user']['id'];
        } else {
            return "Invalid user";
        }
        ...
        */
        ...
        if ($response['response']['code']=='302') {
            ...
            if (stripos($postID, 'sign_in')!==false) {
...
if (!class_exists("nxs_snapClassWL")) {
    ...
    <div class="nxs_box">
      ...
      <div class="nxs_box_inside">
        <?php if (!function_exists('doPostToWaNeLo')) {
                ?>  WaNeLo doesn't have a built-in API for automated posts yet. <br/>You need to get a special <a target="_blank" href="http://www.nextscripts.com/wanelo-automated-posting">library module</a> to be able to publish your content to WaNeLo.
        <?php
            } else {
          ...
          <p style="margin:0px;margin-left:5px;">
            <input value="1" name="<?php echo $ntInfo['lcode']; ?>[<?php echo $indx; ?>][apDo<?php echo $ntInfo['code']; ?>]" onchange="doShowHideBlocks('<?php echo $ntInfo['code']; ?>');" type="checkbox" <?php if ((int)$pbo['do'.$ntInfo['code']] == 1) {
                        echo "checked";
                    } ?> /> <?php if ((int)$pbo['catSel'] == 1) {
                        ?>   <span onmouseout="nxs_hidePopUpInfo('popOnlyCat');" onmouseover="nxs_showPopUpInfo('popOnlyCat', event);"><?php echo "*[".(substr_count($pbo['catSelEd'], ",")+1)."]*" ?></span><?php
                    } ?>
            <strong><?php  _e('Auto-publish to', 'social-networks-auto-poster-facebook-twitter-g'); ?> <?php echo $ntInfo['name']; ?> <i style="color: #005800;"><?php if ($pbo['nName']!='') {
                        echo "(".$pbo['nName'].")";
                    } ?></i></strong>
          &nbsp;&nbsp;<?php if ($ntInfo['tstReq'] && (!isset($pbo[$ntInfo['lcode'].'OK']) || $pbo[$ntInfo['lcode'].'OK']=='')) {
                        ?><b style="color: #800000"><?php  _e('Attention requred. Unfinished setup', 'social-networks-auto-poster-facebook-twitter-g'); ?> ==&gt;</b><?php
                    } ?><a id="do<?php echo $ntInfo['code'].$indx; ?>A" href="#" onclick="doShowHideBlocks2('<?php echo $ntInfo['code'].$indx; ?>');return false;">[<?php  _e('Show Settings', 'social-networks-auto-poster-facebook-twitter-g'); ?>]</a>&nbsp;&nbsp;
          <a href="#" onclick="doDelAcct('<?php echo $ntInfo['lcode']; ?>', '<?php echo $indx; ?>', '<?php if (isset($pbo['bgBlogID'])) {
                        echo $pbo['nName'];
                    } ?>');return false;">[<?php  _e('Remove Account', 'social-networks-auto-poster-facebook-twitter-g'); ?>]</a>
        ...
        public function showNTSettings($ii, $options, $isNew=false)
        {
            ...
            if (!isset($options['qTLng'])) {
             ...
             <div id="doWL<?php echo $ii; ?>Div" class="insOneDiv<?php if ($isNew) {
                echo " clNewNTSets";
            } ?>">     <input type="hidden" name="apDoSWL<?php echo $ii; ?>" value="0" id="apDoSWL<?php echo $ii; ?>" />         
            ...
            <div id="doWL<?php echo $ii; ?>Div" style="margin-left: 10px;"> <div class="nsx_iconedTitle" style="float: right; background-image: url(<?php echo $nxs_plurl; ?>img/wl16.png);"><a style="font-size: 12px;" target="_blank"  href="http://www.nextscripts.com/setup-installation-wanelo-social-networks-auto-poster-wordpress/"><?php $nType="WaNeLo";
            printf(__('Detailed %s Installation/Configuration Instructions', 'social-networks-auto-poster-facebook-twitter-g'), $nType); ?></a></div>
    ...
    <div id="nsx<?php echo $nt.$ii ?>_tab1" class="nsx_tab_content" style="background-image: url(<?php echo $nxs_plurl; ?>img/<?php echo $nt; ?>-bg.png); background-repeat: no-repeat;  background-position:90% 10%;">
            ...
            Please <a href="#" onclick="getWLBoards(jQuery('<?php if ($isNew) {
                echo "#nsx_addNT ";
            } ?>#apWLUName<?php echo $ii; ?>').val(),jQuery('<?php if ($isNew) {
                echo "#nsx_addNT ";
            } ?>#apWLPass<?php echo $ii; ?>').val(), '<?php echo $ii; ?>'); return false;">click here to retrieve your boards</a>
            </div>
            ...
            <?php if ($options['wlBoardsList']!='') {
                ...
                if (base64_encode(base64_decode($gWLBoards)) === $gWLBoards) {
                    $gWLBoards = base64_decode($gWLBoards);
                }
                if ($options['wlBoard']!='') {
                    $gWLBoards = str_replace($options['wlBoard'].'"', $options['wlBoard'].'" selected="selected"', $gWLBoards);
                }
                echo $gWLBoards;
            } else {
                ?>
            ...
            <div id="altFormat" style="">
              ...
              </div><input  name="wl[<?php echo $ii; ?>][apWLMsgFrmt]" id="apWLMsgFrmt" style="width: 50%;" value="<?php if ($options['wlMsgFormat']!='') {
                _e(apply_filters('format_to_edit', htmlentities($options['wlMsgFormat'], ENT_COMPAT, "UTF-8")), 'social-networks-auto-poster-facebook-twitter-g');
            } else {
                echo "%TITLE% - %URL%";
            ...
            <?php if ($isNew) {
                ?> <input type="hidden" name="wl[<?php echo $ii; ?>][apDoWL]" value="1" id="apDoNewWL<?php echo $ii; ?>" /> <?php
            } ?>
    ...
    <?php if (!$isNew) {
                ?> <div class="nxs_tls_cpt"><?php _e('Categories', 'social-networks-auto-poster-facebook-twitter-g'); ?></div>
    <div style="width:100%;"><strong><?php _e('Categories', 'social-networks-auto-poster-facebook-twitter-g'); ?>:</strong>
       <input value="0" id="catSelA<?php echo $ii; ?>" type="radio" name="<?php echo $nt; ?>[<?php echo $ii; ?>][catSel]" <?php if ((int)$options['catSel'] != 1) {
                    echo "checked";
                } ?> /> All                                  
       <input value="1" id="catSelS<?php echo $ntU; ?><?php echo $ii; ?>" type="radio" name="<?php echo $nt; ?>[<?php echo $ii; ?>][catSel]" <?php if ((int)$options['catSel'] == 1) {
                    echo "checked";
                } ?> /> <a href="#" style="text-decoration: none;" class="showCats" id="nxs_SCA_<?php echo $ntU; ?><?php echo $ii; ?>" onclick="jQuery('#catSelS<?php echo $ntU; ?><?php echo $ii; ?>').attr('checked', true); jQuery('#tmpCatSelNT').val('<?php echo $ntU; ?><?php echo $ii; ?>'); nxs_markCats( jQuery('#nxs_SC_<?php echo $ntU; ?><?php echo $ii; ?>').val() ); jQuery('#showCatSel').bPopup({ modalClose: false, appendTo: '#nsStForm', opacity: 0.6, follow: [false, false], position: [75, 'auto']}); return false;">Selected<?php if ($options['catSelEd']!='') {
  ...
  public function setNTSettings($post, $options)
  {
    ...
    foreach ($post as $ii => $pval) {
        if (isset($pval['apWLUName']) && $pval['apWLUName']!='') {
            ...
            if (isset($pval['apDoWL'])) {
                $options[$ii]['doWL'] = $pval['apDoWL'];
            } else {
                $options[$ii]['doWL'] = 0;
            }
            if (isset($pval['nName'])) {
            ...
            if (isset($pval['apWLUName'])) {
                $options[$ii]['wlUName'] = trim($pval['apWLUName']);
            }
            if (isset($pval['apWLPass'])) {
                $options[$ii]['wlPass'] = 'g9c1a'.nsx_doEncode($pval['apWLPass']);
            } else {
                $options[$ii]['wlPass'] = '';
            }
            if (isset($pval['apWLBoard'])) {
                $options[$ii]['wlBoard'] = trim($pval['apWLBoard']);
            }
            if (isset($pval['apWLDefImg'])) {
                $options[$ii]['wlDefImg'] = trim($pval['apWLDefImg']);
            }
            ...
            if (isset($pval['apWLMsgFrmt'])) {
                $options[$ii]['wlMsgFormat'] = trim($pval['apWLMsgFrmt']);
            }
      ...
      <tr><th style="text-align:left;" colspan="2"><?php if ($ntOpt['catSel']=='1' && trim($ntOpt['catSelEd'])!='') {
                    ?> <input type="hidden" class="nxs_SC" id="nxs_SC_WL<?php echo $ii; ?>" value="<?php echo $ntOpt['catSelEd']; ?>" /> <?php
                } ?>
      <?php if ($isAvailWL) {
                    ?><input class="nxsGrpDoChb" value="1" id="doWL<?php echo $ii; ?>"  type="checkbox" name="wl[<?php echo $ii; ?>][doWL]" <?php if ((int)$doWL == 1) {
                        echo 'checked="checked" title="def"';
                    } ?> /> 
      <?php if ($post->post_status == "publish") {
                        ?> <input type="hidden" name="wl[<?php echo $ii; ?>][doWL]" value="<?php echo $doWL; ?>"> <?php
                    } ?> <?php
                } ?>
      <div class="nsx_iconedTitle" style="display: inline; font-size: 13px; background-image: url(<?php echo $nxs_plurl; ?>img/wl16.png);">WaNeLo - <?php _e('publish to', 'social-networks-auto-poster-facebook-twitter-g') ?> (<i style="color: #005800;"><?php echo $ntOpt['nName']; ?></i>)</div></th> <td><?php //## Only show RePost button if the post is "published"
                    if ($post->post_status == "publish" && $isAvailWL) {
                        ?><input alt="<?php echo $ii; ?>" style="float: right;" onmouseout="hidePopShAtt('SV');" onmouseover="showPopShAtt('SV', event);" onclick="return false;"  type="button" class="button" name="rePostToWL_repostButton" id="rePostToWL_button" value="<?php _e('Repost to WaNeLo', 'social-networks-auto-poster-facebook-twitter-g') ?>" />
                    <?php
                    } ?>
          ...
          <a style="font-size: 10px;" href="http://WaNeLo.com<?php echo $pMeta[$ii]['pgID']; ?>" target="_blank"><?php $nType="WaNeLo";
                        printf(__('Posted on', 'social-networks-auto-poster-facebook-twitter-g'), $nType); ?>  <?php echo (isset($pMeta[$ii]['pDate']) && $pMeta[$ii]['pDate']!='')?(" (".$pMeta[$ii]['pDate'].")"):""; ?></a>
                    </span><?php
                    } ?>
                ...
                <?php if (!$isAvailWL) {
                        ?><tr><th scope="row" style="text-align:right; width:150px; padding-top: 5px; padding-right:10px;"></th> <td><b>Setup your WaNeLo Account to AutoPost to WaNeLo</b>
                <?php
                    } else {
                ...
                <input value="0"  type="hidden" name="<?php echo $nt; ?>[<?php echo $ii; ?>][rpstPostIncl]"/><input value="1" type="checkbox" name="<?php echo $nt; ?>[<?php echo $ii; ?>][rpstPostIncl]"  <?php if ((int)$ntOpt['rpstPostIncl'] == 1) {
                                echo "checked";
                            } ?> /> 
                </th>
            ...
            <?php if (!empty($ntOpt['wlBoardsList'])) {
                            ...
                            if ($ntOpt['wlBoard']!='') {
                                $gWLBoards = str_replace($ntOpt['wlBoard'].'"', $ntOpt['wlBoard'].'" selected="selected"', $gWLBoards);
                            }
                            echo $gWLBoards;
                        } else {
                            ?>
        ...
        {
            if (isset($pMeta['isPosted'])) {
                $optMt['isPosted'] = $pMeta['isPosted'];
            } else {
                $optMt['isPosted'] = '';
            }
            if (isset($pMeta['SNAPformat'])) {
                $optMt['wlMsgFormat'] = $pMeta['SNAPformat'];
            }
            ...
            if (isset($pMeta['rpstPostIncl'])) {
                $optMt['rpstPostIncl'] = $pMeta['rpstPostIncl'];
            }
            if (isset($pMeta['apWLBoard']) && $pMeta['apWLBoard']!='' && $pMeta['apWLBoard']!='0') {
                $optMt['wlBoard'] = $pMeta['apWLBoard'];
            }
            if (isset($pMeta['doWL'])) {
                $optMt['doWL'] = $pMeta['doWL'] == 1?1:0;
            } else {
                if (isset($pMeta['SNAPformat'])) {
                    $optMt['doWL'] = 0;
                }
            }
            if (isset($pMeta['SNAPincludeWL']) && $pMeta['SNAPincludeWL'] == '1') {
                $optMt['doWL'] = 1;
            }
            return $optMt;
        }
    ...
    function nxs_rePostToWL_ajax()
    {
      ...
      if (is_array($po) && isset($po[$ii]) && is_array($po[$ii])) {
          ...
          $two = $ntClInst->adjMetaOpt($two, $po[$ii]);
                ...
                if ($result == 200) {
                    die("Successfully sent your post to WaNeLo.");
                } else {
                    die($result);
                }
            }
...
if (!function_exists("nxs_doPublishToWL")) { //## Second Function to Post to G+
    function nxs_doPublishToWL($postID, $options)
    {
        ...
        if ($postID=='0') {
            ...
            $imgURL = nxs_getPostImage($postID, !empty($options['wpImgSize'])?$options['wpImgSize']:'large', $options['wlDefImg']); //prr($options); echo $imgURL."######"; // echo "WW".$postID."|";
            if ($isAttachVid=='1') {
        ...
        if (isset($options['wlSvC'])) {
            $nxs_gCookiesArr = maybe_unserialize($options['wlSvC']);
        }
        $loginError = true;
        if (is_array($nxs_gCookiesArr)) {
            $loginError = doCheckWaNeLo2();
        }
        $extInfo = ' | PostID: '.$postID." - ".$post->post_title;
        if ($loginError!==false) {
            $loginError = doConnectToWaNeLo2($email, $pass);
        }
        if ($loginError!==false) {
            echo $loginError;
