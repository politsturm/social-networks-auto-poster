...
if (!class_exists("nxs_class_SNAP_FB")) {
    class nxs_class_SNAP_FB
    {
        ...
        public function doPostToNT($options, $message)
        {
            ...
            if (empty($options['accessToken']) && empty($options['pageAccessToken']) && empty($options['tpt']) && empty($options['uPass'])) {
      ...
      if (empty($options['tpt']) && !empty($options['uPass'])) {
          if (!class_exists('nxsAPI_FB')) {
              $badOut['Error'] = 'Premium Facebook API Library not found';
              return $badOut;
          }
          $email = $options['uName'];
          ...
          $message['pText'] = $msg;
          $message['postType'] = $options['postType'];
          if ($message['postType']=='I' ||  $message['postType']=='A') {
          ...
          if (empty($options['tpt'])) {
              if (!empty($options['glpid'])) {
                  $message['glpid'] = $options['glpid'];
              }
              $nt = new nxsAPI_FB();
              ...
              $res = $nt->post($options['pgID'], $message);
              if (!empty($res) && is_array($res) && !empty($res['isPosted'])) {
                  return $res;
              } else {
            ...
            //## Make Post
            if (!empty($options['accessToken'])&& empty($options['pageAccessToken'])) {
                $options['pageAccessToken'] = $options['accessToken'];
            }
            //## Get URL info.
      if ($options['postType']!='I' && $options['postType']!='T') {
          $url=$message['url']; //#### Let's ask FB to scrape/re-scrape it.
        $flds=array('id'=>$url, 'scrape'=>'true', 'access_token'=>$options['accessToken'], 'method'=>'post', 'limit'=>250);
          if (empty($options['tpt'])) {
              $flds['appsecret_proof'] = hash_hmac('sha256', $options['accessToken'], nxs_gas($options['appSec']));
          }
          sleep(2);
          $advSet = nxs_mkRemOptsArr(nxs_getNXSHeaders(), '', $flds);
          $response =  nxs_remote_post('https://graph.facebook.com', $advSet);
        
          if (is_nxs_error($response)) {
              $badOut['Error'] = "Error(URL-Info): ". print_r($response, true);
          } else {
              $response = json_decode($response['body'], true);
              global $nxs_SNAP;
              if (!empty($nxs_SNAP)) {
                  $gOptions = $nxs_SNAP->nxs_options;
                  { if (isset($gOptions['extDebug']) && $gOptions['extDebug']=='1') {
                      nxs_LogIt('S', 'Facebook URL Debug', 'FB', '', 'Facebook OG Metatags URL Scrape Info: ', print_r($response, true));
                  }}
              }
            ...
            if (!empty($options['fbURL']) && stripos($options['fbURL'], '/groups/')!=false) {
                $options['destType'] = 'gr';
            }
            if (!empty($options['pgID']) && substr($options['pgID'], 0, 1)=='p') {
                $options['pgID'] =  substr($options['pgID'], 1);
            }
            if (!empty($options['destType']) && $options['destType'] == 'pr') {
                $page_id = $options['authUser'];
            } else {
      ...
      if (empty($msg)) {
          if (function_exists('nxs_LogIt')) {
              nxs_LogIt('W', 'Warning', 'FB - '.$options['nName'], '', 'Your FB message is empty.', '', 'snap');
          }
          $msg = html_entity_decode("&#12288;");
      }       //    $msg = html_entity_decode("&#128078;");
      if (!empty($options['pageAccessToken'])) {
          $mssg = array('access_token'=>$options['pageAccessToken'], 'method'=>'post', 'message'=>$msg);
          if (empty($options['tpt'])) {
              $mssg['appsecret_proof'] = hash_hmac('sha256', $options['pageAccessToken'], nxs_gas($options['appSec']));
          } //prr($mssg);
        
          if ($options['postType']=='I' && trim($imgURL)=='') {
              $options['postType']='T';
          }
          ...
          if ($options['postType']=='A' || $options['postType']=='') {
              $attArr = array('link' =>$message['url']);
              $mssg = array_merge($mssg, $attArr);
              ;
          } elseif ($options['postType']=='I') { /* $facebook->setFileUploadSupport(true); */ $fbWhere = 'photos';
              $mssg['url'] = $imgURL;
              $mssg['caption'] =  $mssg['message'];
                      ...
                      if (empty($albums)) {
                          ...
                          if (is_array($albums) && is_array($albums["data"])) {
                              foreach ($albums["data"] as $album) {
                                  if (!empty($album["type"]) && $album["type"] == "wall") {
                                      $chosen_album = $album;
                                      break;
                                  }
                              }
          ...
          if (!empty($mssg['name']) && function_exists('mb_strcut')) {
              mb_internal_encoding('UTF-8');
              $mssg['name'] = mb_strcut($mssg['name'], 0, 250);
          }
          //## Actual Post
        ...
        $response = nxs_remote_post($destURL, nxs_mkRemOptsArr(nxs_getNXSHeaders('', true), '', $mssg)); // prr($response);
            ...
            if (empty($res)) {
                return "JSON ERROR: ".print_r($response, true);
            }
            if (!empty($res['error'])) {
                if (!empty($res['error']['message'])) {
                    ...
                    if (stripos($res['error']['message'], 'This API call requires a valid app_id')!==false) {
                        if (!is_numeric($page_id) && stripos($options['fbURL'], '/groups/')!=false) {
                            $badOut['Error'] .= ' [ERROR] Unrecognized Facebook Group ID. Please use numeric ID. Please see <a href="http://gd.is/f412">FAQ 4.12</a>';
                        } else {
                    ...
                    if (stripos($res['error']['message'], 'Some of the aliases you requested do not exist')!==false) {
                        $badOut['Error'] .= '| Please check what do you have in the "Facebook URL" field.';
                    }
                    if (stripos($res['error']['message'], 'Unsupported post request')!==false) {
                        $badOut['Error'] .= "<br/>\r\n".'| Are you posting to a secret or closed group? Please see: <a target="_blank" href="http://gd.is/fbe2">http://gd.is/fbe2</a>';
                    }
                    if (stripos($res['error']['message'], 'The target user has not authorized this action')!==false) {
                        $badOut['Error'] .= '| Please Authorize the plugin from the plugin settings Page - Facebook.';
                    }
                    if (stripos($res['error']['message'], 'The user has not authorized the application')!==false && $options['destType'] == 'gr') {
                        $badOut['Error'] .= '| Are you posting to a secret or closed group? Please see: <a target="_blank" href="http://gd.is/fbe2">http://gd.is/fbe2</a>';
                    }
        
                    return $badOut;
                } else {
                    return print_r($res['error'], true);
                }
            }
            if (empty($res['id'])) {
                return print_r($res, true);
