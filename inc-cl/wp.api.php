...
if (!class_exists("nxs_class_SNAP_WP")) {
    class nxs_class_SNAP_WP
    {
        ...
        public function doPostToNT($options, $message)
        {
            ...
            //## Format
            if (empty($message['orID'])) {
                $message['orID']='';
            }
            if (empty($message['tags'])) {
                $message['tags']='';
            }
            if (empty($message['cats'])) {
                $message['cats']='';
            }
            ...
            if ($imgURL!=='' && stripos($imgURL, 'http')!==false) {
                ...
                $filedata = nxs_remote_get($imgURL);
                if (! is_nxs_error($filedata)) {
                    $filedata = $filedata['body'];
                } // echo "AWC?";
        $data = array('name'  => 'image-'.$message['orID'].'.jpg', 'type'  => 'image/jpg', 'bits'  => new NXS_XMLRPC_Base64($filedata), true);
                $status = $nxsToWPclient->query('metaWeblog.newMediaObject', $message['orID'], $options['uName'], $pass, $data);
            ...
            if ($rwpOpt==0) {
                ...
                if ($rwpOpt>3.3) {
                    ...
                    if ($gid!='') {
                        ...
                        $params = array(0, $options['uName'], $pass, $pid, $nxsToWPContent, true);
