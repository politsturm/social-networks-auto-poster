...
if (!class_exists("nxs_class_SNAP_VK")) {
    class nxs_class_SNAP_VK
    {
        ...
        public function nxs_uplImgtoVK($imgURL, $options)
        {
        ...
        public function doPostToNT($options, $message)
        {
            ...
            $response = nxs_remote_post($postUrl, $advSet); // prr($advSet); prr($response);
            ...
            if (isset($ret) && $ret!='') {
                return array('postID'=>$ret, 'isPosted'=>1, 'postURL'=>'http://vk.com/wall'.$ret, 'pDate'=>date('Y-m-d H:i:s'), 'msg'=>$badOut['Error']);
            }
            return $badOut;
        }
    }
