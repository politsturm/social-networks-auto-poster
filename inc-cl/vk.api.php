...
if (!class_exists("nxs_class_SNAP_VK")) {
    class nxs_class_SNAP_VK
    {
        ...
        function nxs_uplImgtoVK($imgURL, $options)
        {
            ...
            $postUrl = 'https://api.vk.com/method/photos.getWallUploadServer?v=5.71&gid=' . $pgIntId . '&access_token=' . $options['appAuthToken'];
            $response = nxs_remote_get($postUrl);
            if (is_nxs_error($response))
                return "Error: URL:" . $postUrl . " | " . print_r($response, true);
            ...
            if (!empty($VKuploadUrl)) {
                ...
                $imgData           = nxs_remote_get($remImgURL);
                if (is_nxs_error($imgData) || empty($imgData['body']) || (!empty($imgData['headers']['content-length']) && (int) $imgData['headers']['content-length'] < 200) || $imgData['headers']['content-type'] == 'text/html' || $imgData['response']['code'] == '403')
                    return 'Could not get image ( ' . $remImgURL . ' ), will post without it - ' . print_r($imgData, true);
                else
                    $imgData = $imgData['body'];
                $tmp = array_search('uri', @array_flip(stream_get_meta_data($GLOBALS[mt_rand()] = tmpfile())));
                if (!is_writable($tmp)) {
                    $msg = "Can't upload image to VK. Your temporary folder or file (file - " . $tmp . ") is not writable.";
                    if (function_exists('wp_upload_dir')) {
                        $uDir = wp_upload_dir();
                        $tmp  = tempnam($uDir['path'], "nx");
                        if (!is_writable($tmp))
                            return $msg . " Your UPLOADS folder or file (file - " . $tmp . ") is not writable. ";
                    } else
                        return $msg;
                }
                rename($tmp, $tmp .= '.png');
                ...
                if (!empty($uploadResultObj->server) && !empty($uploadResultObj->photo) && !empty($uploadResultObj->hash)) {
                    ...
                    $response = nxs_remote_get($postUrl);
                    if (is_nxs_error($response))
                        return "Error: URL:" . $postUrl . " | " . print_r($response, true); //   prr($response);
        ...
        function doPostToNT($options, $message)
        {
            ...
            if (empty($options['imgSize']))
                $options['imgSize'] = '';
            if (!isset($options['appAuthToken']) || trim($options['appAuthToken']) == '') {
            ...
            if (isset($message['imageURL']))
                ...
                $imgURL = '';
            $msgOpts        = array();
            $msgOpts['uid'] = $options['pgID']; // if ($link!='') $msgOpts['link'] = $link;
            ...
            if ($postType == 'I') {
                ...
                if (is_object($imgUpld)) {
                    ...
                    $badOut['Error'] .= '-=IMG ERROR=- ' . print_r($imgUpld, true);
            }
