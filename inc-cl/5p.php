...
if (!class_exists("nxs_snapClass5P")) {
    ...
    {
        public $ntInfo = array('code'=>'5P', 'lcode'=>'5p', 'name'=>'500Px', 'defNName'=>'', 'tstReq' => true, 'instrURL'=>'https://www.nextscripts.com/instructions/setup-installation-500px-social-networks-auto-poster/');
        ...
        {
            $defO = array('nName'=>'', 'do'=>'1', 'appKey'=>'', 'appSec'=>'', 'inclTags'=>'1', 'cat'=>0, 'gal'=>'', 'msgTFormat'=>"%TITLE%", 'msgFormat'=>"%EXCERPT%\r\n\r\n%URL%", 'imgSize'=>'original');
        ...
        public function doAuth()
        {
            ...
            if (isset($_GET['auth']) && $_GET['auth']==$ntInfo['lcode']) {
                ...
                $tum_oauth->request_token_path = '/v1/oauth/request_token';
        ...
        default: echo '<br/><b style="color:red">Could not connect to 500Px. Refresh the page or try again later.</b>'; die();
            ...
            if (isset($_GET['auth']) && $_GET['auth']==$ntInfo['lcode'].'a') {
                ...
                $tum_oauth->baseURL = 'https://api.500px.com';
                $tum_oauth->access_token_path = '/v1/oauth/access_token';
                ...
                $uinfo = $tum_oauth->makeReq('https://api.500px.com/v1/users', '');
                prr($uinfo);
                if (is_array($uinfo) && isset($uinfo['user']) && is_array($uinfo['user'])) {
                    $uinfo = $uinfo['user'];
                    $options['appAppUserName'] = $uinfo['username']."(".$uinfo['fullname'].")";
        ...
        public function accTab($ii, $options, $isNew=false)
        {
            ...
            $ntInfo = $this->ntInfo;
            $nt = $ntInfo['lcode'];
            $this->elemKeySecret($ii, '500Px Consumer Key', '500Px Consumer Secret', $options['appKey'], $options['appSec'], 'appKey', 'appSec', 'https://500px.com/settings/applications'); ?>
    <br/><?php $this->elemTitleFormat($ii, 'Image Title Format', 'msgTFormat', $options['msgTFormat']);
            $this->elemMsgFormat($ii, 'Image Description Format', 'msgFormat', $options['msgFormat']); ?>
    ...
    <select name="5p[<?php echo $ii; ?>][cat]"><option value="error" selected="selected" disabled="">Select default Category</option>
            <?php $suCats = $this->pCats();
            if (isset($options['cat']) && $options['cat']!='') {
                $suCats = str_replace('"'.$options['cat'].'"', '"'.$options['cat'].'" selected="selected"', $suCats);
            }
            echo $suCats; ?>
          </select>
    </div>
    ...
    <?php  if ($options['appKey']=='') {
      ...
      <?php if (!isset($options['appAppUserID']) || $options['appAppUserID']<1) {
        ...
        public function setNTSettings($post, $options)
        {
            foreach ($post as $ii => $pval) {
                if (!empty($pval['appKey']) && !empty($pval['appKey'])) {
                    ...
                    if (isset($pval['cat'])) {
                        $options[$ii]['cat'] = $pval['cat'];
                    } else {
                        $options[$ii]['cat'] = 0;
                    }
                    if (isset($pval['gal'])) {
                        $options[$ii]['gal'] = trim($pval['gal']);
                    } else {
                        $options[$ii]['gal'] = '';
                    }
                } elseif (count($pval)==1) {
        ...
        public function showEdPostNTSettings($ntOpts, $post)
        {
            ...
            foreach ($ntOpts as $ii=>$ntOpt) {
                ...
                $this->elemEdMsgFormat($ii, __('Description Format:', 'social-networks-auto-poster-facebook-twitter-g'), $msgFormat); ?> 
        <?php /* Gallery ?>
     ...
     <tr class="nxstbldo <?php echo 'nxstbldo'.strtoupper($nt).$ii; ?>"><th scope="row" style="text-align:right; width:150px; vertical-align:top; padding-top: 0px; padding-right:10px;"> <?php _e('Category', 'social-networks-auto-poster-facebook-twitter-g'); ?> <br/></th><td>     
        <select name="5p[<?php echo $ii; ?>][cat]"><option value="error" selected="selected" disabled=""><?php _e('Select Category', 'social-networks-auto-poster-facebook-twitter-g'); ?></option>
            <?php $suCats = $this->pCats();
                if (isset($ntOpt['cat']) && $ntOpt['cat']!='') {
                    $suCats = str_replace('"'.$ntOpt['cat'].'"', '"'.$ntOpt['cat'].'" selected="selected"', $suCats);
                }
                echo $suCats; ?>
          </select>
     </td></tr>
...
if (!function_exists("nxs_doPublishTo5P")) {
    function nxs_doPublishTo5P($postID, $options)
    {
        if (!is_array($options)) {
            $options = maybe_unserialize(get_post_meta($postID, $options, true));
        }
        $cl = new nxs_snapClass5P();
