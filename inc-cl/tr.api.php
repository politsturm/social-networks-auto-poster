...
if (!class_exists("nxs_class_SNAP_TR")) {
    class nxs_class_SNAP_TR
    {
        ...
        public function doPostToNT($options, $message)
        {
            ...
            if (empty($options['appKey']) || empty($options['appSec']) || empty($options['accessToken'])) {
            ...
            if (empty($options['pgID'])) {
                ...
                if (substr($options['trURL'], -1)=='/') {
                    $options['pgID'] = substr($options['trURL'], 0, -1);
                }
            }
            ...
            if ($options['postType']=='I') {
                $postArr['type'] = 'photo';
                $postArr['caption'] = $msg;
                $postArr['source'] = str_ireplace(' ', '%20', $imgURL);
            ...
            if ($code == 201) {
                return array('postID'=>$postinfo->response->id, 'isPosted'=>1, 'postURL'=>'http://'.$options['pgID']."/post/".$postinfo->response->id, 'pDate'=>date('Y-m-d H:i:s'));
            } else {
