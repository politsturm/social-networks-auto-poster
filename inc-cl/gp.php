...
if (!function_exists("nxs_CptCheckGP")) {
...
if (!class_exists("nxs_snapClassGP")) {
    class nxs_snapClassGP extends nxs_snapClassNT
    {
        public $ntInfo = array('code'=>'GP', 'lcode'=>'gp', 'name'=>'Google+', 'defNName'=>'uName', 'tstReq' => false, 'imgAct'=>'E', 'instrURL'=>'https://www.nextscripts.com/instructions/setup-installation-google-plus-social-networks-auto-poster/');
        public $noFuncMsg = 'Sorry, but Google+ doesn\'t have a built-in API for automated posts yet. <br/>You need a special API library module to be able to publish your content to Google+.';
      ...
      case 340: $ntOptsOut = $this->toLatestVerNTGen($ntOpts); $ntOptsOut['do'] = $ntOpts['do'.$this->ntInfo['code']]; $ntOptsOut['nName'] = $ntOpts['nName'];
        ...
        // no break
      case 350:
        if (empty($ntOptsOut['postTo'])) {
            $ntOptsOut['postTo'] = !empty($ntOptsOut['commID']) ? 'c'.$ntOptsOut['commID'] : 'p';
        }
        if (empty($ntOptsOut['postAs'])) {
            $ntOptsOut['postAs'] = !empty($ntOptsOut['pageID']) ? $ntOptsOut['pageID'] : 'p';
        }
      ...
      break;
        ...
        {
            $defO = array('nName'=>'', 'do'=>'1', 'uName'=>'', 'uPass'=>'', 'postType'=>'A', 'postTo'=>'', 'postAs'=>'', 'postAsNm'=>'', 'commCat'=>'', 'msgFormat'=>"New post: %TITLE% - %URL%");
        ...
        public function accTab($ii, $options, $isNew=false)
        {
            ...
            if (empty($options['ssid'])) {
                $options['ssid']='';
            }
            if (empty($options['nid'])) {
                $options['nid']='';
            }
            if (empty($options['hsid'])) {
                $options['hsid']='';
            }// $options['v']='340'; $options = $this->toLatestVer($options);
            $ntInfo = $this->ntInfo;
            $nt = $ntInfo['lcode']; ?>
    
    <div id="ups<?php echo $nt.$ii; ?>UP" class="ups<?php echo $nt.$ii; ?>">
      <?php $p = $options['uPass'];
            $this->elemUserPass($ii, $options['uName'], $p); ?>
    </div>
    ...
    if (!empty($p)) {
        $p = (substr($p, 0, 5)=='n5g9a'||substr($p, 0, 5)=='g9c1a'||substr($p, 0, 5)=='b4d7s')?nsx_doDecode(substr($p, 5)):$p;
        $options['uPass'] = 'g9c1a'.nsx_doEncode($p);
        $tPST = (!empty($_POST))?$_POST:'';
        $_POST['pg'] = $options['postTo'];
        $_POST['pstAs'] = $options['postAs'];
        $_POST['u'] = $options['uName'];
        $_POST['p'] = $p;
        $_POST['ii'] = $ii;
        $ntw[$nt][$ii]=$options;
        $opNm = 'nxs_snap_gp_'.sha1('nxs_snap_gp'.$options['uName'].$options['uPass']);
        ...
        if (!empty($opVal) & !is_array($opVal)) {
            ...
            $opNm = 'nxs_snap_gp_wh'.sha1('nxs_snap_gpwh'.$options['uName'].$options['uPass'].$options['postAs']);
            $opVal = nxs_getOption($opNm);
            if (empty($opVal)) {
                $opVal = $this->getListWhereToPost($ntw);
            }
            if (!empty($opVal) & is_array($opVal)) {
                $options = array_merge($options, $opVal);
            } // prr($opVal, 'opVal:');
            if (substr($options['postTo'], 0, 1)=='c') {
                $opNm = 'nxs_snap_gp_cl_'.sha1('nxs_snap_gpcl'.$options['uName'].$options['uPass'].$options['postTo']);
                $opVal = nxs_getOption($opNm);
                if (empty($opVal)) {
                    $opVal = $this->getGPCommInfo($ntw);
                }
                if (!empty($opVal) & is_array($opVal)) {
                    $options = array_merge($options, $opVal);
                } // prr($opVal, 'opVal:');
        ...
        $_POST = $tPST;
            ...
            if (empty($options['postAsNm']) && !empty($options['postAs']) && $options['postAs']!='p') {
                $options['postAsNm'] = 'Page. ID:'.$options['postAs'].')';
            } ?>
    ...
    <div id="nxsGPInfoDiv<?php echo $ii; ?>">
         <div style="width:100%;">
          ...
          <select id="gpPostAs<?php echo $ii; ?>" onchange="nxs_gpPostAsChange('<?php echo $ii; ?>',jQuery(this));" name="gp[<?php echo $ii; ?>][postAs]">
            ...
            if (!empty($options['postAs'])) {
                echo (!empty($pgi) && !empty($options['postAs']) && stripos($pgi, $options['postAs'])===false)?'<option selected="selected" value="'.$options['postAs'].'">'.$options['postAs'].'</option>':'';
            } ?><option <?php if (empty($options['postAs'])) {
                echo 'selected="selected"';
            } ?> value="p"><?php _e('Profile'); ?></option><?php
              if (!empty($options['postAs'])) {
                  $pgi = str_ireplace('selected="selected" ', '', $pgi);
                  $pgi = str_ireplace('value="'.$options['postAs'].'"', 'selected="selected" value="'.$options['postAs'].'"', $pgi);
              }
    ...
    <div id="nxsGPInfoDiv<?php echo $ii; ?>">
         <div style="width:100%;">
          ...
          <select id="gpWhToPost<?php echo $ii; ?>" onchange="nxs_gpWhToPostChange('<?php echo $ii; ?>',jQuery(this));" name="gp[<?php echo $ii; ?>][postTo]">
            ...
            if (!empty($options['postTo'])) {
                echo (empty($pgi) || (!empty($pgi) && stripos($pgi, $options['postTo']))===false)?'<option selected="selected" value="'.$options['postTo'].'">'.$options['postTo'].'</option>':'';
            } ?><option <?php if (empty($options['postTo']) || $options['postTo']=='p') {
                echo 'selected="selected"';
            } ?> value="p"><?php echo (!empty($options['postAsNm'])&& $options['postAs']!='p')?$options['postAsNm']:__('Profile'); ?></option><?php
              if (!empty($options['postTo'])) {
                  $pgi = str_ireplace('selected="selected" ', '', $pgi);
                  $pgi = str_ireplace('value="'.$options['postTo'].'"', 'selected="selected" value="'.$options['postTo'].'"', $pgi);
              }
            echo $pgi; ?><option value="a"><?php _e('.... Enter the Collection or Community ID'); ?></option>
          ...
          <div id="nxsGPMsgDiv<?php echo $ii; ?>"><?php if (!empty($options['uMsg'])) {
                echo $options['uMsg'];
            } ?><?php if ($isNew) {
                ?><?php _e('Please enter your login/password to see the list of your pages, collections, and communities', 'nxs_snap'); ?><?php
            } ?></div>  
    </div> 
    ...
    <div id="nxsGPInfoDivComm<?php echo $ii; ?>" style="padding-left:15px; display: <?php echo (!empty($options['postTo']) && substr($options['postTo'], 0, 1)=='c')?'block':'none'; ?>;"><b><?php _e('Community Category:', 'nxs_snap'); ?></b>
      <div style="padding-left:15px;">      
          <select id="gpCommCat<?php echo $ii; ?>" name="gp[<?php echo $ii; ?>][commCat]">
            ...
            if (!empty($options['commCat'])) {
                $pgi = str_ireplace('selected="selected" ', '', $pgi);
                $pgi = str_ireplace('value="'.$options['commCat'].'"', 'selected="selected" value="'.$options['commCat'].'"', $pgi);
            }
            echo $pgi; ?></select>        
    </div></div>
...
<div style="margin-left: 10px;">        
        <input type="radio" name="gp[<?php echo $ii; ?>][postType]" value="T" <?php if ($options['postType'] == 'T') {
                echo 'checked="checked"';
            } ?> /> <?php _e('Text Post', 'nxs_snap'); ?> - <i><?php _e('just text message', 'nxs_snap'); ?></i><br/>                    
        <input type="radio" name="gp[<?php echo $ii; ?>][postType]" value="I" <?php if ($options['postType'] == 'I') {
                echo 'checked="checked"';
            } ?> /> <?php _e('Google+ Image Post', 'nxs_snap'); ?> - <i><?php _e('big image with text message', 'nxs_snap'); ?></i><br/>
        <input type="radio" name="gp[<?php echo $ii; ?>][postType]" value="A" <?php if (!isset($options['postType']) || $options['postType'] == '' || $options['postType'] == 'A') {
                echo 'checked="checked"';
            } ?> /> <?php _e('Add blogpost to Google+ message as an attachment', 'nxs_snap'); ?><br/>
<div class="popShAtt" id="popShAtt<?php echo $ii; ?>XG"><h3><?php _e('Google+ Post Types', 'nxs_snap'); ?></h3><img src="<?php echo NXS_PLURL; ?>img/gpPostTypesDiff6.png" width="600" height="285" alt="<?php _e('Google+ Post Types', 'nxs_snap'); ?>"/></div>
   ...
   <input value="1"  id="apFBMsgAFrmtA<?php echo $ii; ?>" <?php if (!empty($options['riComments']) && trim($options['riComments'])=='1') {
                    echo "checked";
                } ?> type="checkbox" name="<?php echo  $this->ntInfo['lcode']; ?>[<?php echo $ii; ?>][riComments]"/> <strong><?php _e('Import Facebook Comments', 'social-networks-auto-poster-facebook-twitter-g'); ?></strong>
   <br/>
   
   <div style="margin-bottom: 5px; margin-left: 10px; ">
     <input value="1"  id="apFBMsgAFrmtA<?php echo $ii; ?>" <?php if (!empty($options['riCommentsAA']) && trim($options['riCommentsAA'])=='1') {
                    echo "checked";
                } ?> type="checkbox" name="<?php echo  $this->ntInfo['lcode']; ?>[<?php echo $ii; ?>][riCommentsAA]"/> <strong><?php _e('Auto-approve imported comments', 'social-networks-auto-poster-facebook-twitter-g'); ?></strong></div>   
     <?php
            } else {
                echo "<br/>";
        ...
        public function setNTSettings($post, $options)
        {
            foreach ($post as $ii => $pval) {
                if (!empty($pval['uPass']) && !empty($pval['uPass'])) {
                    ...
                    if (isset($pval['sid'])) {
                        $options[$ii]['sid'] = trim($pval['sid']);
                    }
                    if (isset($pval['ssid'])) {
                        $options[$ii]['ssid'] = trim($pval['ssid']);
                    }
                    if (isset($pval['hsid'])) {
                        $options[$ii]['hsid'] = trim($pval['hsid']);
                    }
                    if (isset($pval['nid'])) {
                        $options[$ii]['nid'] = trim($pval['nid']);
                    }
                    if (isset($pval['apiToUse2'])) {
                        $options[$ii]['apiToUse2'] = trim($pval['apiToUse2']);
                    }
        
                    if (isset($pval['postTo'])) {
                        $options[$ii]['postTo'] = trim($pval['postTo']);
                    }
                    if (isset($pval['postAs'])) {
                        $options[$ii]['postAs'] = trim($pval['postAs']);
                    }
                    if (isset($pval['postAsNm'])) {
                        $options[$ii]['postAsNm'] = trim($pval['postAsNm']);
                    }
                    if (isset($pval['commCat'])) {
                        $options[$ii]['commCat'] = trim($pval['commCat']);
                    }
                    if (isset($pval['postType'])) {
                        $options[$ii]['postType'] = trim($pval['postType']);
                    } else {
                        $options[$ii]['postType'] = 'A';
                    }
        ...
        <input type="radio" name="<?php echo $nt; ?>[<?php echo $ii; ?>][postType]" value="I" <?php if ($postType == 'I') {
                    echo 'checked="checked"';
                } ?> /> <?php _e('Image Post', 'social-networks-auto-poster-facebook-twitter-g') ?>  - <i><?php _e('Text with Image', 'social-networks-auto-poster-facebook-twitter-g') ?></i><br/>
        <input type="radio" name="<?php echo $nt; ?>[<?php echo $ii; ?>][postType]" value="A" <?php if (empty($postType) || $postType == 'A') {
                    echo 'checked="checked"';
                } ?> /><?php _e('Text Post with "attached" blogpost', 'social-networks-auto-poster-facebook-twitter-g') ?>
        </td></tr>
          <?php
          /* ## Select Image & URL ## */ nxs_showImgToUseDlg($nt, $ii, $imgToUse);
   ...
   <div class="nxsPostEd_ElemWrap">   
     ...
     <div class="nxsPostEd_Elem">   
        <input type="radio" name="<?php echo $nt; ?>[<?php echo $ii; ?>][postType]" value="T" class="nxsEdElem nxsImgCtrlCb" data-nt="<?php echo $nt; ?>" data-ii="<?php echo $ii; ?>" <?php if ($postType == 'T') {
                echo 'checked="checked"';
            } ?> /> <?php _e('Text Post', 'social-networks-auto-poster-facebook-twitter-g') ?>  - <i><?php _e('just text message', 'social-networks-auto-poster-facebook-twitter-g') ?></i><br/>
        <input type="radio" name="<?php echo $nt; ?>[<?php echo $ii; ?>][postType]" value="I" class="nxsEdElem nxsImgCtrlCb" data-nt="<?php echo $nt; ?>" data-ii="<?php echo $ii; ?>" <?php if ($postType == 'I') {
                echo 'checked="checked"';
            } ?> /> <?php _e('Image Post', 'social-networks-auto-poster-facebook-twitter-g') ?>  - <i><?php _e('Text with Image', 'social-networks-auto-poster-facebook-twitter-g') ?></i><br/>
        <input type="radio" name="<?php echo $nt; ?>[<?php echo $ii; ?>][postType]" value="A" class="nxsEdElem nxsImgCtrlCb" data-nt="<?php echo $nt; ?>" data-ii="<?php echo $ii; ?>" <?php if (empty($postType) || $postType == 'A') {
                echo 'checked="checked"';
            } ?> /><?php _e('Text Post with "attached" blogpost', 'social-networks-auto-poster-facebook-twitter-g') ?>
     </div>
   </div><?php
        ...
        public function importComments($options='', $postID='', $po='', $fbID='')
        {
            ...
            if (empty($postID)) {
                $postID = $_POST['pid'];
            } //die('Yo'); // $fbID = '1019404527_10212971242848671';
            if (empty($options)) {
                global $nxs_SNAP;
            ...
            $nt->debug = false;
            $loginError = $nt->connect($email, $pass); //  die('STOP IT');
            if (!$loginError) {
  ...
  public function getListOfPages($networks)
  {
      ...
      $opNm = 'nxs_snap_gp_'.sha1('nxs_snap_gp'.$_POST['u'].$pass);
      $opVal = nxs_getOption($opNm);
      $ii = $_POST['ii'];
      $nt = new nxsAPI_GP(); // prr($opVal);
     if (!empty($networks['gp'][$ii]['proxy'])&&!empty($networks['gp'][$ii]['proxyOn'])) {
         $nt->proxy['proxy'] = $networks['gp'][$ii]['proxy']['proxy'];
         if (!empty($networks['gp'][$ii]['proxy']['up'])) {
             $nt->proxy['up'] = $networks['gp'][$ii]['proxy']['up'];
         }
     }
      $currPstAs = !empty($_POST['pstAs'])?$_POST['pstAs']:(!empty($networks['gp'][$ii])?$networks['gp'][$ii]['postAs']:'');
      if (!empty($networks['gp'][$ii]['sid'])) {
          $nt->session = array('ssid'=>$networks['gp'][$ii]['ssid'], 'sid'=>$networks['gp'][$ii]['sid'], 'hsid'=>$networks['gp'][$ii]['hsid']);
      }
      if (empty($_POST['force']) && !empty($opVal['ck']) && !empty($opVal['pgsList'])) {
          ...
          if (!$loginError) {
              ...
              $outMsg = '<b style="color:red;">'.__('Login Problem').'&nbsp;-&nbsp;'.$loginError.'</b>';
      ...
      if (!empty($_POST['isOut'])) {
          echo $pgCust.'<option '.($currPstAs=='p'?'selected="selected" ':'').'value="p">'.__('Profile').'</option>'.$pgs.'<option style="color:#BD5200" value="a">'.__('...enter the Page ID').'</option>';
      }
      $opVal['pgsList'] = $pgs;
      nxs_saveOption($opNm, $opVal);
      return $opVal;
  }
        public function getListWhereToPost($networks)
        {
            $opVal = array();
            $pass = 'g9c1a'.nsx_doEncode($_POST['p']);
            $ii = $_POST['ii'];
            ...
            $currPstAs = !empty($_POST['pstAs'])?$_POST['pstAs']:(!empty($networks['gp'][$ii])?$networks['gp'][$ii]['postAs']:'');
            $currPg = !empty($_POST['pg'])?$_POST['pg']:(!empty($networks['gp'][$ii])?$networks['gp'][$ii]['postTo']:'');
            $opNm = 'nxs_snap_gp_'.sha1('nxs_snap_gp'.$_POST['u'].$pass);
            $opVal = nxs_getOption($opNm);
            $opNm2 = 'nxs_snap_gp_wh'.sha1('nxs_snap_gpwh'.$_POST['u'].$pass.$currPstAs);
            $opVal2 = nxs_getOption($opNm2);
            if (!empty($networks['gp'][$ii]['sid'])) {
                $nt->session = array('ssid'=>$networks['gp'][$ii]['ssid'], 'sid'=>$networks['gp'][$ii]['sid'], 'hsid'=>$networks['gp'][$ii]['hsid']);
            }
            if (empty($_POST['force']) && !empty($opVal['ck']) && !empty($opVal2['whToList'])) {
                $pgs = $opVal2['whToList'];
            } else {
                if (!empty($opVal['ck'])) {
                    $nt->ck = $opVal['ck'];
                ...
                if (!$loginError) {
                    $opVal['ck'] = $nt->ck;
                    $pgs = $nt->getWhereToPostList($currPg, $currPstAs);
                } else {
                    $outMsg = '<b style="color:red;">'.__('Login Problem').'&nbsp;-&nbsp;'.$loginError.'</b>';
                    if (!empty($_POST['isOut'])) {
                        echo $outMsg;
            ...
            if (!empty($_POST['isOut'])) {
                echo $pgCust.'<option '.($currPg=='p'?'selected="selected" ':'').'value="p">'.(!empty($_POST['pstAsNm'])?$_POST['pstAsNm']:__('Profile')).'</option>'.$pgs.'<option style="color:#BD5200" value="a">'.__('...enter the Collection or Community ID').'</option>';
            }
            $opVal2['whToList'] = $pgs;
            nxs_saveOption($opNm2, $opVal2);
            return $opVal2;
        }
        public function getGPCommInfo($networks)
        {
            $ii = $_POST['ii'];
            $currPgID = !empty($_POST['pg'])?$_POST['pg']:(!empty($networks['gp'][$ii])?$networks['gp'][$ii]['postTo']:'');
            $opVal = array();
            $pass = 'g9c1a'.nsx_doEncode($_POST['p']);
            $opNm = 'nxs_snap_gp_cl_'.sha1('nxs_snap_gpcl'.$_POST['u'].$pass.$currPgID);
            $opVal = nxs_getOption($opNm);
            ...
            if (empty($_POST['force']) && !empty($opVal) && !empty($opVal['commCatsList'])) {
                if (!empty($currCat)) {
                    $opVal['commCatsList'] = str_ireplace('selected="selected" ', '', $opVal['commCatsList']);
                    $opVal['commCatsList'] = str_ireplace('value="'.$currCat.'"', 'selected="selected" value="'.$currCat.'"', $opVal['commCatsList']);
                }
                if (!empty($_POST['isOut'])) {
                    echo $opVal['commCatsList'];
                }
            } else {
                $nt = new nxsAPI_GP();
                if (!empty($networks['gp'][$ii]['proxy'])&&!empty($networks['gp'][$ii]['proxyOn'])) {
                ...
                $ii = $_POST['ii'];
                $loginError = $nt->connect(!empty($_POST['u'])?$_POST['u']:$networks['gp'][$ii]['uName'], !empty($_POST['p'])?$_POST['p']:$networks['gp'][$ii]['uPass']);
                if (!$loginError) {  //!empty($networks['gp'][$ii])?$networks['gp'][$ii]['pageID']:'';  echo "DX ".$currPgID;
                    if (!empty($currPgID) && substr($currPgID, 0, 1)=='c') {
                        $currPgID = substr($currPgID, 1);
                        $opVal['commCatsList'] = $nt->getCCatsGP($currPgID, $currCat);
                        nxs_saveOption($opNm, $opVal);
                        if (!empty($_POST['isOut'])) {
                            echo $opVal['commCatsList'];
                        }
...
if (!function_exists("nxs_doPublishToGP")) {
