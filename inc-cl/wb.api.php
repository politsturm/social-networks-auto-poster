...
if (!class_exists("nxs_class_SNAP_WB")) {
    class nxs_class_SNAP_WB
    {
        public $ntCode = 'WB';
        public $ntLCode = 'wb';
        ...
        public function wbCut($msg)
        {
            $reg_exUrl = "/(http|https|ftp|ftps)\:\/\/[a-zA-Z0-9\-\.]+\.[a-zA-Z]{2,3}(\/\S*)?/";
            if (preg_match($reg_exUrl, $msg, $url)) {
                $u = $url[0];
                $msg =  preg_replace($reg_exUrl, "^HRF^^LBW^", $msg);
                $msg = nsTrnc($msg, 140);
                $msg = str_ireplace("^HRF^^LBW^", $u, $msg);
            } else {
                $msg = nsTrnc($msg, 140);
            }
            return $msg;
      ...
      $msg = str_replace('&amp;#039;', "'", $msg);
            ...
            if ($options['attchImg']=='1') {
                ...
                $rep = nxs_curlUploadImg($imgURL, $url, $data, 'pic');
            } else {
                $hdrsArr = array();
                $advSet = nxs_mkRemOptsArr($hdrsArr, '', $data);
                $rep = nxs_remote_post($url, $advSet);
            }
            $cont = json_decode($rep['body'], true);
            if (empty($cont) || empty($cont['created_at'])) {
                $badOut['Error'] .= print_r($rep, true);
                return $badOut;
            }
