...
if (!class_exists("nxs_snapClassYO")) {
    class nxs_snapClassYO extends nxs_snapClassNT
    {
        public $ntInfo = array('code'=>'YO', 'lcode'=>'yo', 'name'=>'Yo', 'defNName'=>'', 'tstReq' => false, 'instrURL'=>'https://www.nextscripts.com/instructions/yo-auto-poster-setup-installation/');
        ...
        {
            $defO = array('nName'=>'', 'do'=>'1', 'apiKey'=>'',  'msgFormat'=>'%TITLE%');
...
if (!function_exists("nxs_doPublishToYO")) {
    function nxs_doPublishToYO($postID, $options)
    {
        if (!is_array($options)) {
            $options = maybe_unserialize(get_post_meta($postID, $options, true));
        }
        $cl = new nxs_snapClassYO();
