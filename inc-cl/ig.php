...
if (!class_exists("nxs_snapClassIG")) {
    class nxs_snapClassIG extends nxs_snapClassNT
    {
        public $ntInfo = array('code'=>'IG', 'lcode'=>'ig', 'name'=>'Instagram', 'defNName'=>'uName', 'tstReq' => false, 'imgAct'=>'E', 'instrURL'=>'https://www.nextscripts.com/instructions/instagram-auto-poster-setup-installation/');
        public $noFuncMsg = 'Sorry, but Instagram doesn\'t have a built-in API for automated posts yet. <br/>You need a special API library module to be able to publish your content to Instagram.';
        public $noFuncMsg2 = 'Instagram API Library module NOT found.<br/><br/><span style="color:black; font-size:15px;">It looks like you got a SNAP Pro plugin with API package using "One time fee" promo offer more then a year ago. <br/>We are sorry, but Instagram was not a part of that package. <br/>As part of the offer agreement, you will be getting indefinite support and updates for Google+, Pinterest, LinkedIn, Reddit, Flipboard and all other networks what were included with your order, but Instagram and all other future networks requre an active API subscription. <a target="_blank" href="http://gd.is/expi">Please see here for more info</a></span><hr/>';
        public function checkIfFunc()
        {
            return class_exists('nxsAPI_IG');
        }
        ...
        {
            $defO = array('nName'=>'', 'do'=>'1', 'uName'=>'', 'uPass'=>'', 'msgTFormat'=>'', 'defImg'=>'', 'msgFormat'=>"%TITLE% \n %HTAGS%");
    ...
    <input type="radio" name="<?php echo $nt; ?>[<?php echo $ii; ?>][imgAct]" value="E" <?php if (empty($options['imgAct']) || $options['imgAct'] == 'E') {
                echo 'checked="checked"';
            } ?> /> <?php _e('Make it Square: Extend', 'social-networks-auto-poster-facebook-twitter-g'); ?> - <i><?php _e('Image will be extended to square', 'social-networks-auto-poster-facebook-twitter-g'); ?></i><br/>
    <input type="radio" name="<?php echo $nt; ?>[<?php echo $ii; ?>][imgAct]" value="C" <?php if (!empty($options['imgAct']) && $options['imgAct'] == 'C') {
                echo 'checked="checked"';
            } ?> /> <?php _e('Make it Square: Crop', 'social-networks-auto-poster-facebook-twitter-g'); ?> - <i><?php _e('Image will be cropped to square', 'social-networks-auto-poster-facebook-twitter-g'); ?></i><br/>                    
    <input type="radio" name="<?php echo $nt; ?>[<?php echo $ii; ?>][imgAct]" value="D" <?php if (!empty($options['imgAct']) && $options['imgAct'] == 'D') {
                echo 'checked="checked"';
            } ?> /> <?php _e("Don't change", 'social-networks-auto-poster-facebook-twitter-g'); ?> - <i><?php _e('Image will be untouched. (Could cause "Uploaded image isn\'t in an allowed aspect ratio" Error)', 'social-networks-auto-poster-facebook-twitter-g'); ?></i><br/>
    <br/></div>    
    ...
    <?php
        ...
        public function setNTSettings($post, $options)
        {
            foreach ($post as $ii => $pval) {
                if (!empty($pval['uPass']) && !empty($pval['uPass'])) {
                    ...
                    if (isset($pval['imgAct'])) {
                        $options[$ii]['imgAct'] = trim($pval['imgAct']);
                    } else {
                        $options[$ii]['imgAct'] = 'E';
                    }
                } elseif (count($pval)==1) {
        ...
        public function adjAfterPost(&$options, &$ret)
        {
            if (!empty($ret['isPosted']) && $ret['isPosted']=='1') {
                nxs_save_glbNtwrks('ig', $options['ii'], '', 'session');
            }
        }
        public function nxsCptCheck()
        {
            $api = new nxs_class_SNAP_IG();
            $api->nxsCptCheck();
        }
    }
}
...
if (!function_exists("nxs_doPublishToIG")) {
    function nxs_doPublishToIG($postID, $options)
    {
        if (!is_array($options)) {
            $options = maybe_unserialize(get_post_meta($postID, $options, true));
        }
        ini_set('memory_limit', '256M');
        $cl = new nxs_snapClassIG();
