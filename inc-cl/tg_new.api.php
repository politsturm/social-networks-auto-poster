...
if (!class_exists("nxs_class_SNAP_TG")) {
    class nxs_class_SNAP_TG
    {
        public $ntCode = 'TG';
        public $ntLCode = 'tg';
        ...
        'Error'=>''
            ...
            if (substr($options['whToPost'], 0, 1)=='g') {
                $options['whToPost'] = substr_replace($options['whToPost'], '-', 0, 1);
            } //## Fix for Groups ID
          ...
          str_ireplace('</em>', '</i>', $msg)
            ...
            $pos = strpos($msg, $bold);
            if ($pos !== false) {
                $newMsg = substr_replace($msg, '<b>', $pos, strlen($bold));
                $pos = strpos($newMsg, $bold, $pos + strlen($bold));
                if ($pos !== false) {
                    $newMsg = substr_replace($newMsg, '</b>', $pos, strlen($bold));
                    $msg = $newMsg;
                }
            }
            ...
            $flds = array(
          'chat_id' => $options['whToPost'],
          'text' => $msg,
          'parse_mode' => 'HTML',
          'disable_web_page_preview' => $options['webPrev']=='0'
      );
            ...
            if (is_array($resp) && !empty($resp['ok']) && $resp['ok'] == 1) {
                if (empty($resp['result']['chat']['username'])) {
                    $purl = 'https://web.telegram.org/#/im?p='.str_replace('-', 'g', $resp['result']['chat']['id']);
                } else {
                    $purl = 'http://telegram.me/'.$resp['result']['chat']['username'];
                }

                return array(
                  'postID'=>$resp['result']['message_id'],
                  'isPosted'=>1,
                  'postURL'=>$purl,
                  'pDate'=>date('Y-m-d H:i:s')
          );
