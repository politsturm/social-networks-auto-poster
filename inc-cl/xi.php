...
if (!class_exists("nxs_snapClassXI")) {
    class nxs_snapClassXI extends nxs_snapClassNT
    {
        public $ntInfo = array('code'=>'XI', 'lcode'=>'xi', 'name'=>'XING', 'defNName'=>'', 'tstReq' => true, 'instrURL'=>'http://www.nextscripts.com/instructions/setup-installation-xing-social-networks-auto-poster/');
      ...
      break;
        ...
        {
            $defO = array('nName'=>'', 'do'=>'1', 'appKey'=>'', 'appSec'=>'', 'uName'=>'', 'uPass'=>'', 'inclTags'=>'1', 'postType'=>'A', 'postTypeP'=>'A', 'postTypeG'=>'I', 'msgFormat'=>"%EXCERPT%\r\n\r\n%URL%");
        ...
        {
            return true;
            return ((!empty($options['appAppUserID']) && !empty($options['accessToken'])) || !empty($options['uName']));
        }
        public function doAuth()
        {
            ...
            if (isset($_GET['auth']) && $_GET['auth']==$ntInfo['lcode']) {
                ...
                $tum_oauth = new nxs_OAuthBaseCl($consumer_key, $consumer_secret);
                $tum_oauth->baseURL = 'https://api.xing.com';
                $tum_oauth->request_token_path = '/v1/request_token';
        ...
        default: echo '<br/><b style="color:red">Could not connect to XING. Refresh the page or try again later.</b>'; die();
            ...
            if (isset($_GET['auth']) && $_GET['auth']==$ntInfo['lcode'].'a') {
                ...
                $tum_oauth->baseURL = 'https://api.xing.com';
                $tum_oauth->access_token_path = '/v1/access_token';
                ...
                prr($uinfo);
                if (is_array($uinfo) && isset($uinfo['users']) && isset($uinfo['users'][0]) && is_array($uinfo['users'][0])) {
                    $uinfo = $uinfo['users'][0];
                    $options['appPGUserName'] = $uinfo['page_name'];
                    $options['appAppUserName'] = $uinfo['display_name']."(".$uinfo['page_name'].")";
                    $options['appAppUserID'] = $uinfo['id'];
                }
        ...
        public function getPgsList($networks)
        {
            $opVal = array();
            $pass = 'g9c1a'.nsx_doEncode($_POST['p']);
            $opNm = 'nxs_snap_xi_'.sha1('nxs_snap_xi'.$_POST['u'].$pass);
            $opVal = nxs_getOption($opNm);
            $ii = $_POST['ii'];
            $nt = new nxsAPI_XI();// $nt->debug = true; // prr($opVal);
        ...
        public function getGrpList($networks)
        {
            $opVal = array();
            $pass = 'g9c1a'.nsx_doEncode($_POST['p']);
            $opNm = 'nxs_snap_xi_'.sha1('nxs_snap_xi'.$_POST['u'].$pass);
        ...
        public function getGrpForums($networks)
        {
            $opVal = array();
            $pass = 'g9c1a'.nsx_doEncode($_POST['p']);
            $opNm = 'nxs_snap_xi_'.sha1('nxs_snap_xi'.$_POST['u'].$pass);
            ...
            $ii = $_POST['ii'];
            $nt = new nxsAPI_XI();
            $nt->debug = true;// prr($opVal);
            $currPstAs = !empty($_POST['pggID'])?$_POST['pggID']:(!empty($networks['xi'][$ii]['pggID'])?$networks['xi'][$ii]['pggID']:'');
            $currForum = !empty($_POST['gpfID'])?$_POST['gpfID']:(!empty($networks['xi'][$ii]['gpfID'])?$networks['xi'][$ii]['gpfID']:'');
            if (empty($_POST['force']) && !empty($opVal['ck']) && !empty($opVal['grpForums'])) {
                $pgs = $opVal['grpForums'];
            } else {
                if (!empty($opVal['ck'])) {
                    $nt->ck = $opVal['ck'];
                ...
                if (!$loginError) {
                    $opVal['ck'] = $nt->ck;
                    $pgs = $nt->getGrpForums('https://www.xing.com/communities/groups/'.$currPstAs, $currForum);
                } else {
                    $outMsg = '<b style="color:red;">'.__('Login Problem').'&nbsp;-&nbsp;'.$loginError.'</b>';
                    if (!empty($_POST['isOut'])) {
                        echo $outMsg;
            ...
            if (!empty($_POST['isOut'])) {
                echo $pgCust.$pgs.'<option style="color:#BD5200" value="a">'.__('...enter the Group Forum ID').'</option>';
            }
            $opVal['grpForums'] = $pgs;
            nxs_saveOption($opNm, $opVal);
            return $opVal;
        ...
        public function accTab($ii, $options, $isNew=false)
        {
            ...
            $ntInfo = $this->ntInfo;
            $nt = $ntInfo['lcode'];
            if (!empty($options['appKey']) && $options['appKey']=='x5g9a') {
                $options['appKey']='';
            } ?>
   ...
   <div style="display: <?php echo (empty($options['apiToUse']) && empty($options['appKey']))?"block":"none"; ?>;">    
    ...
    <?php if (!empty($options['appKey'])) {
                $options['apiToUse'] = 'xixi';
            } ?>
    ...
    <?php if (class_exists('nxsAPI_XI')) {
                $opNm = 'nxs_snap_xi_'.sha1('nxs_snap_xi'.$options['uName'].$options['uPass']);
                $opVal = nxs_getOption($opNm); //prr($opVal);
                if (empty($opVal) && !empty($options['uPass'])) {
                    ...
                    $_POST['ii']=$ii;
                    $ntw[$nt][$ii]=$options;
                    $opVal = $this->getPgsList($ntw);
                    $opVal = $this->getGrpList($ntw);
                    $_POST = $tPST;
                }
    ...
    <div style="width:100%;"><b style="font-size: 15px;"><?php _e('Where to Post', 'social-networks-auto-poster-facebook-twitter-g');
      ...
      <div style="margin-left: 10px;">        
        <input class="xiWhereToPost<?php echo $ii; ?>" type="radio" name="xi[<?php echo $ii; ?>][whToPost]" value="PR" <?php if ($options['whToPost'] == 'PR') {
        ...
        <div style="margin-left: 30px;"> <div style="width:100%;"><strong style="font-size: 15px;">Post Type:</strong></div><div style="margin-left: 10px;">
           <input type="radio" name="<?php echo $nt; ?>[<?php echo $ii; ?>][postTypeP]" value="T" <?php if ($options['postTypeP'] == 'T') {
                    echo 'checked="checked"';
           ...
           <input type="radio" name="<?php echo $nt; ?>[<?php echo $ii; ?>][postTypeP]" value="A" <?php if (!isset($options['postTypeP']) || $options['postTypeP'] == '' || $options['postTypeP'] == 'A') {
                    echo 'checked="checked"';
                } ?> /> <?php _e('Post link to the blogpost', 'social-networks-auto-poster-facebook-twitter-g'); ?><br/>
         </div><br/>
        </div>
        
        <input class="xiWhereToPost<?php echo $ii; ?>" type="radio" name="xi[<?php echo $ii; ?>][whToPost]" value="C" <?php if ($options['whToPost'] == 'C') {
                    echo 'checked="checked"';
                } ?> /> 
        ...
        <div style="margin-left: 30px;<?php echo (empty($options['uName']) && empty($options['uPass']))?'display:none;':''; ?>" id="nxsXI2InfoDiv<?php echo $ii; ?>"> 
         <div style="width:100%;">
          ...
          </div> <div id="nxsXI2MsgDiv<?php echo $ii; ?>"><?php if (!empty($options['uMsg'])) {
                    echo $options['uMsg'];
                } ?><?php if ($isNew) {
                    ?><i style="color: #800080"><?php _e('Please Enter your username and password to select your page', 'nxs_snap'); ?><?php
                } ?></i></div>                        <div style="margin-top: 10px; margin-bottom: 10px;"><?php $this->elemTitleFormat($ii, 'Company Update Title Format', 'msgCTFormat', empty($options['msgCTFormat'])?'%TITLE%':$options['msgCTFormat']); ?></div>                                                                                                                          
        </div> 
        
        <br/><input class="xiWhereToPost<?php echo $ii; ?>" type="radio" name="xi[<?php echo $ii; ?>][whToPost]" value="G" <?php if ($options['whToPost'] == 'G') {
                    echo 'checked="checked"';
                } ?> /> 
         ...
         <div style="width:100%;">
          ...
          <select id="xi2GfID<?php echo $ii; ?>" onchange="nxs_xi2GfChange('<?php echo $ii; ?>',jQuery(this));" name="xi[<?php echo $ii; ?>][gpfID]">
            <?php $pgi = !empty($options['grpForums'])?$options['grpForums']:'';
                if (!empty($options['gpfID'])) {
                    echo (!empty($options['gpfID']) && stripos($pgi, $options['gpfID'])===false)?'<option selected="selected" value="'.$options['gpfID'].'">'.$options['gpfID'].'</option>':'';
                }
                if (!empty($options['gpfID'])) {
                    $pgi = str_ireplace('selected="selected" ', '', $pgi);
                    $pgi = str_ireplace('value="'.$options['gpfID'].'"', 'selected="selected" value="'.$options['gpfID'].'"', $pgi);
                    $pgi = str_ireplace('data-val="'.$options['gpfID'].'"', 'selected="selected" data-val="'.$options['gpfID'].'"', $pgi);
                }
                echo $pgi; ?><option value="a"><?php _e('.... Enter the Group Forum ID'); ?></option>
          </select>
          <div id="nxsXI2GInfoDivBlock<?php echo $ii; ?>" style="display: inline-block;">
          ...
          <div style="width:100%;"><strong style="font-size: 15px;">Post Type:</strong></div><div style="margin-left: 10px;">
            <input type="radio" name="<?php echo $nt; ?>[<?php echo $ii; ?>][postTypeG]" value="T" <?php if ($options['postTypeG'] == 'T') {
                    echo 'checked="checked"';
                } ?> /> <?php _e('Text Post', 'social-networks-auto-poster-facebook-twitter-g'); ?> - <i><?php _e('just text message', 'social-networks-auto-poster-facebook-twitter-g'); ?></i><br/>                                  
            <input type="radio" name="<?php echo $nt; ?>[<?php echo $ii; ?>][postTypeG]" value="I" <?php if (!isset($options['postTypeG']) || $options['postTypeG'] == '' || $options['postTypeG'] == 'I') {
                    echo 'checked="checked"';
                } ?> /> <?php _e('Image Post', 'social-networks-auto-poster-facebook-twitter-g'); ?><br/>
          </div>
        </div>                       
    ...
    <?php
        ...
        public function setNTSettings($post, $options)
        {
            foreach ($post as $ii => $pval) {
                if (!empty($pval['appKey']) || !empty($pval['uName'])) {
                    if (!isset($options[$ii])) {
                        $options[$ii] = array();
                    }
                    ...
                    if (isset($pval['postTypeP'])) {
                        $options[$ii]['postTypeP'] = trim($pval['postTypeP']);
                    }
                    if (isset($pval['postTypeG'])) {
                        $options[$ii]['postTypeG'] = trim($pval['postTypeG']);
                    }
        
                    if (isset($pval['gpfID'])) {
                        $options[$ii]['gpfID'] = trim($pval['gpfID']);
                    }
                    if (isset($pval['pggID'])) {
        ...
        public function showEdPostNTSettings($ntOpts, $post)
        {
            ...
            foreach ($ntOpts as $ii=>$ntOpt) {
                ...
                $postTypeP = isset($ntOpt['postTypeP'])?$ntOpt['postTypeP']:'';
                $postTypeG = isset($ntOpt['postTypeG'])?$ntOpt['postTypeG']:'';
                ...
                $msgTCFormat = !empty($ntOpt['msgCTFormat'])?htmlentities($ntOpt['msgCTFormat'], ENT_COMPAT, "UTF-8"):'';
        ...
        <input type="radio" name="<?php echo $nt; ?>[<?php echo $ii; ?>][postTypeP]" value="T" <?php if ($postTypeP == 'T') {
                            echo 'checked="checked"';
                        } ?> /> <?php _e('Text Post', 'social-networks-auto-poster-facebook-twitter-g') ?>  - <i><?php _e('just text message', 'social-networks-auto-poster-facebook-twitter-g') ?></i><br/>
        <input type="radio" name="<?php echo $nt; ?>[<?php echo $ii; ?>][postTypeP]" value="A" <?php if (!isset($postTypeP) || $postTypeP == '' || $postTypeP == 'A') {
                            echo 'checked="checked"';
                        } ?> /><?php _e('Text Post with "attached" blogpost', 'social-networks-auto-poster-facebook-twitter-g') ?>
        </td></tr>    
        <?php
                    } elseif ($ntOpt['whToPost']=='C') {
                        $this->elemEdTitleFormat($ii, __('Title Format:', 'social-networks-auto-poster-facebook-twitter-g'), $msgTCFormat);
                    } else {
                        $this->elemEdTitleFormat($ii, __('Title Format:', 'social-networks-auto-poster-facebook-twitter-g'), $msgTFormat); ?> 
            <tr class="nxstbldo <?php echo 'nxstbldo'.strtoupper($nt).$ii; ?>"><th scope="row" style="text-align:right; width:150px; vertical-align:top; padding-top: 0px; padding-right:10px;"> <?php _e('Post Type:', 'social-networks-auto-poster-facebook-twitter-g') ?> <br/></th><td>     
        <input type="radio" name="<?php echo $nt; ?>[<?php echo $ii; ?>][postTypeG]" value="T" <?php if ($postTypeG == 'T') {
                            echo 'checked="checked"';
                        } ?> /> <?php _e('Text Post', 'social-networks-auto-poster-facebook-twitter-g') ?>  - <i><?php _e('just text message', 'social-networks-auto-poster-facebook-twitter-g') ?></i><br/>
        <input type="radio" name="<?php echo $nt; ?>[<?php echo $ii; ?>][postTypeG]" value="A" <?php if (!isset($postTypeG) || $postTypeG == '' || $postTypeG == 'I') {
                            echo 'checked="checked"';
                        } ?> /><?php _e('Image Post', 'social-networks-auto-poster-facebook-twitter-g') ?>
        </td></tr>    
        <?php
                    } ?>    
        ...
        public function showEdPostNTSettingsV4($ntOpt, $post)
        {
            ...
            $ntU = $this->ntInfo['code'];
            $ii = $ntOpt['ii'];
            if (!empty($ntOpt['appKey']) && $ntOpt['appKey']=='x5g9a') {
                $ntOpt['appKey']='';
            }//prr($ntOpt['postType']);
            if (empty($ntOpt['imgToUse'])) {
                $ntOpt['imgToUse'] = '';
   ...
   <div class="nxsPostEd_ElemWrap">   
     ...
     <div class="nxsPostEd_Elem">   
        ...
        <input type="radio" name="<?php echo $nt; ?>[<?php echo $ii; ?>][postTypeP]" value="T" class="nxsEdElem" data-ii="<?php echo $ii; ?>" data-nt="<?php echo $nt; ?>" <?php if ($postTypeP == 'T') {
                    echo 'checked="checked"';
                } ?> /> <?php _e('Text Post', 'social-networks-auto-poster-facebook-twitter-g') ?>  - <i><?php _e('just text message', 'social-networks-auto-poster-facebook-twitter-g') ?></i><br/>
        <input type="radio" name="<?php echo $nt; ?>[<?php echo $ii; ?>][postTypeP]" value="A" class="nxsEdElem" data-ii="<?php echo $ii; ?>" data-nt="<?php echo $nt; ?>" <?php if (!isset($postTypeP) || $postTypeP == '' || $postTypeP == 'A') {
                    echo 'checked="checked"';
                } ?> /><?php _e('Text Post with "attached" blogpost', 'social-networks-auto-poster-facebook-twitter-g') ?>
        ...
        <input type="radio" name="<?php echo $nt; ?>[<?php echo $ii; ?>][postTypeG]" value="T" class="nxsEdElem" data-ii="<?php echo $ii; ?>" data-nt="<?php echo $nt; ?>" <?php if ($postTypeG == 'T') {
                    echo 'checked="checked"';
                } ?> /> <?php _e('Text Post', 'social-networks-auto-poster-facebook-twitter-g') ?>  - <i><?php _e('just text message', 'social-networks-auto-poster-facebook-twitter-g') ?></i><br/>
        <input type="radio" name="<?php echo $nt; ?>[<?php echo $ii; ?>][postTypeG]" value="A" class="nxsEdElem" data-ii="<?php echo $ii; ?>" data-nt="<?php echo $nt; ?>" <?php if (!isset($postTypeG) || $postTypeG == '' || $postTypeG == 'I') {
                    echo 'checked="checked"';
                } ?> /><?php _e('Image Post', 'social-networks-auto-poster-facebook-twitter-g') ?>
        ...
        public function adjMetaOpt($optMt, $pMeta)
        {
            ...
            if (!empty($pMeta['msgTFormat'])) {
                $optMt['msgCTFormat'] = $pMeta['msgTFormat'];
            }
            return $optMt;
        }
...
if (!function_exists("nxs_doPublishToXI")) {
    function nxs_doPublishToXI($postID, $options)
    {
        if (!is_array($options)) {
            $options = maybe_unserialize(get_post_meta($postID, $options, true));
        }
        $cl = new nxs_snapClassXI();
