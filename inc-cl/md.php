...
if (!class_exists("nxs_snapClassMD")) {
    class nxs_snapClassMD extends nxs_snapClassNT
    {
        public $ntInfo = array('code'=>'MD', 'lcode'=>'md', 'name'=>'Medium', 'defNName'=>'', 'tstReq' => true, 'instrURL'=>'https://www.nextscripts.com/instructions/medium-auto-poster-setup-installation/');
        ...
        {
            $defO = array('nName'=>'', 'do'=>'1', 'appKey'=>'', 'appSec'=>'', 'inclTags'=>'1', 'pubList'=>'', 'publ'=>'0', 'msgTFormat'=>'%TITLE%', 'msgFormat'=>"%FULLTEXT%\r\n\r\n%URL%");
            $this->showGNewNTSettings($ii, $defO);
        }
        //#### Show Unit  Settings
        public function checkIfSetupFinished($options)
        {
            return !empty($options['accessToken']) && !empty($options['appKey']);
        }
        public function doAuth()
        {
            ...
            if (isset($_GET['state']) && strlen($_GET['state'])>13 && substr($_GET['state'], 0, 12)=='nxsmdauth-md') {
                ...
                $hdrsArr['Accept']='application/json';
                $hdrsArr['Accept-Charset']='utf-8';
                ...
                $advSet = nxs_mkRemOptsArr($hdrsArr, '', $data);
                $rep = nxs_remote_post('https://api.medium.com/v1/tokens/', $advSet);
                prr($advSet);
                prr($rep);
                $bd = json_decode($rep['body'], true);
                if (!is_array($bd)) {
                    die('ERROR');
                } else {
                    prr($bd);
                }
                $options['accessToken'] = $bd['access_token'];
            
                $hdrsArr['Authorization']='Bearer '.$options['accessToken'];
                $advSet = nxs_mkRemOptsArr($hdrsArr);
                $rep = nxs_remote_get('https://api.medium.com/v1/me', $advSet);
                prr($advSet);
                prr($rep);
                if (is_nxs_error($rep)) {
                ...
                $bd = json_decode($rep['body'], true);
                if (!is_array($bd) || !is_array($bd['data'])) {
                    die('ERROR');
                } else {
                    prr($bd);
                }
                $bd = $bd['data'];
                $options['appAppUserID'] = $bd['id'];
                $options['appAppUserName'] = $bd['name'].' ('.$bd['username'].')';
                $options['appAppUserURL'] = $bd['url'];
                ...
                $pubList = array();
                foreach ($bd as $d) {
                    $repX = nxs_remote_get('https://api.medium.com/v1/publications/'.$d['id'].'/contributors', $advSet);
                    $bdX = json_decode($repX['body'], true);
                    if (!is_array($bdX) || !is_array($bdX['data'])) {
                        die('ERROR');
                    } else {
                        prr($bdX);
                    }
                    $bdX = $bdX['data'];
                    foreach ($bdX as $dX) {
                        if ($dX['userId']==$options['appAppUserID']) {
                            $pubList[] = array('id'=>$d['id'], 'name'=>$d['name']);
                            break;
                        }
                    }
                }
        ...
        public function accTab($ii, $options, $isNew=false)
        {
            ...
            $ntInfo = $this->ntInfo;
            $nt = $ntInfo['lcode'];
            $this->elemKeySecret($ii, 'Medium Client ID', 'Medium Client Secret', $options['appKey'], $options['appSec']); ?><br/><br/><div><b style="font-size: 15px;"><?php _e('Where to post:', 'social-networks-auto-poster-facebook-twitter-g'); ?></b><select name="md[<?php echo $ii; ?>][publ]"><option <?php if (empty($options['publ'])) {
                echo 'selected="selected"';
            } ?>  value="0">Profile</option>
      <?php if (!empty($options['pubList'])) {
                ?> <?php foreach ($options['pubList'] as $pb) {
                    ?> <option <?php if ((string)$pb['id']==(string)$options['publ']) {
                        echo 'selected="selected"';
                    } ?> value="<?php echo $pb['id']; ?>">[Publication]&nbsp;<?php echo $pb['name']; ?></option> <?php
                }
            } ?></select>
    </div>
    ...
    <div style="margin-bottom: 20px;margin-top: 5px;"><input value="1" type="checkbox" name="<?php echo $nt; ?>[<?php echo $ii; ?>][inclTags]"  <?php if ((int)$options['inclTags'] == 1) {
                echo "checked";
            } ?> /> 
        ...
        public function setNTSettings($post, $options)
        {
            foreach ($post as $ii => $pval) {
                if (!empty($pval['appKey']) && !empty($pval['appKey'])) {
                    ...
                    if (isset($pval['publ'])) {
                        $options[$ii]['publ'] = trim($pval['publ']);
                    } else {
                        $options[$ii]['publ'] = 0;
                    }
                } elseif (count($pval)==1) {
...
if (!function_exists("nxs_doPublishToMD")) {
    function nxs_doPublishToMD($postID, $options)
    {
        if (!is_array($options)) {
            $options = maybe_unserialize(get_post_meta($postID, $options, true));
        }
        $cl = new nxs_snapClassMD();
