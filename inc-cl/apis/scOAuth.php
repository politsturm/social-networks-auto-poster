...
if (!class_exists('nxssc_SignatureMethod')) {
...
if (!class_exists('nxssc_SigMethod_HMAC_SHA1')) {
...
class wpScoopITOAuth
{
    ...
    public function get_normalized_parameters($params)
    {
        $items = array();
    ...
    public function genRndString($length = 8)
    {
    ...
    public function makeHTTPHeaders($ref, $post=false)
    {
        $hdrsArr = array();
    ...
    public function oAuthRespToArr($str)
    {
        $arr = explode('&', $str);
        $out = array();
        foreach ($arr as $ar) {
    ...
    public function getReqToken($cbu)
    {
        $args = array(
        ...
        if (stripos($response['body'], 'oauth_token_secret=')===false) {
            echo 'Bad oAuth Login:'.$response['body'];
        } else {
            return $this->oAuthRespToArr($response['body']);
        }
    }
    public function getAccToken($verifier)
    {
        $args = array(
    ...
    public function to_header($params, $realm=null)
    {
        $first = true;
        prr($params);
        if ($realm) {
            $out = 'OAuth realm="' . nxssc_SigMethod_HMAC_SHA1::urlencode_rfc3986($realm) . '"';
            $first = false;
        } else {
            $out = 'OAuth';
        }
        ...
        foreach ($params as $k => $v) {
            ...
            $out .= ($first) ? ' ' : ', '."\r";
            $out .= nxssc_SigMethod_HMAC_SHA1::urlencode_rfc3986($k) . '="' . nxssc_SigMethod_HMAC_SHA1::urlencode_rfc3986($v) . '"';
            $first = false;
        }
        prr($out);
        return $out;
    ...
    public function makeReq($url, $params='', $type='GET')
    {
        $args = array(
        'oauth_token' => $this->access_token,
        'oauth_consumer_key' => $this->consumer_key,
      ...
      $args['oauth_signature'] = $this->sign_method->sign2($req, $this->consumer_secret, $this->access_secret);
        ...
        if ($type=='GET') {
          ...
          $response = nxs_remote_get($url, array( 'method' => 'GET', 'timeout' => 45, 'redirection' => 0,  'headers' => $hdrsArr)); //prr($response);
            ...
            if (!empty($argsAddStr)) {
                $argsStr .= $argsAddStr;
            } //prr($url);  prr($hdrsArr); prr($argsStr);  prr($argsT);
