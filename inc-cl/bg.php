...
if (!class_exists("nxs_snapClassBG")) {
    class nxs_snapClassBG extends nxs_snapClassNT
    {
        public $ntInfo = array('code'=>'BG', 'lcode'=>'bg', 'name'=>'Blogger', 'defNName'=>'', 'tstReq' => true, 'instrURL'=>'https://www.nextscripts.com/setup-installation-blogger-social-networks-auto-poster-wordpress/');
        //#### Update
      ...
      case 340: $ntOptsOut = $this->toLatestVerNTGen($ntOpts); $ntOptsOut['do'] = $ntOpts['do'.$this->ntInfo['code']]; $ntOptsOut['nName'] = $ntOpts['nName'];  $ntOptsOut['blogID'] = $ntOpts['bgBlogID'];
        if (empty($ntOpts['apiToUse'])) {
            if (!empty($ntOpts['APIKey'])) {
                $ntOpts['apiToUse'] = 'bg';
            }
            if (!empty($ntOpts['bgUName']) && !empty($ntOpts['bgPass'])) {
                $ntOpts['apiToUse'] = 'nx';
            }
        } $ntOptsOut['apiToUse'] = $ntOpts['apiToUse'];
        if ($ntOptsOut['apiToUse']=='nx') {
            $ntOptsOut['uName'] = $ntOpts['bgUName'];
            $ntOptsOut['uPass'] = $ntOpts['bgPass'];
        } else {
            $ntOptsOut['appKey'] = $ntOpts['APIKey'];
            $ntOptsOut['appSec'] = $ntOpts['APISec'];
            $ntOptsOut['accessToken'] = $ntOpts['AccessToken'];
            $ntOptsOut['accessTokenSec'] = $ntOpts['AccessTokenSecret'];
            $options['refreshToken'] =  $options['RefreshToken'];
            $options['accessTokenExp'] =  $options['AccessTokenExp'];
            $ntOptsOut['blogInfo'] = $ntOpts['blogInfo'];
      ...
      break;
        ...
        {
            $defO = array('nName'=>'', 'do'=>'1', 'blogID'=>'', 'appKey'=>'', 'appSec'=>'', 'uName'=>'', 'uPass'=>'', 'inclTags'=>1, 'msgFormat'=>"%RAWTEXT%", 'msgTFormat'=>"%TITLE%", 'imgSize'=>'original');
        ...
        public function doAuth()
        {
            ...
            if (isset($_GET['code']) && $_GET['code']!='' && isset($_GET['state']) && substr($_GET['state'], 0, 7) == 'nxs-bg-') {
                ...
                if (isset($options['appKey'])) {
                    ...
                    $options['accessTokenSec'] = 'No Need for oAuth V2';
                    $options['accessTokenExp'] = $currTime + $resp['expires_in'];
                    $options['refreshToken'] = $resp['refresh_token'];
                    echo "<br/>----=={ Expires: ".date('Y-m-d H:i:s', $options['accessTokenExp'])." }==---- <br/>";
        
                    if (!empty($options['blogID'])) {
                        if (substr($options['blogID'], 0, 4)=='http') {
                            $tknURL = 'https://www.googleapis.com/blogger/v3/blogs/byurl/?url='.$options['blogID'].'?access_token='.$options['accessToken'];
                        } else {
                            $tknURL = 'https://www.googleapis.com/blogger/v3/blogs/'.$options['blogID'].'?access_token='.$options['accessToken'];
                        }
                    }
                    $response  = nxs_remote_get($tknURL, $wprg);
                    prr($tknURL);
                    prr($response);
                    $user = json_decode($response['body'], true);
                    prr($user);
                    if (!empty($user['url'])) {
                        $options['blogURL'] = $user['url'];
                        $options['blogID'] = $user['id'];
                        $options['blogInfo'] = $user['name']." [".$user['id']."] (".$user['url'].")";
                        nxs_save_glbNtwrks($ntInfo['lcode'], $ii, $options, '*'); ?><script type="text/javascript">window.location = "<?php echo $nxs_snapSetPgURL; ?>"</script>      
        <?php
                    }
                }
                die();
    ...
    <br/><?php $this->elemTitleFormat($ii, 'Message Title Format', 'msgTFormat', $options['msgTFormat']);
            $this->elemMsgFormat($ii, 'Message Format', 'msgFormat', $options['msgFormat']); ?>
  ...
  public function setNTSettings($post, $options)
  {
    ...
    foreach ($options as $oo => $v) {
        ...
        if (isset($oo) && $oo!=='' && ((!empty($v['appKey']) && !empty($v['appSec'])) || (!empty($v['uPass']) && !empty($v['uName'])))) {
            $otp[$oo] = $v;
        }
    }
      $options = $otp;
      foreach ($post as $ii => $pval) {
          if (!empty($pval['blogID']) && !empty($pval['blogID'])) {
              if (!isset($options[$ii])) {
                  $options[$ii] = array();
              }
              ...
              if (isset($pval['blogID'])) {
                  $options[$ii]['blogID'] = trim($pval['blogID']);
              }
          } elseif (count($pval)==1) {
              if (isset($pval['do'])) {
