...
if (!class_exists('nxsAPI_DA')) {
    class nxsAPI_DA
    {
        ...
        public $mh = '';
        public $debug = false;
        public function headers($ref, $org='', $post=false, $aj=false)
        {
            $hdrsArr = array();
        ...
        public function connect($u, $p)
        {
            ...
            if (!$this->check()) {
                ...
                if (isset($rep['headers']['location']) && stripos($rep['headers']['location'], 'wrong-password')!==false) {
                ...
                $response = nxs_remote_post($url, $advSet);
                ...
                $ck =  $response['cookies'];
                foreach ($ck as $i=>$ckk) {
                    $ck[$i]->value = urlencode($ckk->value);
                }
                ...
                if (isset($response['headers']['location']) && ($response['headers']['location']=='http://www.deviantart.com' || $response['headers']['location']=='https://www.deviantart.com/users/loggedin')) {
                    $hdrsArr = $this->headers('http://www.deviantart.com');
                    $advSet = nxs_mkRemOptsArr($hdrsArr, $ck, '', '', 2);
                    $rep = nxs_remote_get('http://www.deviantart.com/browse/all/', $advSet);// prr($advSet); prr($rep); die();
            
                    if (is_nxs_error($rep)) {
                        $badOut = "ERROR (Login 3): ".print_r($rep, true);
                        return $badOut;
                    }
                    $mh = CutFromTo($rep['body'], "logoutme').submit();\"><i></i>", 'data-ga_click_event');
                    $mh = CutFromTo($mh, 'href="', '"'); // prr($mh);
                    $ck2 =  $rep['cookies'];
                    for ($i=0;$i<count($ck);$i++) {
                        if ($ck[$i]->name=='userinfo') {
                            $ck[$i]->value = urlencode($ck2[0]->value);
                    ...
                    $badOut = print_r($response, true)." - ERROR  Login 4";
                }
                return $badOut;
            } else {
                if ($this->debug) {
                    echo "[DA] Saved Data is OK;<br/>\r\n";
                }
                return false;
            }
        }
        public function post($post)
        {
            ...
            $hdrsArr = $this->headers('http://www.deviantart.com/');
            $badOut = '';
            $rep = nxs_remote_get($mh.'/journal/?edit', nxs_mkRemOptsArr($hdrsArr, $ck));
            if (is_nxs_error($rep)) {
                $badOut = " - ERROR (Post 1): ".$mh.'/journal/?edit' . " | " . print_r($rep, true);
                return $badOut;
            }
            ...
            while (stripos($contents, '"hidden"')!==false) {
                ...
                if (!in_array($name, $md)) {
                    ...
                    $flds[$name]= urldecode(nxs_decodeEntitiesFull($val));
                }
            }
            $flds['subject'] = nsTrnc(nxs_decodeEntitiesFull(addslashes($post['title'])), 50);
            $flds['body'] = trim(addslashes($post['text']));
            $flds['song'] = '';
            $flds['game'] = '';
            $flds['book'] = '';
            $flds['food'] = '';
            ...
            $advSets = nxs_mkRemOptsArr($hdrsArr, $ck, $dfldsq1);
            $rep = nxs_remote_post($mh.'/global/difi/?', $advSets); //prr($mh.'/global/difi/?');  prr($advSets);       prr($rep);
            if (is_nxs_error($rep)) {
            ...
            $advSets = nxs_mkRemOptsArr($hdrsArr, $ck, $dfldsq2);
            $rep = nxs_remote_post($mh.'/global/difi/?', $advSets); // sleep(6); //## Important.      //prr($advSets); prr($rep); die();
            $advSets = nxs_mkRemOptsArr($hdrsArr, $ck, $fldsOut);
            $response = nxs_remote_post($mh.'/journal/?edit', $advSets); //prr($mh.'/journal/?edit');  prr($advSets); prr($response);
      if ($response['response']['code']=='200' && stripos($response['body'], 'field_error')!==false) {
          ...
          return $badOut;
            ...
            if ($response['response']['code']=='302') {
                ...
                $rep = nxs_remote_get($mh.'/journal/', $advSet);
                if (is_nxs_error($rep)) {
                    $badOut = print_r($rep, true)." - ERROR DFI 1";
                    return $badOut;
                }
...
if (!class_exists("nxs_class_SNAP_DA")) {
    class nxs_class_SNAP_DA
    {
        ...
        public function doPostToNT($options, $message)
        {
            ...
            if (!isset($options['uName']) || trim($options['uName'])=='' || !isset($options['uPass']) || trim($options['uPass'])=='') {
            ...
            if (!empty($message['pTitle'])) {
                ...
                $title = nxs_doFormatMsg($options['msgTFormat'], $message);
            }
            $title = nsTrnc($title, 300);
            if (!empty($message['pText'])) {
                $text = $message['pText'];
            } else {
                $text = nxs_doFormatMsg($options['msgFormat'], $message);
            }
            //## Get Saved Login Info
            if (function_exists('nxs_getOption')) {
                $opVal = array();
                $opNm = 'nxs_snap_da_'.sha1('nxs_snap_da'.$options['uName'].$options['uPass']);
