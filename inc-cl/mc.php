...
if (!class_exists("nxs_snapClassMC")) {
    class nxs_snapClassMC extends nxs_snapClassNT
    {
        public $ntInfo = array('code'=>'MC', 'lcode'=>'mc', 'name'=>'MailChimp', 'defNName'=>'uName', 'tstReq' => false, 'instrURL'=>'https://www.nextscripts.com/instructions/mailchimp-auto-poster-setup-installation/');
        ...
        {
            $defO = array('nName'=>'', 'do'=>'1', 'apikey'=>'', 'fromEmail'=>'', 'fromName'=>'', 'dc'=>'', 'msgTFormat'=>'%TITLE%', 'listID'=>'',  'msgFormat'=>'%EXCERPT% - %URL%');
	...
	<br/><?php $this->elemTitleFormat($ii, 'Message Subject', 'msgTFormat', $options['msgTFormat']);
            $this->elemMsgFormat($ii, 'Message Text', 'msgFormat', $options['msgFormat']);
        }
        public function advTab()
        {
        }
        ...
        public function setNTSettings($post, $options)
        {
            foreach ($post as $ii => $pval) {
                if (!empty($pval['apikey']) && !empty($pval['apikey'])) {
                    if (!isset($options[$ii])) {
                        $options[$ii] = array();
                    }
                    ...
                    if (isset($pval['apikey'])) {
                        $options[$ii]['apikey'] = trim($pval['apikey']);
                        $options[$ii]['dc'] = end(explode('-', trim($pval['apikey'])));
                    }
                    if (isset($pval['listID'])) {
                        $options[$ii]['listID'] = trim($pval['listID']);
                    }
                    if (isset($pval['segment'])) {
                        $options[$ii]['segment'] = trim($pval['segment']);
                    }
                    if (isset($pval['fromEmail'])) {
                        $options[$ii]['fromEmail'] = trim($pval['fromEmail']);
                    }
                    if (isset($pval['fromName'])) {
                        $options[$ii]['fromName'] = trim($pval['fromName']);
                    }
                } elseif (count($pval)==1) {
                    if (isset($pval['do'])) {
        ...
        public function showEdPostNTSettings($ntOpts, $post)
        {
            ...
            foreach ($ntOpts as $ii=>$ntOpt) {
                ...
                $doNT = $ntOpt['do'] && (is_array($pMeta) || $ntOpt['fltrsOn']!='1');
		...
		<?php $this->elemEdTitleFormat($ii, __('Subject Format:', 'social-networks-auto-poster-facebook-twitter-g'), $msgTFormat);
...
if (!function_exists("nxs_doPublishToMC")) {
    function nxs_doPublishToMC($postID, $options)
    {
        if (!is_array($options)) {
            $options = maybe_unserialize(get_post_meta($postID, $options, true));
        }
        $cl = new nxs_snapClassMC();
