...
if (!class_exists("nxs_snapClassPN")) {
    class nxs_snapClassPN extends nxs_snapClassNT
    {
        public $ntInfo = array('code'=>'PN', 'lcode'=>'pn', 'name'=>'Pinterest', 'defNName'=>'uName', 'tstReq' => false, 'instrURL'=>'https://www.nextscripts.com/setup-installation-pinterest-social-networks-auto-poster-wordpress');
  
        public $noFuncMsg = 'Sorry, but Pinterest doesn\'t have a built-in API for automated posts yet. <br/>You need a special API library module to be able to publish your content to Pinterest.';
        public function checkIfFunc()
        {
            return class_exists('nxsAPI_PN');
        }
      ...
      break;
        ...
        {
            $defO = array('nName'=>'', 'do'=>'1', 'uName'=>'', 'uPass'=>'',  'cImgURL'=>'', 'isAttachVid'=>'', 'isAttachVid'=>'', 'defImg'=>'', 'msgFormat'=>"%EXCERPT%");
        ...
        public function getMergeOptInfo($options, $ii)
        {
            $p = $options['uPass'];
            if (!empty($p)) {
                ...
                $_POST['pnBoard'] = $options['pnBoard'];
                $_POST['u'] = $options['uName'];
                $_POST['p'] = $p;
                $_POST['ii'] = $ii;
                $opNm = 'nxs_snap_pn_'.sha1('nxs_snap_pn'.$options['uName'].$options['uPass']);
                $opVal = nxs_getOption($opNm);
                if (empty($opVal)) {
                    $ntw[$nt][$ii]=$options;
                    $opVal = $this->getListOfPNBoards($ntw);
                }
        ...
        public function accTab($ii, $options, $isNew=false)
        {
            ...
            $options = $this->getMergeOptInfo($options, $ii); ?>
    ...
    <div id="nxsPNInfoDiv<?php echo $ii; ?>">
         <div style="width:100%;">
          ...
          <select id="pnBoard<?php echo $ii; ?>" onchange="nxs_pnBoardChange('<?php echo $ii; ?>',jQuery(this));" name="pn[<?php echo $ii; ?>][pnBoard]">
            ...
            if (!empty($options['pnBoard'])) {
                $pgi = str_ireplace('selected="selected" ', '', $pgi);
                $pgi = str_ireplace('value="'.$options['pnBoard'].'"', 'selected="selected" value="'.$options['pnBoard'].'"', $pgi);
            } else {
                echo '<option value="">None(Click refresh icon to retrieve your boards)</option>';
            }
            echo $pgi; ?>
          ...
          <div id="nxsPNMsgDiv<?php echo $ii; ?>"><?php if (!empty($options['uMsg'])) {
                echo $options['uMsg'];
            } ?><?php if ($isNew) {
                ?><?php _e('Please enter your login/password to see the list of your boards', 'nxs_snap'); ?><?php
            } ?></div>                                                 
    </div>
...
<input type="radio" name="pn[<?php echo $ii; ?>][cImgURL]" value="R" <?php if (empty($options['cImgURL']) || $options['cImgURL'] == 'R') {
                echo 'checked="checked"';
            } ?> /><?php _e('Regular Post URL', 'social-networks-auto-poster-facebook-twitter-g'); ?>&nbsp;&nbsp;
<!-- <input type="radio" name="pn[<?php echo $ii; ?>][cImgURL]" value="S" <?php if (!empty($options['cImgURL']) && $options['cImgURL'] == 'S') {
                echo 'checked="checked"';
            } ?> /> Shortened Post URL&nbsp;&nbsp; -->
<input type="radio" name="pn[<?php echo $ii; ?>][cImgURL]" value="N" <?php if (!empty($options['cImgURL']) && $options['cImgURL'] == 'N') {
                echo 'checked="checked"';
            } ?> /><?php _e('No Clickthrough URL', 'social-networks-auto-poster-facebook-twitter-g'); ?>&nbsp;&nbsp;</div>
            ...
            <br/><br/>   
             ...
             <div style="margin-bottom: 5px; margin-left: 0px; "><input value="1"  id="isAttachVid" type="checkbox" name="pn[<?php echo $ii; ?>][isAttachVid]"  <?php if (isset($options['isAttachVid']) && (int)$options['isAttachVid'] == 1) {
                echo "checked";
            } ?> />    <strong><?php _e('If post has a video use it instead of image.', 'social-networks-auto-poster-facebook-twitter-g'); ?></strong><br/><i><?php _e('Video will be pinned instead of featured image. Only Youtube is supported at this time.', 'social-networks-auto-poster-facebook-twitter-g'); ?></i>
    <br/><br/></div>  
    <?php $this->elemMsgFormat($ii, 'Post Text Format', 'msgFormat', $options['msgFormat']); ?><br/ ><?php
        ...
        public function setNTSettings($post, $options)
        {
            foreach ($post as $ii => $pval) {
                if (!empty($pval['uPass']) && !empty($pval['uPass'])) {
                    ...
                    if (isset($pval['pnBoard'])) {
                        $options[$ii]['pnBoard'] = trim($pval['pnBoard']);
                    }
                    if (isset($pval['isAttachVid'])) {
                        $options[$ii]['isAttachVid'] = trim($pval['isAttachVid']);
                    } else {
        ...
        <tr style="<?php echo !empty($ntOpt['do'])?'display:table-row;':'display:none;'; ?>" class="nxstbldo nxstbldo<?php echo strtoupper($nt).$ii; ?>"><th scope="row" style="text-align:right; width:150px; padding-top: 5px; padding-right:10px;">Select Board</th>
            ...
            <?php  $ntOpt = $this->getMergeOptInfo($ntOpt, $ii);
                if (!empty($ntOpt['pnBoardsList'])) {
                    if ($ntOpt['pnBoard']!='') {
                        $gPNBoards = str_replace($ntOpt['pnBoard'].'"', $ntOpt['pnBoard'].'" selected="selected"', str_ireplace('selected="selected" ', '', $ntOpt['pnBoardsList']));
                    }
                    echo $gPNBoards;
                } else {
                    ?>
  ...
  public function adjMetaOpt($optMt, $pMeta)
  {
    ...
    if (!empty($pMeta['pnBoard'])) {
        $optMt['pnBoard'] = $pMeta['pnBoard'];
    }
      return $optMt;
  }
        ...
        //## PN Specific
        public function getListOfPNBoards($networks)
        {
            $opVal = array();
            $pass = 'g9c1a'.nsx_doEncode($_POST['p']);
            $opNm = 'nxs_snap_pn_'.sha1('nxs_snap_pn'.$_POST['u'].$pass);
            $opVal = nxs_getOption($opNm);
            $ii = $_POST['ii'];
            if (!class_exists("nxsAPI_PN")) {
                $outMsg = '<b style="color:red;">'.__('API Not Found').'&nbsp;-&nbsp;'.$loginError.'</b>';
            ...
            $nt = new nxsAPI_PN();
            $nt->debug = false; // prr($opVal);
            $currPstAs = !empty($_POST['pnBoard'])?$_POST['pnBoard']:(!empty($networks['pn'][$ii])?$networks['pn'][$ii]['pnBoard']:'');
            if (empty($_POST['force']) && !empty($opVal['ck']) && !empty($opVal['pnBoardsList'])) {
                $pgs = $opVal['pnBoardsList'];
            } else {
                if (!empty($opVal['ck'])) {
                    $nt->ck = $opVal['ck'];
                }
            ...
            if (!empty($_POST['isOut'])) {
                echo(stripos($pgs, 'No Boards Found')===false?$pgCust:'').$pgs;
            } // .'<option style="color:#BD5200" value="a">'.__('...enter the SubReddit ID').'</option>';
            $opVal['pnBoardsList'] = $pgs;
