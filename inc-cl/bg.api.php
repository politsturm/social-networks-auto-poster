...
if (!class_exists("nxs_class_SNAP_BG")) {
    class nxs_class_SNAP_BG
    {
        ...
        public function doPostToNT($options, $message)
        {
            ...
            if (class_exists('nxsAPI_GP') && !empty($options['uName']) && empty($options['accessToken'])) {
                ...
                if ($options['accessTokenExp']<$currTime) {
                    ...
                    $response  = nxs_remote_post($tknURL);
                    $resp = json_decode($response['body'], true);
                    $options['accessToken'] = $resp['access_token'];
                    $options['accessTokenExp'] = $currTime + $resp['expires_in'];
                    nxs_save_glbNtwrks('bg', $options['ii'], $resp['access_token'], 'accessToken');
                ...
                $post = json_encode($post); // prr($post);
        $hdrsArr = array('Content-Type'=>'application/json');
                ...
                $ret = '';
                $response  = nxs_remote_post($tknURL, $advSet); //prr($tknURL); prr($response);
