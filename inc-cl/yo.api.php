...
if (!class_exists("nxs_class_SNAP_YO")) {
    class nxs_class_SNAP_YO
    {
        public $ntCode = 'YO';
        public $ntLCode = 'yo';
        ...
        public function doPostToNT($options, $message)
        {
            ...
            foreach ($users as $user) {
                $data['username'] = $user->meta_value;//  echo $user->user_id.'|'.$user->meta_value;
                $advSet = nxs_mkRemOptsArr($hdrsArr, '', $data);
                $rep = nxs_remote_post('http://api.justyo.co/yo/', $advSet); // prr($advSet); prr($rep);
        $jsRep = json_decode($rep['body'], true);
                if (!empty($jsRep['success'])) {
                    $gCnt++;
                } else {
                    $bCnt++;
                }   // $hddr = print_r($rep['headers'], true);  $limit = trim(CutFromTo($hddr, '[x-ratelimit-remaining] =>', '[')); prr($limit);
            }
