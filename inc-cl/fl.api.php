...
if (!class_exists("nxs_class_SNAP_FL")) {
    class nxs_class_SNAP_FL
    {
        ...
        public function createFile($imgURL)
        {
            ...
            $imgData = nxs_remote_get($remImgURL, array('timeout' => 45));
            if (is_nxs_error($imgData)) {
                $badOut['Error'] = print_r($imgData, true)." - ERROR";
                return $badOut;
            }
            if (isset($imgData['content-type'])) {
                $cType = !empty($imgData['content-type'])?$imgData['content-type']:'image/jpeg';
            }
            $imgData = $imgData['body'];
            $tmp=array_search('uri', @array_flip(stream_get_meta_data($GLOBALS[mt_rand()]=tmpfile())));
            if (!is_writable($tmp)) {
                $badOut['Error'] = "Your temporary folder or file (file - ".$tmp.") is not witable. Can't upload images to Flickr";
                return $badOut;
            }
            rename($tmp, $tmp.='.png');
