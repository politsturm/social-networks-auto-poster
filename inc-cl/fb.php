...
if (!class_exists("nxs_snapClassFB")) { class nxs_snapClassFB extends nxs_snapClassNT { 
  ...
  function getPageToken($fbo){ $advSet = nxs_mkRemOptsArr(nxs_getNXSHeaders()); $errMsg = ''; $fbPgID = $fbo['pgID'];   
      ...
      $opNm = 'nxs_snap_fb_'.sha1('nxs_snap_fb'.$fbo['authUser'].nxs_gak($fbo['appKey'])); $opVal = nxs_getOption($opNm); if (!empty($opVal['pageListArr'])) foreach ($opVal['pageListArr'] as $pgL) if ($pgL['id']==$fbPgID) { $fbo['pageAccessToken'] = $pgL['tk'];  $fbo['uMsg'] = ''; return $fbo; }
      //## Token from FB API 
  ...
  function accTab($ii, $options, $isNew=false){ global $nxs_snapSetPgURL; $ntInfo = $this->ntInfo; $nt = $ntInfo['lcode']; $ntU = strtoupper($nt);// prr($options); 
    ...
    <?php  if ( empty($options['apiToUse']) || $options['apiToUse'] =='nx' || $options['apiToUse'] =='nxv2') { //############# NXS API ?>    
      ...
      <?php if (class_exists('nxsAPI_FB')) {  $pass = (substr($options['uPass'], 0, 5)=='g9c1a')?nsx_doDecode(substr($options['uPass'], 5)):$options['uPass']; 
        ...
        <div class="subDiv" id="sub<?php echo $ii; ?>DivN" style="display: block;"><?php if ( empty($options['apiToUse']) || $options['apiToUse'] =='nx') { $l1 = 'Facebook User ID (c_user)'; $l2 = 'Session ID (xs)'; } else {$l1 = 'Facebook Username/Email'; $l2 = 'Password';} $this->elemKeySecret($ii,$l1,$l2, $options['uName'], $pass,'uName','uPass');  ?>
