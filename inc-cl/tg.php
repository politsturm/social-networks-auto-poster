...
if (!class_exists("nxs_snapClassTG")) {
    class nxs_snapClassTG extends nxs_snapClassNT
    {
        public $ntInfo = array('code'=>'TG', 'lcode'=>'tg', 'name'=>'Telegram', 'defNName'=>'uName', 'tstReq' => false, 'instrURL'=>'https://www.nextscripts.com/instructions/telegram-auto-poster-setup-installation/');
        ...
        {
            $defO = array('nName'=>'', 'do'=>'1', 'botTkn'=>'', 'msgTFormat'=>'', 'whToPost'=>'', 'attchImg'=>0, 'webPrev'=>1, 'msgFormat'=>'%TITLE% - %URL%');
            $this->showGNewNTSettings($ii, $defO);
        }
        //#### Show Unit  Settings
        public function checkIfSetupFinished($options)
        {
            return !empty($options['botTkn']);
        }
        public function accTab($ii, $options, $isNew=false)
        {
            $ntInfo = $this->ntInfo;
            $nt = $ntInfo['lcode'];
            if (empty($options['attchImg'])) {
                $options['attchImg'] = 0;
            }
            if (!isset($options['webPrev'])) {
                $options['webPrev'] = 1;
            } ?> <br/ >
    ...
    <div style="margin: 0px;"><input value="1" type="checkbox" name="<?php echo $nt; ?>[<?php echo $ii; ?>][webPrev]"  <?php if ((int)$options['webPrev'] == 1) {
                echo "checked";
            } ?> /> <strong><?php _e('Enable Web Preview', 'social-networks-auto-poster-facebook-twitter-g'); ?></strong></div>
    <br/><?php
        }
        ...
        public function setNTSettings($post, $options)
        {
            foreach ($post as $ii => $pval) {
                if (!empty($pval['botTkn']) && !empty($pval['botTkn'])) {
                    if (!isset($options[$ii])) {
                        $options[$ii] = array();
                    }
                    $options[$ii] = $this->saveCommonNTSettings($pval, $options[$ii]);
                    //## Uniqe Items
                    if (isset($pval['botTkn'])) {
                        $options[$ii]['botTkn'] = trim($pval['botTkn']);
                    }
                    ...
                    if (isset($pval['webPrev'])) {
                        $options[$ii]['webPrev'] = trim($pval['webPrev']);
                    } else {
                        $options[$ii]['webPrev'] = 0;
                    }
		...
		<?php $this->elemEdMsgFormat($ii, __('Message Format:', 'social-networks-auto-poster-facebook-twitter-g'), $msgFormat); ?>
          ...
          <input value="1" type="checkbox" name="<?php echo $nt; ?>[<?php echo $ii; ?>][webPrev]"  <?php if ((int)$ntOpt['webPrev'] == 1) {
                    echo "checked";
                } ?> /> <strong><?php _e('Enable Web Preview', 'social-networks-auto-poster-facebook-twitter-g'); ?></strong></div></td></tr>
            <tr><td>&nbsp;</td><td><div style="margin: 0px;"><input value="0" type="hidden" name="<?php echo $nt; ?>[<?php echo $ii; ?>][attchImg]"/>
          <input value="1" type="checkbox" name="<?php echo $nt; ?>[<?php echo $ii; ?>][attchImg]"  <?php if ((int)$ntOpt['attchImg'] == 1) {
                    echo "checked";
                } ?> /> <strong><?php _e('Attach Image to the Post', 'social-networks-auto-poster-facebook-twitter-g'); ?></strong></div></td></tr>
        <?php
   ...
   <div class="nxsPostEd_ElemWrap">        
     <div class="nxsPostEd_Elem">   
        <div style="margin: 0px;"><input value="0" type="hidden" name="<?php echo $nt; ?>[<?php echo $ii; ?>][webPrev]"/>
          <input value="1" type="checkbox" name="<?php echo $nt; ?>[<?php echo $ii; ?>][webPrev]" class="nxsEdElem" data-ii="<?php echo $ii; ?>" data-nt="<?php echo $nt; ?>" <?php if (!empty($ntOpt['webPrev'])) {
                echo "checked";
            } ?> /> <strong><?php _e('Enable Web Preview', 'social-networks-auto-poster-facebook-twitter-g'); ?></strong></div>
        <div style="margin: 0px;"><input value="0" type="hidden" name="<?php echo $nt; ?>[<?php echo $ii; ?>][attchImg]"/>
          <input value="1" type="checkbox" name="<?php echo $nt; ?>[<?php echo $ii; ?>][attchImg]" class="nxsEdElem" data-ii="<?php echo $ii; ?>" data-nt="<?php echo $nt; ?>" <?php if (!empty($ntOpt['attchImg'])) {
                echo "checked";
            } ?> /> <strong><?php _e('Attach Image to the Post', 'social-networks-auto-poster-facebook-twitter-g'); ?></strong></div>
     </div>
   </div><?php
        ...
        public function adjMetaOpt($optMt, $pMeta)
        {
            ...
            if (!isset($optMt['webPrev'])) {
                $optMt['webPrev'] = 1;
            }
            //if (!empty($pMeta['tgBoard'])) $optMt['tgBoard'] = $pMeta['tgBoard'];
            if (isset($pMeta['webPrev'])) {
                $optMt['webPrev'] = trim($pMeta['webPrev']);
            }
            if (isset($pMeta['attchImg'])) {
                $optMt['attchImg'] = trim($pMeta['attchImg']);
            }
            return $optMt;
        }
...
if (!function_exists("nxs_doPublishToTG")) {
    function nxs_doPublishToTG($postID, $options)
    {
        if (!is_array($options)) {
            $options = maybe_unserialize(get_post_meta($postID, $options, true));
        }
        $cl = new nxs_snapClassTG();
