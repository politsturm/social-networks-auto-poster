...
if (!class_exists('nxsAPI_ST')) {
    class nxsAPI_ST
    {
        public $ck = array();
        public $debug = false;
        public function headers($ref, $org='', $type='GET', $aj=false)
        {
            $hdrsArr = array();
            $hdrsArr['Cache-Control']='max-age=0';
            $hdrsArr['Connection']='keep-alive';
        ...
        public function check()
        {
            ...
            if (!empty($ck) && is_array($ck)) {
                $hdrsArr = $this->headers('https://sett.com');
                if ($this->debug) {
                    echo "[ST] Checking....;<br/>\r\n";
                }
        ...
        public function connect($u, $p)
        {
            ...
            if (!$this->check()) {
                ...
                $rep = nxs_remote_get('http://sett.com/', array('headers' => $hdrsArr, 'httpversion' => '1.1', 'sslverify'=>$sslverify));
                if (is_nxs_error($rep)) {
                ...
                $ck = $rep['cookies'];
                $rep = nxs_remote_get($llURL, array('headers' => $hdrsArr, 'cookies' => $ck, 'httpversion' => '1.1', 'sslverify'=>$sslverify));
                if (is_nxs_error($rep)) {
                    $badOut = print_r($rep, true);
                    if (stripos($badOut, 'Operation timed out')!==false) {
                        return "Invalid login email!";
                    } else {
                        return "ERROR (Login Form 2): ".$badOut;
                    }
                }
                if ($rep['response']['code']!='200') {
                ...
                if ($rep['response']['code']=='200') {
                    ...
                    if (stripos($content, '{"user":{"')!==false) {
                        $ck = $rep['cookies'];
                        $this->ck = $ck;
                        $loginArr = json_decode($content, true); /* prr($loginArr); */  return false;
                    }
...
if (!class_exists("nxs_class_SNAP_ST")) {
    class nxs_class_SNAP_ST
    {
        ...
        public function createFile($imgURL)
        {
        ...
        public function doPostToNT($options, $message)
        {
            ...
            if (function_exists('nxs_getOption')) {
                $opVal = array();
                $opNm = 'nxs_snap_st_'.sha1('nxs_snap_st'.$options['uName'].$options['uPass']);
