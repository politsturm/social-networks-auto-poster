...
if (!class_exists("nxs_snapClassTW")) {
    class nxs_snapClassTW extends nxs_snapClassNT
    {
        public $ntInfo = array('code'=>'TW', 'lcode'=>'tw', 'name'=>'Twitter', 'defNName'=>'', 'tstReq' => true, 'instrURL'=>'https://www.nextscripts.com/setup-installation-twitter-social-networks-auto-poster-wordpress/');
        public $defO = array('nName'=>'', 'do'=>'1', 'twURL'=>'', 'appKey'=>'', 'appSec'=>'', 'accessToken'=>'', 'accessTokenSec'=>'', 'attchImg'=>1, 'msgFormat'=>"New post (%TITLE%) has been published on %SITENAME% - %URL%", 'imgSize'=>'original');
        //#### Update
      ...
      break;
        ...
        {
            return !empty($options['appKey']) && !empty($options['accessToken']);
        }
    ...
    <?php $this->elemKeySecret($ii, 'API Key', 'API Secret', $options['appKey'], $options['appSec'], 'appKey', 'appSec', 'https://apps.twitter.com/');
            $this->elemKeySecret($ii, 'Access Token', 'Access Token Secret', $options['accessToken'], $options['accessTokenSec'], 'accessToken', 'accessTokenSec'); ?>
    <br/><?php $this->elemMsgFormat($ii, 'Message Format', 'msgFormat', $options['msgFormat']); ?>    
    <div style="margin: 0px;"><input value="1" type="checkbox" name="<?php echo $nt; ?>[<?php echo $ii; ?>][attchImg]"  <?php if ((int)$options['attchImg'] == 1) {
                echo "checked";
            } ?> /> <strong><?php _e('Attach Image to the Post', 'social-networks-auto-poster-facebook-twitter-g'); ?></strong></div>
    <br/><br/><?php
        }
        public function advTab($ii, $options)
        {
            $nt = $this->ntInfo['lcode']; ?><div class="nxs_tls_cpt"><?php _e('Auto Import of Replies and Mentions:', 'social-networks-auto-poster-facebook-twitter-g'); ?></div>   
   ...
   <?php global $nxs_SNAP;
            ...
            if (!empty($gOptions['riActive']) && $gOptions['riActive'] == '1') {
                ?>
   <input value="1" id="riC<?php echo $ii; ?>" <?php if (!empty($options['riComments']) && trim($options['riComments'])=='1') {
                    echo "checked";
                } ?> type="checkbox" name="<?php echo $nt; ?>[<?php echo $ii; ?>][riComments]"/> <b><?php _e('Import Twitter Replies', 'social-networks-auto-poster-facebook-twitter-g'); ?></b>
   <br/>
   <input value="1" id="riCM<?php echo $ii; ?>" <?php if (!empty($options['riCommentsM']) && trim($options['riCommentsM'])=='1') {
                    echo "checked";
                } ?> type="checkbox" name="<?php echo $nt; ?>[<?php echo $ii; ?>][riCommentsM]"/> <b><?php _e('Import Twitter Mentions', 'social-networks-auto-poster-facebook-twitter-g'); ?></b>
   <br/> 
   <input value="1" id="riCA<?php echo $ii; ?>" <?php if (!empty($options['riCommentsAA']) && trim($options['riCommentsAA'])=='1') {
                    echo "checked";
                } ?> type="checkbox" name="<?php echo $nt; ?>[<?php echo $ii; ?>][riCommentsAA]"/> <b><?php _e('Auto-approve imported comments', 'social-networks-auto-poster-facebook-twitter-g'); ?></b>
   <?php
            } else {
                echo "<br/>";
   ...
   <div class="nxs_tls_bd"><div class="nxs_tls_sbInfo"><br/>
     <input value="1" <?php if (!empty($options['tw140']) && trim($options['tw140'])=='1') {
                echo "checked";
            } ?> type="checkbox" name="<?php echo $nt; ?>[<?php echo $ii; ?>][tw140]"/> <b><?php _e('This Twitter account is still limited to 140 characters', 'social-networks-auto-poster-facebook-twitter-g'); ?></b>
   </div></div>
   <?php
        }
        ...
        public function setNTSettings($post, $options)
        {
            foreach ($post as $ii => $pval) {
                if (!empty($pval['appKey']) && !empty($pval['appKey'])) {
                    ...
                    //## Uniqe Items
                    if (isset($pval['twURL'])) {
                        $options[$ii]['twURL'] = trim($pval['twURL']);
                    }
                    if (isset($pval['accessToken'])) {
                        $options[$ii]['accessToken'] = trim($pval['accessToken']);
                    }
                    if (isset($pval['accessTokenSec'])) {
                        $options[$ii]['accessTokenSec'] = trim($pval['accessTokenSec']);
                    }
        
                    if (isset($pval['tw140'])) {
                        $options[$ii]['tw140'] = $pval['tw140'];
                    } else {
                        $options[$ii]['tw140'] = 0;
                    }
                    ...
                    if (isset($pval['attchImg'])) {
                        $options[$ii]['attchImg'] = trim($pval['attchImg']);
                    } else {
    ...
    <div class="nxsPostEd_ElemWrap">
     <div class="nxsPostEd_Elem">       
      ...
      <input value="1" type="checkbox" name="<?php echo $nt; ?>[<?php echo $ii; ?>][attchImg]" class="nxsEdElem" data-ii="<?php echo $ii; ?>" data-nt="<?php echo $nt; ?>" <?php if ((int)$ntOpt['attchImg'] == 1) {
                echo "checked";
            } ?> /><?php _e('Attach Image to the Post', 'social-networks-auto-poster-facebook-twitter-g'); ?>
    </div></div>
        <?php
        ...
        public function adjMetaOpt($optMt, $pMeta)
        {
            ...
            if (!empty($pMeta['attchImg'])) {
                $optMt['attchImg'] = $pMeta['attchImg'];
            } else {
                $optMt['attchImg'] = 0;
            }
            return $optMt;
        ...
        public function adjPreFormatWP(&$options, $postID)
        {
            if (!empty($postID)) {
                ...
                $noRepl = str_ireplace("%HTAGS%", "", $noRepl);
                $noRepl = str_ireplace("%HCATS%", "", $noRepl);
                $noRepl = preg_replace('/%H?C(F|T)-[a-zA-Z0-9_]+%/', '', $noRepl);
                $noRepl = nxs_doSpin($noRepl);
                $twLim = $twLim - nxs_strLen($noRepl);
                if (stripos($noRepl, 'http')!==false) {
                    $twLim = $twLim - 5;
                }
                ...
                if (stripos($twMsgFormat, '%TITLE%')!==false || stripos($twMsgFormat, '%TEXT%')!==false || stripos($twMsgFormat, '%EXCERPT%')!==false || stripos($twMsgFormat, '%ANNOUNCE%')!==false || stripos($twMsgFormat, '%RAWEXCERPT%')!==false || stripos($twMsgFormat, '%FULLTEXT%')!==false || stripos($twMsgFormat, '%RAWTEXT%')!==false) {
                    $whatToleave = 45;
                } else {
                    $whatToleave = 0;
                }
                if (stripos($twMsgFormat, '%XTAGS%')!==false || stripos($twMsgFormat, '%HTAGS%')!==false) {
                    ...
                    $tagsExclFrmHT = $gOptions['tagsExclFrmHT'];
                    ...
                    $tggs = array();
                    foreach ($t as $tagA) {
                        $frmTag = trim(str_replace(' ', $htS, preg_replace('/xC2xA0/', $htS, nxs_clean_string(trim(ucwords(str_ireplace('&', '', str_ireplace('&amp;', '', $tagA->name))))))));
                   ...
                   ((stripos($twMsgFormat, '%RAWTEXT%')!==false) && preg_match('/\b'.$frmTag.'\b/i', $pRawText))) {
                    ...
                    if (count($tggs)<2) {
                        ...
                        $tags = array_shift($tggs); //## Always keep the first.
            $tags2 = implode(' ', $tggs); //## other elemets
            $tgsTwLim = $twLim-$whatToleave;
                        $tags2 = trim(nsTrnc(' '.$tags2.' ', $tgsTwLim, " ", ""));
                        $tags .= !empty($tags2)?(' '.$tags2):'';
                    }
          
          
                    $twMsgFormat = str_ireplace("%XTAGS%", $tags, $twMsgFormat);
                ...
                if (stripos($twMsgFormat, '%XCATS%')!==false || stripos($twMsgFormat, '%HCATS%')!==false) {
                   ...
                   ((stripos($twMsgFormat, '%RAWTEXT%')!==false) && (stripos($pRawText, $cat->name)!==false || stripos($pRawText, $frmTag)!==false))) {
                    ...
                    if (count($ctts)<2) {
                        ...
                        $cats = array_shift($ctts); //## Always keep the first.
            $cats2 = implode(' ', $ctts); //## other elemets
            $tgsTwLim = $twLim-$whatToleave;
                        $cats2 = trim(nsTrnc(' '.$cats2.' ', $tgsTwLim, " ", ""));
                        $cats .= !empty($cats2)?(' '.$cats2):'';
                    }
                    ...
                    $twMsgFormat = str_ireplace("%XCATS%", $cats, $twMsgFormat);
                        ...
                        if (substr($mms, 0, 1)=='-' && stripos($mms, '%')!==false) {
                            ...
                            if (is_nxs_error($cfItem)) {
                                ...
                                foreach ($cfItem as $frmTag) {
                                    ...
                                    $frmTag =  trim(str_replace(' ', $htS, nxs_clean_string(trim(ucwords(str_ireplace('&', '', str_ireplace('&amp;', '', $frmTag)))))));
                                    $tggs[] = '#'.$frmTag;
                                }
                                $cfItem = implode(' ', $tggs);
        ...
        public function adjPublishWP(&$options, &$message, $postID)
        {
            ...
            if (!empty($options['attchImg']) && $options['attchImg']=='1') {
                if (!empty($options['imgToUse'])) {
                    ...
                    $imgURL = nxs_getPostImage($postID, !empty($options['wpImgSize'])?$options['wpImgSize']:'large');
                }
        ...
        public function importComments($options='', $postID='', $po='')
        {
            if (empty($postID)) {
                $postID = $_POST['pid'];
            }
            ...
            if ($code=='200' && isset($tmhOAuth->response['response'])) {
                $twList = json_decode($tmhOAuth->response['response'], true);
            } else {
                $twList = '';
            }
            ...
            $lastID = '';
            //## Do Replies
            if (!empty($twList) && is_array($twList)) {
