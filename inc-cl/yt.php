...
if (!class_exists("nxs_snapClassYT")) {
    class nxs_snapClassYT extends nxs_snapClassNT
    {
        public $ntInfo = array('code'=>'YT', 'lcode'=>'yt', 'name'=>'YouTube', 'defNName'=>'uName', 'tstReq' => false, 'imgAct'=>'E', 'instrURL'=>'https://www.nextscripts.com/instructions/youtube-social-networks-auto-poster-wordpress-setup-installation/');
        public $noFuncMsg = 'Sorry, but YouTube doesn\'t have a built-in API for automated posts yet. <br/>You need a special API library module to be able to publish your content to YouTube.';
      ...
      break;
        ...
        {
            $defO = array('nName'=>'', 'do'=>'1', 'uName'=>'', 'uPass'=>'', 'ytPageID'=>'', 'ytGPPageID'=>'', 'msgFormat'=>"New post: %TITLE% - %URL%");
        ...
        public function accTab($ii, $options, $isNew=false)
        {
            $ntInfo = $this->ntInfo;
            $nt = $ntInfo['lcode'];
            $this->elemUserPass($ii, $options['uName'], $options['uPass']); ?> <br/ >    
        ...
        public function setNTSettings($post, $options)
        {
            foreach ($post as $ii => $pval) {
                if (!empty($pval['uPass']) && !empty($pval['uPass'])) {
                    ...
                    if (isset($pval['ytPageID'])) {
                        $options[$ii]['ytPageID'] = trim($pval['ytPageID']);
                    }
...
if (!function_exists("nxs_doPublishToYT")) {
