...
$nxs_snapAvNts[] = array(
    ...
    'ptype' => 'F',
    'status' => 'A',
    'desc' => 'Post text, image or share a link to your profile or group page'
);
...
if (!class_exists("nxs_snapClassVK")) {
    class nxs_snapClassVK extends nxs_snapClassNT
    {
        ...
        function toLatestVer($ntOpts)
        {
            ...
            switch ($v) {
                case 340:
                    $ntOptsOut             = $this->toLatestVerNTGen($ntOpts);
                    $ntOptsOut['do']       = $ntOpts['do' . $this->ntInfo['code']];
                    $ntOptsOut['nName']    = $ntOpts['nName'];
                    $ntOptsOut['url']      = $ntOpts['url'];
                    $ntOptsOut['pgID']     = $ntOpts['vkPgID'];
                    $ntOptsOut['pgIntID']  = $ntOpts['pgIntID'];
                    $ntOptsOut['postType'] = $ntOpts['postType'];

                    $ntOptsOut['appID']        = $ntOpts['vkAppID'];
                    $ntOptsOut['appAuthToken'] = $ntOpts['vkAppAuthToken'];
                    $ntOptsOut['appAuthUser']  = $ntOpts['vkAppAuthUser'];
                    $ntOptsOut['authResp']     = $ntOpts['apVKAuthResp'];
        ...
        //#### Show Unit  Settings
        function checkIfSetupFinished($options)
        {
            return !empty($options['appAuthUser']);
        }
        function accTab($ii, $options, $isNew = false)
        {
            $ntInfo = $this->ntInfo;
            $nt     = $ntInfo['lcode'];
?>
     <div style="width:100%;"><strong><?php
            echo $ntInfo['name'];
?>&nbsp;<?php
            _e('URL', 'social-networks-auto-poster-facebook-twitter-g');
?>:</strong> </div>
     <div style="font-size: 11px; margin: 0px;"><?php
            _e('Could be your VK.Com Profile, Public or Group Page', 'social-networks-auto-poster-facebook-twitter-g');
?></div>
     <input name="vk[<?php
            echo $ii;
?>][url]" id="apurl" style="width: 50%;" value="<?php
            _e(apply_filters('format_to_edit', htmlentities($options['url'], ENT_COMPAT, "UTF-8")), 'social-networks-auto-poster-facebook-twitter-g');
?>" />
     <br/><br/>
     <div style="width:100%;"><strong><?php
            echo $ntInfo['name'];
?>&nbsp;<?php
            _e('Application ID', 'social-networks-auto-poster-facebook-twitter-g');
?>:</strong> <a href="https://vk.com/apps?act=manage" target="_blank"><?php
            _e('[VK Apps]', 'social-networks-auto-poster-facebook-twitter-g');
?></a> </div>
    <input name="vk[<?php
            echo $ii;
?>][appID]" style="width: 30%;" value="<?php
            _e(apply_filters('format_to_edit', htmlentities($options['appID'], ENT_COMPAT, "UTF-8")), 'social-networks-auto-poster-facebook-twitter-g');
?>" />
    <br/>
    <?php
            if ($options['appID'] != '') {
?>
    <div style="width:100%;"><strong><?php
                echo $ntInfo['name'];
?>&nbsp;<?php
                _e('Auth Response', 'social-networks-auto-poster-facebook-twitter-g');
?>:</strong> </div><input name="vk[<?php
                echo $ii;
?>][authResp]" style="width: 50%;" value="<?php
                _e(apply_filters('format_to_edit', htmlentities($options['authResp'], ENT_COMPAT, "UTF-8")), 'social-networks-auto-poster-facebook-twitter-g');
?>" /><br/><br/>
    <?php
            }
?>
...
?>
            <?php
                _e('<b>Authorize Your vKontakte(VK) Account</b>. Please click "Update Settings" to be able to Authorize your account.', 'social-networks-auto-poster-facebook-twitter-g');
?>
            <?php
            } else {
                if (isset($options['appAuthUser']) && $options['appAuthUser'] > 0) {
?>
            <?php
                    _e('Your vKontakte(VK) Account has been authorized.');
?> User ID: <?php
                    _e(apply_filters('format_to_edit', htmlentities($options['appAuthUser'], ENT_COMPAT, "UTF-8")), 'social-networks-auto-poster-facebook-twitter-g');
?>.
            <?php
                    _e('You can', 'social-networks-auto-poster-facebook-twitter-g');
?> Re- <?php
                }
?>
            <a target="_blank" href="https://oauth.vk.com/authorize?client_id=<?php
                echo $options['appID'];
?>&scope=offline,wall,photos,pages&redirect_uri=https://oauth.vk.com/blank.html&display=page&v=5.42&response_type=token<?php
                '&auth=vk&acc=' . $ii;
?>">Authorize Your vKontakte(VK) Account</a>
            <?php
                if (!isset($options['appAuthUser']) || $options['appAuthUser'] < 1) {
?> <div class="blnkg">&lt;=== <?php
                    _e('Authorize your account', 'social-networks-auto-poster-facebook-twitter-g');
?> ===</div> <?php
                }
?>
            <?php
...
?><br/><br/> <?php
            $this->elemMsgFormat($ii, 'Message Text Format', 'msgFormat', $options['msgFormat']);
?>
...
?>VKX');" onmouseover="showPopShAtt('<?php
            echo $ii;
?>VKX', event);" onclick="return false;" class="underdash" href="https://www.nextscripts.com/blog/"><?php
            _e('What\'s the difference?', 'social-networks-auto-poster-facebook-twitter-g');
?></a>) </div>
...
?>][postType]" value="A" <?php
            if (empty($options['postType']) || $options['postType'] == 'A')
                echo 'checked="checked"';
?> /> <span><?php
            _e('Text Post with "attached" link', 'social-networks-auto-poster-facebook-twitter-g');
?></span><br/>
...
?>VKX"><h3><?php
            echo $ntInfo['name'];
?>&nbsp;<?php
            _e('Post Types', 'social-networks-auto-poster-facebook-twitter-g');
?></h3><img src="<?php
            echo NXS_PLURL;
?>img/vkPostTypesDiff6.png" width="600" height="257"/></div>
   <?php
        }
        ...
        function setNTSettings($post, $options)
        {
            foreach ($post as $ii => $pval) {
                if (!empty($pval['appID'])) {
                    ...
                    if (isset($pval['appID']))
                        $options[$ii]['appID'] = trim($pval['appID']);
                    //## Get VK Info
                    if (isset($pval['url'])) {
                        $options[$ii]['url'] = trim($pval['url']);
                        if (substr($options[$ii]['url'], 0, 4) !== 'http')
                            $options[$ii]['url'] = 'https://' . $options[$ii]['url'];
                        ...
                        $options[$ii]['pgID'] = $vkPgID; //echo $vkPgID;
                        if (strpos($options[$ii]['url'], '?') !== false)
                            $options[$ii]['url'] = substr($options[$ii]['url'], 0, strpos($options[$ii]['url'], '?')); // prr($pval); prr($options[$ii]); // die();
                    }
                    if (isset($pval['authResp'])) {
                        $options[$ii]['authResp']     = trim($pval['authResp']);
                        ...
                        if (!empty($pval['authResp'])) {
                            $hdrsArr  = nxs_getNXSHeaders($pval['url']);
                            $advSet   = nxs_mkRemOptsArr($hdrsArr);
                            $response = nxs_remote_get($pval['url'], $advSet); //prr($response);
                            if (is_nxs_error($response)) {
                                echo "ERROR: <br/>";
                                prr($response);
                                nxsLogIt(array(
                                    'type' => 'E',
                                    'msg' => 'VK Auth Validation Error:',
                                    'extInfo' => print_r($response, true)
                                ));
                                return $options;
                            }
...
?>
       <tr style="<?php
                echo !empty($ntOpt['do']) ? 'display:table-row;' : 'display:none;';
?>" class="nxstbldo nxstbldo<?php
                echo strtoupper($nt) . $ii;
?>"><th scope="row" style="text-align:right; width:150px; vertical-align:top; padding-top: 0px; padding-right:10px;"> <?php
...
?>VKX');" onmouseover="showPopShAtt('<?php
                echo $ii;
?>VKX', event);" onclick="return false;" class="underdash" href="#"><?php
                _e('What\'s the difference?', 'social-networks-auto-poster-facebook-twitter-g');
?></a>)</th><td>
          <input class="nxs_postEditCtrl"  type="radio" name="vk[<?php
                echo $ii;
?>][postType]" value="T" <?php
                if ($postType == 'T')
                    echo 'checked="checked"';
...
?>"/></div>
        ...
        function showEdPostNTSettingsV4($ntOpt, $post)
        {
            $post_id = $post->ID;
            $nt      = $this->ntInfo['lcode'];
            $ntU     = $this->ntInfo['code'];
...
?></a>)
     ...
     <div class="nxsPostEd_Elem">
        <input type="radio" name="vk[<?php
            echo $ii;
?>][postType]" value="T" class="nxsEdElem" data-ii="<?php
            echo $ii;
?>" data-nt="<?php
            echo $nt;
?>" <?php
...
?></i><br/>
          <input type="radio" name="vk[<?php
            echo $ii;
?>][postType]" value="I" class="nxsEdElem" data-ii="<?php
            echo $ii;
?>" data-nt="<?php
            echo $nt;
?>" <?php
...
?></i><br/>
          <input type="radio" name="vk[<?php
            echo $ii;
?>][postType]" value="A" class="nxsEdElem" data-ii="<?php
            echo $ii;
?>" data-nt="<?php
            echo $nt;
?>" <?php
