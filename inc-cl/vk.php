...
if (!class_exists("nxs_snapClassVK")) {
    ...
    {
        public $ntInfo = array('code'=>'VK', 'lcode'=>'vk', 'name'=>'VK.Com', 'defNName'=>'uName', 'tstReq' => false, 'instrURL'=>'https://www.nextscripts.com/setup-installation-vkontakte-social-networks-auto-poster-wordpress/');
      ...
      break;
        ...
        {
            $defO = array('nName'=>'', 'do'=>'1', 'uName'=>'', 'url'=>'', 'pgID'=>'', 'pgIntID'=>'', 'postType'=>'A', 'appID'=>'', 'appAuthToken'=>'','appAuthUser'=>'','authResp'=>'', 'msgFormat'=>"%EXCERPT%");
    ...
    <?php  if ($options['appID']=='') {
            ...
            <?php if (!isset($options['appAuthUser']) || $options['appAuthUser']<1) {
                    ?> <div class="blnkg">&lt;=== <?php _e('Authorize your account', 'social-networks-auto-poster-facebook-twitter-g'); ?> ===</div> <?php
                } ?>        
            <?php
            } ?><br/><br/> <?php $this->elemMsgFormat($ii, 'Message Text Format', 'msgFormat', $options['msgFormat']); ?>
...
<div style="margin-left: 10px;">
        ...
        <input type="radio" name="vk[<?php echo $ii; ?>][postType]" value="T" <?php if ($options['postType'] == 'T') {
        ...
        <input type="radio" name="vk[<?php echo $ii; ?>][postType]" value="I" <?php if ($options['postType'] == 'I') {
                echo 'checked="checked"';
        ...
        <input type="radio" name="vk[<?php echo $ii; ?>][postType]" value="A" <?php if (empty($options['postType']) || $options['postType'] == 'A') {
                echo 'checked="checked"';
            } ?> /> <span><?php _e('Text Post with "attached" link', 'social-networks-auto-poster-facebook-twitter-g'); ?></span><br/>
   ...
   <?php
                    ...
                    if (isset($pval['authResp'])) {
                        ...
                        if (!empty($pval['authResp'])) {
                            ...
                            if (is_nxs_error($response)) {
                                echo "ERROR: <br/>";
                                prr($response);
                                nxsLogIt(array('type'=>'E', 'msg'=>'VK Auth Validation Error:', 'extInfo'=>print_r($response, true)));
                                return $options;
                            }
       ...
       <tr style="<?php echo !empty($ntOpt['do'])?'display:table-row;':'display:none;'; ?>" class="nxstbldo nxstbldo<?php echo strtoupper($nt).$ii; ?>"><th scope="row" style="text-align:right; width:150px; vertical-align:top; padding-top: 0px; padding-right:10px;"> <?php _e('Post Type:', 'social-networks-auto-poster-facebook-twitter-g') ?> <br/>
          ...
          <input class="nxs_postEditCtrl"  type="radio" name="vk[<?php echo $ii; ?>][postType]" value="T" <?php if ($postType == 'T') {
                    echo 'checked="checked"';
                } ?> /> <?php _e('Text Post', 'social-networks-auto-poster-facebook-twitter-g') ?> - <i><?php _e('just text message', 'social-networks-auto-poster-facebook-twitter-g') ?></i><br/>       
          <input class="nxs_postEditCtrl"  type="radio" name="vk[<?php echo $ii; ?>][postType]" value="I" <?php if ($postType == 'I') {
                    echo 'checked="checked"';
                } ?> /> <?php _e('Image Post', 'social-networks-auto-poster-facebook-twitter-g') ?> - <i><?php _e('big image with text message', 'social-networks-auto-poster-facebook-twitter-g') ?></i><br/>
          <input class="nxs_postEditCtrl"  type="radio" name="vk[<?php echo $ii; ?>][postType]" value="A" <?php if (!isset($postType) || $postType == '' || $postType == 'A') {
                    echo 'checked="checked"';
     ...
     <div class="nxsPostEd_Elem">   
        <input type="radio" name="vk[<?php echo $ii; ?>][postType]" value="T" class="nxsEdElem" data-ii="<?php echo $ii; ?>" data-nt="<?php echo $nt; ?>" <?php if ($postType == 'T') {
                echo 'checked="checked"';
            } ?> /> <?php _e('Text Post', 'social-networks-auto-poster-facebook-twitter-g') ?> - <i><?php _e('just text message', 'social-networks-auto-poster-facebook-twitter-g') ?></i><br/>       
          <input type="radio" name="vk[<?php echo $ii; ?>][postType]" value="I" class="nxsEdElem" data-ii="<?php echo $ii; ?>" data-nt="<?php echo $nt; ?>" <?php if ($postType == 'I') {
                echo 'checked="checked"';
            } ?> /> <?php _e('Image Post', 'social-networks-auto-poster-facebook-twitter-g') ?> - <i><?php _e('big image with text message', 'social-networks-auto-poster-facebook-twitter-g') ?></i><br/>
          <input type="radio" name="vk[<?php echo $ii; ?>][postType]" value="A" class="nxsEdElem" data-ii="<?php echo $ii; ?>" data-nt="<?php echo $nt; ?>" <?php if (!isset($postType) || $postType == '' || $postType == 'A') {
                echo 'checked="checked"';
            } ?> /> <?php _e('Text Post with "attached" blogpost', 'social-networks-auto-poster-facebook-twitter-g') ?>
