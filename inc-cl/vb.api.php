...
if (!class_exists('nxsAPI_VB')) {
    class nxsAPI_VB
    {
        public $ck = array();
        public $ver = 4;
        public $debug = false;
        public function check($u, $url)
        {
            if ($this->debug) {
                echo '[VB] Checking '.$u.";<br/>\r\n";
            }
            $ck = $this->ck;
            if (!empty($ck) && is_array($ck)) {
                $hdrsArr = nxs_getNXSHeaders($url);
                $advSet = nxs_mkRemOptsArr($hdrsArr, $ck);
                $response = nxs_remote_get($url, $advSet);
                if (is_nxs_error($response)) {
                    if ($this->debug) {
                        echo "Invalid Connection. ".print_r($response, true);
                    }
                    return false;
                }
                if (stripos($response['body'], 'logouthash=')===false) {
                    if ($this->debug) {
                        echo "[VB] Bad Saved Login;<br/>\r\n";
                    }
                    return false;
                }
                if (stripos($response['body'], 'usercp.php')!==false && stripos($response['body'], 'logouthash')!==false) { /*echo "You are IN"; */ return true;
                } else {
                    if ($this->debug) {
                        echo "[VB] No Login;<br/>\r\n";
                    }
        ...
        public function connect($u, $p, $url)
        {
            $badOut = 'Error: ';
            //## Check if alrady IN
            if (!$this->check($u, $url)) {
                if ($this->debug) {
                    echo "[VB] NO Saved Data;<br/>\r\n";
                }
                $hdrsArr = nxs_getNXSHeaders($url);
                $mids = '';
                $ck = $this->ck;
                ...
                if ($mdhashLoc===false) {
                    if (stripos($contents, '"idLoginIframe"')!==false) {
                        $lUrl = CutFromTo($contents, '"idLoginIframe"', '</iframe>');
                        $lUrl = CutFromTo($lUrl, 'src="', '"');
                        $response = nxs_remote_get($lUrl, $advSet);
                        $contents = $response['body'];
                        $mdhashLoc = stripos($contents, 'id="idLoginForm"');
                        if ($mdhashLoc===false) {
                            return "L2. No supported vBulletin script found at ".$lUrl;
                        }
                        $frmTxt = CutFromTo($contents, 'id="idLoginForm"', '</form>');
                        $this->ver = 5;
                    } else {
                        return "No supported vBulletin script found at ".$url;
                    }
                } else {
                ...
                $hdrsArr = nxs_getNXSHeaders($url, true);
                $hdrsArr['Upgrade-Insecure-Requests'] = 1;
                $advSet = nxs_mkRemOptsArr($hdrsArr, $ck, $flds);
                $r2 = nxs_remote_post($baseURL.($this->ver==5?'auth/login':'login.php?do=login'), $advSet); // prr($advSet); prr( $baseURL.($this->ver==5?'auth/login':'login.php?do=login') );
                if (stripos($r2['body'], 'exec_refresh()')!==false || stripos($r2['body'], 'url = url.replace')!==false) {
                    $ck = nxsMergeArraysOV($ck, $r2['cookies']);
                    $this->ck = $ck;
                    return false;
                } else {
                ...
                if ($this->debug) {
                    echo "[VB] Saved Data is OK;<br/>\r\n";
                }
                return false;
            }
        }
        public function post($url, $subj, $msg, $lnk, $tags)
        {
            $hdrsArr = nxs_getNXSHeaders($url);
            $ck = $this->ck;
            $mids='';
            ...
            if (stripos($contents, '"nodeid": "')!==false) {
                $node = CutFromTo($contents, '"nodeid": "', '"');
                $this->ver = 5;
                if (stripos($contents, 'new-conversation-btn')!==false) {
                    $mdd='t';
                } elseif (stripos($contents, 'create-content/text/')!==false) {
                    $mdd='p';
                } else {
                    return "No Thread/Post Controls found";
                }
            } elseif (stripos($contents, 'newthread.php?do=newthread')!==false) {
                $mdd='t';
            ...
            if ($mdd=='t') {
                if ($this->ver == 5) {
                    $tURL =  $baseURL.'new-content/'.$node;
                } else {
                    $fid = CutFromTo($contents, 'newthread.php?do=newthread', '"');
                    $tURL =  $baseURL.'newthread.php?do=newthread'.str_replace('&amp;', '&', $fid);
                }
                $response = nxs_remote_get($tURL, $advSet);
                $contents = $response['body'];
                $frmTxt = ($this->ver == 5)?CutFromTo($contents, 'create-content/text/', '</form>'):CutFromTo($contents, 'newthread.php?do=postthread', '</form>');
                $md = array();
            ...
            if ($mdd=='p') {
                if ($this->ver < 5) {
                    ...
                    $response = nxs_remote_get($baseURL.'newreply.php?do=newreply'.str_replace('&amp;', '&', $fid), $advSet);
                    $contents = $response['body'];
                }
                $frmTxt =  ($this->ver == 5)?CutFromTo($contents, 'action="create-content/text/"', '</form>'):CutFromTo($contents, 'newreply.php?do=postreply', '</form>');
                $md = array();
                $flds  = array(); //prr($frmTxt);
                while (stripos($frmTxt, '<input')!==false) {
                    $inpField = trim(CutFromTo($frmTxt, '<input', '>'));
                    $name = trim(CutFromTo($inpField, 'name="', '"'));
                    if ($name!='' && !in_array($name, $md)) {
                        $md[] = $name;
                        $val = trim(CutFromTo($inpField, 'value="', '"'));
                        $flds[$name]= $val;
                        $mids .= "&".$name."=".$val;
            ...
            unset($flds['preview']);
            $advSet = nxs_mkRemOptsArr($hdrsArr, $ck, $flds);
            $r2 = nxs_remote_post($smURL, $advSet);  // prr($r2['response']);  prr(htmlentities($r2['body'])); $r2['body'] = ''; prr($r2); die();
      if (is_nxs_error($r2)) {
          return "Invalid Connection. ".print_r($r2, true);
            ...
            if (stripos($r2['body'], 'tag can only be ')!==false) {
                ...
                $advSet = nxs_mkRemOptsArr($hdrsArr, $ck, $flds);
                $r2 = nxs_remote_post($smURL, $advSet);
            }
            if (is_nxs_error($r2)) {
                return "Invalid Connection. ".print_r($r2, true);
            }
      
            if (stripos($r2['body'], '{"')!==false) {
                $jsRet = json_decode($r2['body'], true);
                if (is_array($jsRet)) {
                    if (empty($jsRet['retUrl']) && !empty($jsRet['nodeId']) && !empty($flds['ret'])) {
                        $jsRet['retUrl'] =  $flds['ret'];
                    }
                    if (!empty($jsRet['retUrl']) && !empty($jsRet['nodeId'])) {
                        return array('postID'=>$jsRet['nodeId'], 'isPosted'=>1, 'postURL'=>$jsRet['retUrl'], 'pDate'=>date('Y-m-d H:i:s'));
                    } else {
                        return "Something wrong - Error: ".print_r($jsRet, true);
                    }
                }
            }
            ...
            if ($r2['response']['code']=='302' || $r2['response']['code']=='303') {
                return array('postID'=>$r2['headers']['location'], 'isPosted'=>1, 'postURL'=>$r2['headers']['location'], 'pDate'=>date('Y-m-d H:i:s'));
            }
            if (stripos($r2['body'], '<newpostid>')!==false || stripos($r2['body'], 'postbit postid="')!==false) {
                return array('postID'=>'', 'isPosted'=>1, 'postURL'=>'', 'pDate'=>date('Y-m-d H:i:s'));
            }
    ...
    class nxs_class_SNAP_VB
    {
        ...
        public function doPostToNT($options, $message)
        {
            ...
            if (function_exists('nxs_getOption')) {
                $opVal = array();
                $opNm = 'nxs_snap_vb_'.sha1('nxs_snap_vb'.$options['uName'].$options['uPass']);
            ...
            $loginErr = $nt->connect($options['uName'], $pass, $options['vbURL']);
            if (!$loginErr) {
                $ret = $nt->post($options['vbURL'], $msgT, $msg, $urlToGo, $message['tags']);
