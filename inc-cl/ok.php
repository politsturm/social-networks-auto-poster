...
if (!class_exists("nxs_snapClassOK")) {
    class nxs_snapClassOK extends nxs_snapClassNT
    {
        public $ntInfo = array('code'=>'OK', 'lcode'=>'ok', 'name'=>'ok.ru', 'defNName'=>'', 'tstReq' => true, 'instrURL'=>'https://www.nextscripts.com/instructions/setup-installation-okru-social-networks-auto-poster/');
        ...
        {
            $defO = array('nName'=>'', 'do'=>'1', 'appKey'=>'', 'appSec'=>'', 'gid'=>'', 'access_token'=>'', 'secret_session_key'=>'', 'postType'=>'A', 'msgFormat'=>"%EXCERPT%\r\n\r\n%URL%", 'imgSize'=>'original');
            $this->showGNewNTSettings($ii, $defO);
        }
        //#### Show Unit  Settings
        public function checkIfSetupFinished($options)
        {
            return !empty($options['gid']) && !empty($options['appKey']);
        }
        ...
        public function accTab($ii, $options, $isNew=false)
        {
            ...
            $ntInfo = $this->ntInfo;
            $nt = $ntInfo['lcode'];
            $this->elemKeySecret($ii, 'App Key', 'App Secret', $options['appKey'], $options['appSec'], 'appKey', 'appSec');
            $this->elemKeySecret($ii, 'access_token', 'secret_session_key', $options['access_token'], $options['secret_session_key'], 'access_token', 'secret_session_key'); ?>
    ...
    <div style="margin-left: 10px;">
      ...
      <input type="radio" name="<?php echo $nt; ?>[<?php echo $ii; ?>][postType]" value="A" <?php if (empty($options['postType']) || $options['postType'] == 'A') {
                echo 'checked="checked"';
    ...
    <br/><br/> <?php
        ...
        public function setNTSettings($post, $options)
        {
            foreach ($post as $ii => $pval) {
                if (!empty($pval['appKey']) && !empty($pval['appKey'])) {
                    ...
                    //## Uniqe Items
                    if (isset($pval['gid'])) {
                        $options[$ii]['gid'] = trim($pval['gid']);
                    }
                    if (isset($pval['access_token'])) {
                        $options[$ii]['access_token'] = trim($pval['access_token']);
                    }
                    if (isset($pval['secret_session_key'])) {
                        $options[$ii]['secret_session_key'] = trim($pval['secret_session_key']);
                    }
        ...
        public function showEdPostNTSettingsV4($ntOpt, $post)
        {
            ...
            $postType = isset($ntOpt['postType'])?$ntOpt['postType']:'T';
            $msgFormat = !empty($ntOpt['msgFormat'])?htmlentities($ntOpt['msgFormat'], ENT_COMPAT, "UTF-8"):'';
            $msgTFormat = !empty($ntOpt['msgTFormat'])?htmlentities($ntOpt['msgTFormat'], ENT_COMPAT, "UTF-8"):'';
            $imgToUse = $ntOpt['imgToUse'];
            $urlToUse = $ntOpt['urlToUse'];
   ...
   <div class="nxsPostEd_ElemWrap">   
     ...
     <div class="nxsPostEd_Elem">   
        ...
        <input type="radio" name="<?php echo $nt; ?>[<?php echo $ii; ?>][postType]" value="A" class="nxsEdElem" data-ii="<?php echo $ii; ?>" data-nt="<?php echo $nt; ?>" <?php if (empty($postType) || $postType == 'A') {
                echo 'checked="checked"';
            } ?> /><?php _e('Text Post with "attached" blogpost', 'social-networks-auto-poster-facebook-twitter-g') ?>
     </div>
   </div><?php
...
if (!function_exists("nxs_doPublishToOK")) {
    function nxs_doPublishToOK($postID, $options)
    {
        if (!is_array($options)) {
            $options = maybe_unserialize(get_post_meta($postID, $options, true));
        }
        $cl = new nxs_snapClassOK();
