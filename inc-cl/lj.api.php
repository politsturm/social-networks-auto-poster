...
if (!class_exists("nxs_class_SNAP_LJ")) {
    class nxs_class_SNAP_LJ
    {
        ...
        public function nxs_getLJHeaders($up)
        {
            $hdrsArr = array();
        ...
        public function doPostToNT($options, $message)
        {
            ...
            $nxsToLJclient = new NXS_XMLRPC_Client('https://www.'.$server.'/interface/xmlrpc');
            ...
            // prr($nxsToLJContent);
            if (!$nxsToLJclient->query('LJ.XMLRPC.postevent', $nxsToLJContent)) { /* prr($nxsToLJclient); */ $ret = 'Something went wrong - '.$nxsToLJclient->getErrorCode().' : '.$nxsToLJclient->getErrorMessage();
            } else {
                $ret = 'OK';
            }
