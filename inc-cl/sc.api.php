...
if (!class_exists("nxs_class_SNAP_SC")) {
    class nxs_class_SNAP_SC
    {
        ...
        public function doPostToNT($options, $message)
        {
            ...
            if (empty($options['accessToken']) && empty($options['uPass'])) {
        ...
        $uname = $options['uName'];
                ...
                $nt->t = $options['topicURL'];//        prr($message);
                if ($loginErr) {
                ...
                if (!empty($message['tags']) && !is_array($message['tags'])) {
                    $message['tags'] = explode(',', $message['tags']);
                }
                $iURL = (($postType=='I' || $postType=='A') && !empty($imgURL))?$imgURL:'';
                $pURL = $postType=='A'?$message['url']:'';
                $html = '';
                $ret = $nt->post($text, $pURL, $iURL, $msgT, $html, $message['tags']);
                return $ret;
            } else {
                require_once('apis/scOAuth.php');
                $tum_oauth = new wpScoopITOAuth(nxs_gak($options['appKey']), nxs_gas($options['appSec']), $options['accessToken'], $options['accessTokenSec']);
