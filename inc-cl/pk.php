...
if (!class_exists("nxs_snapClassPK")) {
    class nxs_snapClassPK extends nxs_snapClassNT
    {
        public $ntInfo = array('code'=>'PK', 'lcode'=>'pk', 'name'=>'Plurk', 'defNName'=>'', 'tstReq' => true, 'instrURL'=>'https://www.nextscripts.com/setup-installation-plurk-social-networks-auto-poster-wordpress/');
        public $defO = array('nName'=>'', 'do'=>'1', 'appKey'=>'', 'appSec'=>'', 'attchImg'=>1, 'msgFormat'=>"%TITLE% - %URL%",  'pkCat'=>'', 'pkURL'=>'');
        //#### Update
      ...
      break;
        ...
        public function doAuth()
        {
            ...
            if (isset($_GET['auth']) && $_GET['auth']=='pk') {
                ...
                $options['oAuthTokenSecret'] = $request_token['oauth_token_secret']; //prr($tum_oauth); prr($options); //die();
            ...
            if (isset($_GET['auth']) && $_GET['auth']=='pka') {
                ...
                if ($options['pgID']!='') {
                    ...
                    die("<span style='color:red;'>ERROR: Authorization Error: <span style='color:darkred; font-weight: bold;'>".$options['pgID']."</span></span>");
                }
            }
        }
    ...
    <div style=""> <div style="width:100%;"><strong id="altFormatText">Plurk prefix:</strong> </div>  
      <select name="pk[<?php echo $ii; ?>][pkCat]" id="pkCat<?php echo $ii; ?>">
        <?php  $pkCats = $this->pkCats();
            if (isset($options['pkCat']) && $options['pkCat']!='') {
                $pkCats = str_replace($options['pkCat'].'"', $options['pkCat'].'" selected="selected"', $pkCats);
            }
            echo $pkCats; ?>
      </select>            
    </div>      
    <div style="margin: 0px;"><input value="1"  id="apLIAttch" type="checkbox" name="pk[<?php echo $ii; ?>][attchImg]"  <?php if ((int)$options['attchImg'] == 1) {
                echo "checked";
            } ?> /> <strong>Attach Image to Plurk Post</strong></div>
    ...
    <?php
            ...
            <?php
                ...
                if (!empty($options['accessToken']) && $options['accessTokenSec']!=='') {
                    ?>
            ...
            <a href="<?php echo $nxs_snapSetPgURL.(stripos($nxs_snapSetPgURL, '?')!==false?'&':'?'); ?>auth=pk&acc=<?php echo $ii; ?>">Authorize Your Plurk Account</a> 
              <?php if (empty($options['oAuthTokenSecret'])) {
        ...
        public function setNTSettings($post, $options)
        {
            foreach ($post as $ii => $pval) {
                if (!empty($pval['appKey']) && !empty($pval['appKey'])) {
                    ...
                    //## Uniqe Items
                    if (isset($pval['pkCat'])) {
                        $options[$ii]['pkCat'] = trim($pval['pkCat']);
                    }
                    ...
                    if (isset($pval['pkURL'])) {
                        $options[$ii]['pkURL'] = trim($pval['pkURL']);
