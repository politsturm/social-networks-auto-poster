...
if (!class_exists("nxs_snapClassRD")) {
    class nxs_snapClassRD extends nxs_snapClassNT
    {
        public $ntInfo = array('code'=>'RD', 'lcode'=>'rd', 'name'=>'Reddit', 'defNName'=>'uName', 'tstReq' => false, 'instrURL'=>'https://www.nextscripts.com/setup-installation-reddit-social-networks-auto-poster-wordpress/');
  
        public $noFuncMsg = 'Sorry, but Reddit doesn\'t have a built-in API for automated posts yet. <br/>You need a special API library module to be able to publish your content to Reddit.';
        public function checkIfFunc()
        {
            return class_exists('nxsAPI_RD');
        }
      ...
      break;
        ...
        {
            $defO = array('nName'=>'', 'do'=>'1', 'uName'=>'', 'uPass'=>'', 'postType'=>'', 'rdSubReddit'=>'', 'msgTFormat'=>'%TITLE%', 'msgFormat'=>"%EXCERPT%");
        ...
        public function accTab($ii, $options, $isNew=false)
        {
            ...
            if (!empty($p)) {
                ...
                $_POST['rdSR'] = $options['rdSubReddit'];
                $_POST['u'] = $options['uName'];
                $_POST['p'] = $p;
                $_POST['ii'] = $ii;
                $opNm = 'nxs_snap_rd_'.sha1('nxs_snap_rd'.$options['uName'].$options['uPass']);
                $opVal = nxs_getOption($opNm);
                if (empty($opVal)) {
                    $ntw[$nt][$ii]=$options;
                    $opVal = $this->getListOfSubReddits($ntw);
                }
    ...
    <div id="nxsRDInfoDiv<?php echo $ii; ?>">
         <div style="width:100%;">
          ...
          <select id="rdSubReddit<?php echo $ii; ?>" onchange="nxs_rdSRChange('<?php echo $ii; ?>',jQuery(this));" name="rd[<?php echo $ii; ?>][rdSubReddit]">
            ...
            if (!empty($options['rdSubReddit'])) {
                $pgi = str_ireplace('selected="selected" ', '', $pgi);
                $pgi = str_ireplace('value="'.$options['rdSubReddit'].'"', 'selected="selected" value="'.$options['rdSubReddit'].'"', $pgi);
            } else {
                echo '<option value="">None(Click refresh icon to retrieve your subreddits)</option>';
            }
            echo $pgi; ?>
          ...
          <div id="nxsRDMsgDiv<?php echo $ii; ?>"><?php if (!empty($options['uMsg'])) {
                echo $options['uMsg'];
            } ?><?php if ($isNew) {
                ?><?php _e('Please enter your login/password to see the list of your subreddits', 'nxs_snap'); ?><?php
            } ?></div>                                                 
    ...
    <div style="width:100%;"><strong id="altFormatText">Post Type:</strong></div>                      
      <div style="margin-left: 10px;">
        <input type="radio" name="rd[<?php echo $ii; ?>][postType]" value="A" <?php if (empty($options['postType']) || $options['postType'] == 'A') {
                echo 'checked="checked"';
            } ?> /> <?php _e('Link Post', 'social-networks-auto-poster-facebook-twitter-g'); ?>
        <br/>
        <input type="radio" name="rd[<?php echo $ii; ?>][postType]" value="T" <?php if (!empty($options['postType']) && $options['postType'] == 'T') {
                echo 'checked="checked"';
            } ?> /> <?php _e('Text Post', 'social-networks-auto-poster-facebook-twitter-g'); ?> - <i><?php _e('set the text format below', 'social-networks-auto-poster-facebook-twitter-g'); ?></i>
     </div>
  ...
  <script type="text/javascript">      
    ...
    <?php $this->elemTitleFormat($ii, 'Post Title Format', 'msgTFormat', $options['msgTFormat']);
        ...
        <input type="radio" name="rd[<?php echo $ii; ?>][postType]" value="A" <?php if (empty($postType) || $postType == 'A') {
                    echo 'checked="checked"';
                } ?> /><?php _e('Link Post', 'social-networks-auto-poster-facebook-twitter-g') ?>
        <br/>
      <input type="radio" name="rd[<?php echo $ii; ?>][postType]" value="T" <?php if ($postType == 'T') {
                    echo 'checked="checked"';
                } ?> /> <?php _e('Text Post', 'social-networks-auto-poster-facebook-twitter-g') ?><br/>               
     </td></tr>
        <?php
   ...
   <div class="nxsPostEd_ElemWrap">   
     ...
     <div class="nxsPostEd_Elem">   
       <input type="radio" name="rd[<?php echo $ii; ?>][postType]" class="nxsEdElem" value="A" data-ii="<?php echo $ii; ?>" data-nt="<?php echo $nt; ?>" <?php if (empty($postType) || $postType == 'A') {
                echo 'checked="checked"';
            } ?> /><?php _e('Link Post', 'social-networks-auto-poster-facebook-twitter-g') ?>
        <br/>
      <input type="radio" name="rd[<?php echo $ii; ?>][postType]" class="nxsEdElem" value="T" data-ii="<?php echo $ii; ?>" data-nt="<?php echo $nt; ?>" <?php if ($postType == 'T') {
                echo 'checked="checked"';
  ...
  public function adjMetaOpt($optMt, $pMeta)
  {
    ...
    return $optMt;
        ...
        //## RD Specific
  public function getListOfSubReddits($networks)
  {
      $opVal = array();
      $pass = 'g9c1a'.nsx_doEncode($_POST['p']);
      $opNm = 'nxs_snap_rd_'.sha1('nxs_snap_rd'.$_POST['u'].$pass);
      $opVal = nxs_getOption($opNm);
      $ii = $_POST['ii'];
      $nt = new nxsAPI_RD();
      $nt->debug = false; // prr($opVal);
     $currPstAs = !empty($_POST['rdSR'])?$_POST['rdSR']:(!empty($networks['rd'][$ii])?$networks['rd'][$ii]['rdSubReddit']:'');
      if (empty($_POST['force']) && !empty($opVal['ck']) && !empty($opVal['rdSubRedditsList'])) {
