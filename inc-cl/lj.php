...
if (!class_exists("nxs_snapClassLJ")) {
    class nxs_snapClassLJ extends nxs_snapClassNT
    {
        public $ntInfo = array('code'=>'LJ', 'lcode'=>'lj', 'name'=>'LiveJournal', 'defNName'=>'uName', 'tstReq' => false, 'instrURL'=>'https://www.nextscripts.com/setup-installation-livejournal-social-networks-auto-poster-for-wordpress/');
      ...
      break;
        ...
        {
            $defO = array('nName'=>'', 'do'=>'1', 'uName'=>'', 'uPass'=>'', 'ljSrv'=>'livejournal.com',  'commID'=>'', 'inclTags'=>'1', 'msgTFormat'=>'%TITLE%', 'msgFormat'=>"%FULLTEXT%");
    ...
    <div id="nxsLJInfoDiv<?php echo $ii; ?>">
     <div style="width:100%;">
      <select id="lj1delayHrs" name="lj[<?php echo $ii; ?>][ljSrv]"><option  <?php if (empty($options['ljSrv']) || $options['ljSrv']=='LJ') {
                ?> selected="selected" <?php
            } ?> value="LJ">LiveJournal.com</option>
         <option <?php if (isset($options['ljSrv']) && $options['ljSrv']=='DW') {
                ?> selected="selected" <?php
            } ?> value="DW">DreamWidth.org</option>
      </select> 
     </div>         
    </div><br/>
    ...
    <?php $this->elemTitleFormat($ii, 'Post Title Format', 'msgTFormat', $options['msgTFormat']);
            $this->elemMsgFormat($ii, 'Post Text Format', 'msgFormat', $options['msgFormat']); ?>    
    <div style="margin-bottom: 20px;margin-top: 5px;"><input value="1"  id="ljInclTags<?php echo $ii; ?>" type="checkbox" name="lj[<?php echo $ii; ?>][inclTags]"  <?php if ((int)$options['inclTags'] == 1) {
                echo "checked";
            } ?> /> 
