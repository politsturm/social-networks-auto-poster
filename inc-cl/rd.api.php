    ...
    class nxs_class_SNAP_RD
    {
        ...
        public function doPostToNT($options, $message)
        {
            ...
            if (!class_exists("nxsAPI_RD")) {
                $badOut['Error'] .= "Reddit API Library not found";
                return $badOut;
            }
            //## Check settings
            if (!is_array($options)) {
            ...
            if (!$loginErr) {
                $ret = $nt->post($text, $title, $options['rdSubReddit'], $options['postType']=='A'?$message['url']:'');
                //## Save Login Info
                if (function_exists('nxs_saveOption')) {
                    if (empty($opVal['ck'])) {
                        $opVal['ck'] = '';
                    }
                    if (is_array($ret) && !empty($ret['isPosted']) && $ret['isPosted']=='1' && $opVal['ck'] != $nt->ck) {
                        $opVal['ck'] = $nt->ck;
