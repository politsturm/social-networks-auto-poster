    ...
    class nxs_class_SNAP_FP
    {
        ...
        public function doPostToNT($options, $message)
        {
            ...
            if (!class_exists("nxsAPI_FP")) {
                $badOut['Error'] .= "Flipboard API Library not found";
                return $badOut;
            }
            //## Check settings
            if (!is_array($options)) {
            ...
            if (function_exists('nxs_getOption')) {
                $opVal = array();
                $opNm = 'nxs_snap_fp_'.sha1('nxs_snap_fp'.$options['uName'].$options['uPass']);
            ...
            if (!empty($message['session']) || !empty($options['session'])) {
                ...
                $nt->cuid = !empty($message['cuid'])?$message['cuid']:$options['cuid'];
            }
