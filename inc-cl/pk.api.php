...
if (!class_exists("nxs_class_SNAP_PK")) {
    class nxs_class_SNAP_PK
    {
        ...
        public function doPostToNT($options, $message)
        {
            ...
            if (!isset($options['appKey']) || trim($options['appSec'])=='' || empty($options['accessToken'])) {
            ...
            $postinfo = $tum_oauth->makeReq('https://www.plurk.com/APP/Timeline/plurkAdd', $postArr); //  prr($postinfo);
      ...
      if ($code == 200 && $pkID!='') {
          ...
          $link = 'https://www.plurk.com/p/'.$shorten;
