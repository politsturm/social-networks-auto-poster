...
if (!class_exists("nxs_snapClassWB")) {
    class nxs_snapClassWB extends nxs_snapClassNT
    {
        public $ntInfo = array('code'=>'WB', 'lcode'=>'wb', 'name'=>'weibo', 'defNName'=>'', 'tstReq' => true, 'instrURL'=>'https://www.nextscripts.com/instructions/setup-installation-weibo-social-networks-auto-poster/');
        ...
        {
            $defO = array('nName'=>'', 'do'=>'1', 'appKey'=>'', 'appSec'=>'', 'inclTags'=>'1', 'cat'=>0, 'gal'=>'', 'attchImg'=>1, 'msgFormat'=>"%EXCERPT%\r\n\r\n%URL%", 'imgSize'=>'original');
            $this->showGNewNTSettings($ii, $defO);
        }
        //#### Show Unit  Settings
        public function checkIfSetupFinished($options)
        {
            return !empty($options['appAppUserID']) && !empty($options['accessToken']);
        }
      ...
      */
            ...
            if (isset($_GET['code']) && isset($_GET['state']) && stripos($_GET['state'], $ntInfo['lcode'].'a-')!==false) {
                $ii = explode('-', $_GET['state']);
                $ii = $ii[1];
                $options = $this->nt[$ii];
                $appkey = nxs_gak($options['appKey']);
                $appSecret = nxs_gas($options['appSec']);
                $url = 'https://api.weibo.com/oauth2/access_token?client_id='.$appkey.'&client_secret='.$appSecret.'&grant_type=authorization_code&redirect_uri='.urlencode($nxs_snapSetPgURL).'&code='.$_GET['code'];
                $rep = nxs_remote_post($url);
                $cont = json_decode($rep['body'], true);
                if (empty($cont) || empty($cont['access_token'])) {
                    prr($cont);
                    prr($rep);
                    die();
                }
                $options['accessToken'] = $cont['access_token'];
                $options['appAppUserID'] = $cont['uid'];
                $options['appAppUserName'] = $cont['uid'];
                nxs_save_glbNtwrks($ntInfo['lcode'], $ii, $options, '*');  //prr($options); die();
        ...
        public function accTab($ii, $options, $isNew=false)
        {
            ...
            $ntInfo = $this->ntInfo;
            $nt = $ntInfo['lcode'];
            $this->elemKeySecret($ii, 'App Key', 'App Secret', $options['appKey'], $options['appSec'], 'appKey', 'appSec', 'http://open.weibo.com/development'); ?>
   ...
   <div class="nxsPostEd_ElemWrap">        
     <div class="nxsPostEd_Elem">   
       ...
       <input value="1" type="checkbox" name="<?php echo $nt; ?>[<?php echo $ii; ?>][attchImg]" class="nxsEdElem" data-ii="<?php echo $ii; ?>" data-nt="<?php echo $nt; ?>" <?php if ((int)$ntOpt['attchImg'] == 1) {
           echo "checked";
       } ?> /> <strong><?php _e('Attach Image to the Post', 'social-networks-auto-poster-facebook-twitter-g'); ?></strong>
     </div>
   </div><?php
       // ## Select Image & URL
...
if (!function_exists("nxs_doPublishToWB")) {
    function nxs_doPublishToWB($postID, $options)
    {
        if (!is_array($options)) {
            $options = maybe_unserialize(get_post_meta($postID, $options, true));
        }
        $cl = new nxs_snapClassWB();
