...
if (!class_exists("nxs_class_SNAP_GP")) {
    class nxs_class_SNAP_GP
    {
        ...
        public function doPostToNT($options, $message)
        {
            ...
            $opVal = array();
            $opNm = md5('nxs_snap_gp'.$options['uName'].$options['uPass']);
      ...
      if ($gpPostType=='I' || $gpPostType=='A') {
          if (isset($message['imageURL'])) {
            ...
            if (!empty($options['sid'])) {
                $nt->session = array('ssid'=>$options['ssid'], 'sid'=>$options['sid'], 'hsid'=>$options['hsid']);
            }
            if (!empty($options['ck'])) {
                $nt->ck = $options['ck'];
            }
            ...
            if (!$loginError) {
                ...
                $pageID = (!empty($options['postAs']) && $options['postAs']!='p')?$options['postAs']:'';
                $postTo = (!empty($options['postTo']) && $options['postTo']!='p')?$options['postTo']:'';
                if (!empty($postTo)) {
                    if (strlen($postTo)<10) {
                        $comPgID = $postTo;  /*Maybe get name */
                    } elseif (substr($postTo, 0, 1)=='c') {
                        $comPgID = substr($postTo, 1);
                        $comPGCatID = $options['commCat'];
                    }
