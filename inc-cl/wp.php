...
if (!class_exists("nxs_snapClassWP")) {
    class nxs_snapClassWP extends nxs_snapClassNT
    {
        public $ntInfo = array('code'=>'WP', 'lcode'=>'wp', 'name'=>'WP Based Blog', 'defNName'=>'uName', 'tstReq' => false, 'instrURL'=>'https://www.nextscripts.com/setup-installation-wp-based-social-networks-auto-poster-wordpress/');
      ...
      break;
        ...
        {
            $defO = array('nName'=>'', 'do'=>'1', 'uName'=>'', 'uPass'=>'', 'wpURL'=>'', 'inclCats'=>'1', 'inclTags'=>'1',  'msgTFormat'=>'%TITLE%', 'msgFormat'=>"%RAWTEXT%");
        ...
        public function accTab($ii, $options, $isNew=false)
        {
            $ntInfo = $this->ntInfo;
            $nt = $ntInfo['lcode'];
            if (!isset($options['inclTags'])) {
                $options['inclTags'] = 1;
            }
            if (!isset($options['inclCats'])) {
                $options['inclCats'] = 1;
            } ?> 
    ...
    <?php $this->elemUserPass($ii, $options['uName'], $options['uPass']);
            echo "<br/>";
            $this->elemTitleFormat($ii, 'Post Title Format', 'msgTFormat', $options['msgTFormat']);
            $this->elemMsgFormat($ii, 'Post Text Format', 'msgFormat', $options['msgFormat']); ?><br/ >
    <div style="margin-bottom: 20px;margin-top: 5px;"><input value="1" type="checkbox" name="<?php echo $nt; ?>[<?php echo $ii; ?>][inclTags]"  <?php if (!empty($options['inclTags'])) {
                echo "checked";
            } ?> /> 
    ...
    <div style="margin-bottom: 20px;margin-top: 5px;"><input value="1" type="checkbox" name="<?php echo $nt; ?>[<?php echo $ii; ?>][inclCats]"  <?php if (!empty($options['inclCats'])) {
                echo "checked";
            } ?> /> 
