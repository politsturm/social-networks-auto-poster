...
if (isset($_GET['pg']) && $_GET['pg']=='nxs' && isset($_GET['ca']) && $_GET['ca']!='') {
    $ch = curl_init();
...
if (!function_exists("nxs_ntp_time")) {
...
if (!class_exists("nxs_snapClassLI")) {
    class nxs_snapClassLI extends nxs_snapClassNT
    {
        public $ntInfo = array('code'=>'LI', 'lcode'=>'li', 'name'=>'LinkedIn', 'defNName'=>'', 'tstReq' => true, 'instrURL'=>'https://www.nextscripts.com/setup-installation-linkedin-social-networks-auto-poster-wordpress/');
        public $defO = array('nName'=>'', 'do'=>'1', 'pgID'=>'', 'pgcID'=>'', 'appKey'=>'', 'appSec'=>'', 'uName'=>'', 'uPass'=>'',  'uPage'=>'', 'inclTags'=>1, 'msgFormat'=>"New post (%TITLE%) has been published on %SITENAME%", 'msgTFormat'=>"%TITLE%", 'msgCTFormat'=>"%TITLE%", 'msgCFormat'=>"%RAWTEXT%", 'msgATFormat'=>"", 'msgAFormat'=>"",  'imgSize'=>'original');
        //#### Update
      ...
      case 340: $ntOptsOut = $this->toLatestVerNTGen($ntOpts); $ntOptsOut['do'] = $ntOpts['do'.$this->ntInfo['code']]; $ntOptsOut['nName'] = $ntOpts['nName'];
        if (!empty($ntOpts['liAPIKey'])) {
            $ntOpts['apiToUse'] = (!empty($ntOpts['isV2']))?'liv2':'liv1';
        } elseif (!empty($ntOpts['ulName']) && !empty($ntOpts['uPass'])) {
            $ntOpts['apiToUse'] = 'nx';
        }  $ntOptsOut['apiToUse'] = $ntOpts['apiToUse'];
        if ($ntOptsOut['apiToUse']=='nx') {
            $ntOptsOut['uName'] = $ntOpts['ulName'];
            $ntOptsOut['uPass'] = $ntOpts['uPass'];
        } else {
            $ntOptsOut['appKey'] = $ntOpts['liAPIKey'];
            $ntOptsOut['appSec'] = $ntOpts['liAPISec'];
            $ntOptsOut['oAuthVerifier'] =  !empty($ntOpts['liOAuthVerifier'])?$ntOpts['liOAuthVerifier']:'';
            $ntOptsOut['accessToken'] = !empty($ntOpts['liAccessToken'])?$ntOpts['liAccessToken']:'';
            $ntOptsOut['accessTokenSec'] = !empty($ntOpts['liAccessTokenSecret'])?$ntOpts['liAccessTokenSecret']:'';
            $ntOptsOut['oAuthToken'] =  !empty($ntOpts['liOAuthToken'])?$ntOpts['liOAuthToken']:'';
            $ntOptsOut['oAuthTokenSecret'] = !empty($ntOpts['liOAuthTokenSecret'])?$ntOpts['liOAuthTokenSecret']:'';
            $ntOptsOut['accessTokenExp'] = !empty($ntOpts['liAccessTokenExp'])?$ntOpts['liAccessTokenExp']:'';
            $ntOptsOut['liUserID'] = !empty($ntOpts['liUserID'])?$ntOpts['liUserID']:'';
            $ntOptsOut['liUserInfo'] = !empty($ntOpts['liUserInfo'])?$ntOpts['liUserInfo']:'';
        ...
        if (substr($ntOpts['uPage'], 0, 4)=='http') {
            if (stripos($ntOpts['uPage'], 'groups')!==false && stripos($ntOpts['uPage'], 'gid=')!==false) {
                $lid = CutFromTo($ntOpts['uPage'].'&', 'gid=', '&');
            } else {
                $lid = $ntOpts['uPage'];
                if (strpos($lid, '?')!==false) {
                    $lid = substr($lid, 0, strpos($lid, '?'));
                }
                if (substr($lid, -1)=='/') {
                    $lid = substr($lid, 0, -1);
                }
                $lid = substr(strrchr($lid, "/"), 1);
            }
        }  $ntOptsOut['pgcID'] = ''; $ntOptsOut['pggID'] = ''; if (!empty($lid)) {
            if (stripos($ntOpts['uPage'], 'groups')!==false) {
                $ntOptsOut['whToPost'] = 'G';
                $ntOptsOut['pggID'] = $lid;
            } else {
                $ntOptsOut['whToPost'] = 'C';
                $ntOptsOut['pgcID'] = $lid;
            }
        }
      ...
      break;
        ...
        {
            return !empty($options['accessToken']) || !empty($options['uPass']);
        }
        public function doAuth()
        {
            ...
            if (isset($_GET['code']) && $_GET['code']!='' && isset($_GET['state']) && substr($_GET['state'], 0, 7) == 'nxs-li-') {
                ...
                unset($gGet['activated']);
                unset($gGet['stylesheet']);
                ...
                if (isset($nto['appKey'])) {
                    ...
                    $response  = nxs_remote_post($tknURL, $wprg);
                    prr($tknURL);
                    ...
                    $tknURL = 'https://api.linkedin.com/v1/people/~:(id,first-name,last-name)?format=json&oauth2_access_token='.$nto['accessToken'];
                    $response = nxs_remote_get($tknURL, nxs_mkRemOptsArr(nxs_getNXSHeaders()));
                    prr($tknURL);
                    prr($response);
                    ...
                    if (!empty($user['id'])) {
                        ...
                        prr($nto['liUserInfo'], 'Authorized user');
                        $gURL = 'https://api.linkedin.com/v1/companies?format=json&is-company-admin=true&oauth2_access_token='.$nto['accessToken'];
                        $response = nxs_remote_get($gURL, nxs_mkRemOptsArr(nxs_getNXSHeaders()));
                        prr($response);
                        $userPages = json_decode($response['body'], true);
                        prr($userPages, 'USER PAGES:');
                        $pgs = '';
                        if (!empty($userPages['values'])) {
                            foreach ($userPages['values'] as $up) {
                                $pgs .= '<option '.($up['id']==$nto['pgID'] ? 'selected="selected"':'').' value="'.$up['id'].'">'.$up['name'].' ('.$up['id'].')</option>';
                            }
                        }
                        $opVal = array();
                        $opNm = 'nxs_snap_li_'.sha1('nxs_snap_li'.$nto['liUserID'].nxs_gak($nto['appKey']));
                        $opVal['pgList'] = $pgs;
            ...
            if (isset($_GET['auth']) && $_GET['auth']=='li') {
                ...
                $ii = $_GET['acc'];
                $api_key = nxs_gak($options['appKey']);
                $api_secret = nxs_gas($options['appSec']);
                ...
                $options['oAuthToken'] = $request_token->key;
                $options['oAuthTokenSecret'] = $request_token->secret;
                prr($li_oauth);
          ...
          default: echo '<br/><b style="color:red">Could not connect to LinkedIn. Refresh the page or try again later.</b>'; die('</div></div>');
            ...
            if (isset($_GET['auth']) && $_GET['auth']=='lia') {
                ...
                $li_oauth->request_token = new nsx_trOAuthConsumer($options['oAuthToken'], $options['oAuthTokenSecret'], 1);
                $li_oauth->oauth_verifier = $_REQUEST['oauth_verifier'];
                $li_oauth->getAccessToken($_REQUEST['oauth_verifier']);
                $options['oAuthVerifier'] = $_REQUEST['oauth_verifier'];
                $options['accessToken'] = $li_oauth->access_token->key;
                $options['accessTokenSec'] = $li_oauth->access_token->secret;
                try {
                    $xml_response = $li_oauth->getProfile("~:(id,first-name,last-name)");
                } catch (Exception $o) {
                    prr($o);
                    die("<span style='color:red;'>ERROR: Authorization Error</span></div></div>");
                }
                ...
                if ($userinfo!='') {
                    ...
                    echo '<div style="text-align:center;color:green; font-weight: bold; font-size:20px;" >ALL OK. You have been authorized. Refreshing page....</div><script type="text/javascript">setTimeout(function(){ window.location = "'.$nxs_snapSetPgURL.'"; }, 3000);</script>';
                    die('</div></div>');
                }
                prr($xml_response);
                die("<span style='color:red;'>ERROR: Something is Wrong with your LinkedIn account</span></div></div>");
            }
        }
  
        public function getListOfPagesLIV2($networks)
        {
            $opVal = array();
            if (empty($_POST['u'])) {
                return $opVal;
            }
            $opNm = 'nxs_snap_li_'.sha1('nxs_snap_li'.$_POST['u'].$_POST['p']);
            $opVal = nxs_getOption($opNm);
            $ii = $_POST['ii'];
            if (empty($networks['li'][$ii]['accessToken'])) {
                return $opVal;
            }
            $currPstAs = !empty($_POST['pgID'])?$_POST['pgID']:(!empty($networks['li'][$ii])?$networks['li'][$ii]['pgID']:'');
            if (empty($_POST['force']) && !empty($opVal['pgList'])) {
                $pgs = $opVal['pgList'];
            } else {
                $options = $networks['li'][$ii];
                $gURL = 'https://api.linkedin.com/v1/companies?format=json&is-company-admin=true&count=100&oauth2_access_token='.$options['accessToken'];
                $response = nxs_remote_get($gURL, nxs_mkRemOptsArr(nxs_getNXSHeaders()));
                $userPages = json_decode($response['body'], true);
                $pgs = '';
                if (!empty($userPages['values'])) {
                    foreach ($userPages['values'] as $up) {
                        $pgs .= '<option '.($up['id']==$options['pgID'] ? 'selected="selected"':'').' value="'.$up['id'].'">'.$up['name'].' ('.$up['id'].')</option>';
                    }
                }
            }
            ...
            if (!empty($_POST['isOut'])) {
                echo $pgCust.'<option '.($options['pgID']=='p'?'selected="selected" ':'').'value="p">'.__('Profile').'</option>'.$pgs.'<option style="color:#BD5200" value="a">'.__('...enter the Page ID').'</option>';
            }
            $opVal['pgList'] = $pgs;
            nxs_saveOption($opNm, $opVal);
            return $opVal;
        ...
        public function getListOfPagesNXS($networks)
        {
            $opVal = array();
            $pass = 'g9c1a'.nsx_doEncode($_POST['p']);
            $opNm = 'nxs_snap_li_'.sha1('nxs_snap_li'.$_POST['u'].$pass);
            $opVal = nxs_getOption($opNm);
            $ii = $_POST['ii'];
            $nt = new nxsAPI_LI(); // prr($opVal);
            $currPstAs = !empty($_POST['pgcID'])?$_POST['pgcID']:(!empty($networks['li'][$ii])?$networks['li'][$ii]['pgcID']:'');
            $options = $networks['li'][$ii];
            if (empty($_POST['force']) && !empty($opVal['ck']) && !empty($opVal['pgsList'])) {
                $pgs = $opVal['pgsList'];
            } else {
                if (!empty($opVal['ck'])) {
                    $nt->ck = $opVal['ck'];
                ...
                if (!empty($message['session']) || !empty($options['session'])) {
                    $sid = !empty($message['session'])?$message['session']:$options['session'];
                    if (empty($nt->ck)) {
                        $nt->ck = array();
                    }
                    foreach ($nt->ck as $ci=>$cc) {
                        if ($nt->ck[$ci]->name=='li_at') {
                            unset($nt->ck[$ci]);
                        }
                    }
                    $c = new NXS_Http_Cookie(array('name' => 'li_at', 'value' => $sid));
                    $nt->ck[] = $c;
                }
                $loginError=$nt->connect($_POST['u'], $_POST['p']);
                if (!$loginError) {
                    $opVal['ck'] = $nt->ck;
                    $pgs = $nt->getPgsList($currPstAs);
                } else {
                    $outMsg = '<b style="color:red;">'.__('Login Problem').'&nbsp;-&nbsp;'.strip_tags($loginError).'</b>';
                    if (!empty($_POST['isOut'])) {
                        echo $outMsg;
                    }
                    return $outMsg;
            ...
            $pgCust = (!empty($pgs) && !empty($currPstAs) && stripos($pgs, $currPstAs)===false)?'<option selected="selected" value="'.$currPstAs.'">'.$currPstAs.'</option>':'';
            if (!empty($_POST['isOut'])) {
                echo $pgCust.$pgs.'<option style="color:#BD5200" value="a">'.__('...enter the Company Page ID').'</option>';
            }
            $opVal['pgsList'] = $pgs;
            nxs_saveOption($opNm, $opVal);
            return $opVal;
        }
        public function getListOfGroupsNXS($networks)
        {
            $opVal = array();
            $pass = 'g9c1a'.nsx_doEncode($_POST['p']);
            $opNm = 'nxs_snap_li_'.sha1('nxs_snap_li'.$_POST['u'].$pass);
            ...
            $options = (!empty($networks['li'][$ii]))?$networks['li'][$ii]:array();
            if (empty($_POST['force']) && !empty($opVal['ck']) && !empty($opVal['grpList'])) {
                $pgs = $opVal['grpList'];
            } else {
                if (!empty($opVal['ck'])) {
                    $nt->ck = $opVal['ck'];
            ...
            $pgCust = (!empty($pgs) && !empty($currPstAs) && stripos($pgs, $currPstAs)===false)?'<option selected="selected" value="'.$currPstAs.'">'.$currPstAs.'</option>':'';
            if (!empty($_POST['isOut'])) {
                echo $pgCust.$pgs.'<option style="color:#BD5200" value="a">'.__('...enter the Group ID').'</option>';
            }
            $opVal['grpList'] = $pgs;
            nxs_saveOption($opNm, $opVal);
            return $opVal;
    ...
    <div id="nxs_<?php echo $nt; ?>_apiliv1div_<?php echo $ii; ?>" class="nxs_<?php echo $nt; ?>_apidiv_<?php echo $ii; ?> nxs_<?php echo $nt; ?>_apiliv1div_<?php echo $ii; ?>" style="display: <?php echo (!empty($options['apiToUse']) && $options['apiToUse'] =='liv1')?"block":"none"; ?>;"><h3>LinkedIn API V1</h3>    
      ...
      <?php  if ($options['appKey']=='') {
        ...
        <b><?php _e('Authorize Your '.$ntInfo['name'].' Account', 'social-networks-auto-poster-facebook-twitter-g'); ?></b> <?php _e('Please click "Update Settings" to be able to Authorize your account.', 'social-networks-auto-poster-facebook-twitter-g');
                ...
                if (!empty($options['accessToken']) && !empty($options['accessTokenSec'])) {
    ...
    <div id="nxs_<?php echo $nt; ?>_apiliv2div_<?php echo $ii; ?>" class="nxs_<?php echo $nt; ?>_apidiv_<?php echo $ii; ?> nxs_<?php echo $nt; ?>_apiliv2div_<?php echo $ii; ?>" style="display: <?php echo (empty($options['apiToUse']) || $options['apiToUse'] =='liv2')?"block":"none"; ?>;"><h3>LinkedIn API V2</h3>    
      ...
      <?php if (empty($options['liUserID'])) {
                $options['liUserID'] = '';
            } //## List of Pages
            $opNm = 'nxs_snap_li_'.sha1('nxs_snap_li'.$options['liUserID'].nxs_gak($options['appKey']));
            $opVal = nxs_getOption($opNm);
            if (empty($opVal)) {
                $tPST = (!empty($_POST))?$_POST:'';
                $_POST['pgID'] = $options['pgID'];
                $_POST['u'] = $options['liUserID'];
                $_POST['p'] = nxs_gak($options['appKey']);
                $_POST['ii'] = $ii;
                $ntw[$nt][$ii]=$options;
                $opVal = $this->getListOfPagesLIV2($ntw);
                $_POST = $tPST;
            }
    ...
    <div id="nxsLIInfoDiv<?php echo $ii; ?>" style="<?php echo empty($options['accessToken'])?'display:none;':''; ?>">
         <div style="width:100%;">
          ...
          <select id="lipgID<?php echo $ii; ?>" onchange="nxs_liPageChange('<?php echo $ii; ?>',jQuery(this));" name="li[<?php echo $ii; ?>][pgID]">
            ...
            echo '<option '.($options['pgID']=='p'?'selected="selected" ':'').'value="p">'.__('Profile').'</option>';
          ...
          </div> <div id="nxsLIMsgDiv<?php echo $ii; ?>"><?php if (!empty($options['uMsg'])) {
                echo $options['uMsg'];
            } ?><?php if ($isNew) {
                ?><?php _e('Please authorize your account', 'nxs_snap'); ?><?php
            } ?></div>                                                                                                    
    ...
    <?php if (class_exists('nxsAPI_LI')) {
                if (!empty($options['uPass'])&&!empty($options['uName'])) {
                    $opNm = 'nxs_snap_li_'.sha1('nxs_snap_li'.$options['uName'].$options['uPass']);
                    $opVal = nxs_getOption($opNm);
                    if (empty($opVal)) {
                        $tPST=(!empty($_POST))?$_POST:'';
                        if (empty($_POST)) {
                        ...
                        if (!empty($options['uPass'])) {
                            ...
                            $_POST['ii']=$ii;
                            $ntw[$nt][$ii]=$options;
                            $opVal = $this->getListOfPagesNXS($ntw);
                            $opVal = $this->getListOfGroupsNXS($ntw);
                        }
                        $_POST = $tPST;
                    }
                    if (!empty($opVal) & !is_array($opVal)) {
    ...
    <div style="width:100%;"><b style="font-size: 15px;"><?php _e('Where to Post', 'social-networks-auto-poster-facebook-twitter-g');
                if (empty($options['whToPost'])) {
                    $options['whToPost'] = 'PR';
                } ?>:</b> </div>
      <div style="margin-left: 10px;">        
        <input class="liWhereToPost<?php echo $ii; ?>" type="radio" name="li[<?php echo $ii; ?>][whToPost]" value="PR" <?php if ($options['whToPost'] == 'PR') {
                    echo 'checked="checked"';
                } ?> /> <?php _e('Profile Update', 'social-networks-auto-poster-facebook-twitter-g'); ?> - <i><?php _e('Post to your profile', 'social-networks-auto-poster-facebook-twitter-g'); ?></i><br/>
        <input class="liWhereToPost<?php echo $ii; ?>" type="radio" name="li[<?php echo $ii; ?>][whToPost]" value="C" <?php if ($options['whToPost'] == 'C') {
                    echo 'checked="checked"';
                } ?> /> <?php _e('Company Page', 'social-networks-auto-poster-facebook-twitter-g'); ?> - <i><?php _e('Post to Company page', 'social-networks-auto-poster-facebook-twitter-g'); ?></i><br/> 
         ...
         <div style="width:100%;">
          ...
          <select id="li2pgID<?php echo $ii; ?>" onchange="nxs_li2PageChange('<?php echo $ii; ?>',jQuery(this));" name="li[<?php echo $ii; ?>][pgcID]">
            <?php $pgi = !empty($options['pgsList'])?$options['pgsList']:'';
                if (!empty($options['pgcID'])) {
                    echo (!empty($options['pgcID']) && stripos($pgi, $options['pgcID'])===false)?'<option selected="selected" value="'.$options['pgcID'].'">'.$options['pgcID'].'</option>':'';
                }
                if (!empty($options['pgcID'])) {
                    $pgi = str_ireplace('selected="selected" ', '', $pgi);
                    $pgi = str_ireplace('value="'.$options['pgcID'].'"', 'selected="selected" value="'.$options['pgcID'].'"', $pgi);
                    $pgi = str_ireplace('data-val="'.$options['pgcID'].'"', 'selected="selected" data-val="'.$options['pgcID'].'"', $pgi);
                }
                echo $pgi; ?><option value="a"><?php _e('.... Enter the Company Page ID'); ?></option>
          </select>
          ...
          </div> <div id="nxsLI2MsgDiv<?php echo $ii; ?>"><?php if (!empty($options['uMsg'])) {
                    echo $options['uMsg'];
                } ?><?php if ($isNew) {
                    ?><i style="color: #800080"><?php _e('Please Enter your username and password to select your page', 'nxs_snap'); ?><?php
                } ?></i></div>                                                                          
        </div> 
        <input class="liWhereToPost<?php echo $ii; ?>" type="radio" name="li[<?php echo $ii; ?>][whToPost]" value="G" <?php if ($options['whToPost'] == 'G') {
                    echo 'checked="checked"';
                } ?> /> <?php _e('Group', 'social-networks-auto-poster-facebook-twitter-g'); ?> - <i><?php _e('Post to LinkedIn Group', 'social-networks-auto-poster-facebook-twitter-g'); ?></i><br/> 
         ...
         <div style="width:100%;">
          ...
          <select id="li2GpgID<?php echo $ii; ?>" onchange="nxs_li2GPageChange('<?php echo $ii; ?>',jQuery(this));" name="li[<?php echo $ii; ?>][pggID]">
            <?php $pgi = !empty($options['grpList'])?$options['grpList']:'';
                if (!empty($options['pggID'])) {
                    echo (!empty($options['pggID']) && stripos($pgi, $options['pggID'])===false)?'<option selected="selected" value="'.$options['pggID'].'">'.$options['pggID'].'</option>':'';
                }
                if (!empty($options['pggID'])) {
                    $pgi = str_ireplace('selected="selected" ', '', $pgi);
                    $pgi = str_ireplace('value="'.$options['pggID'].'"', 'selected="selected" value="'.$options['pggID'].'"', $pgi);
                    $pgi = str_ireplace('data-val="'.$options['pggID'].'"', 'selected="selected" data-val="'.$options['pggID'].'"', $pgi);
                }
                echo $pgi; ?><option value="a"><?php _e('.... Enter the Group ID'); ?></option>
          </select>
        ...
        <input class="liWhereToPost<?php echo $ii; ?>" type="radio" name="li[<?php echo $ii; ?>][whToPost]" value="P" <?php if ($options['whToPost'] == 'P') {
                    echo 'checked="checked"';
                } ?> /> <?php _e('Pulse Article', 'social-networks-auto-poster-facebook-twitter-g'); ?> - <i><?php _e('Rich text post article shared to the "Pulse" section', 'social-networks-auto-poster-facebook-twitter-g'); ?></i><br/>
    ...
    <?php
                ...
                nxs_show_noLibWrn('"NextScripts API Library for LinkedIN" is NOT installed');
            } ?>           
    </div><br/>
    ...
    <div style="width:100%;"><strong id="altFormatText">Post Type:</strong> </div>                      
            <div style="margin-left: 10px;">
        <?php if (empty($options['postType'])) {
                if ((!empty($options['liAttch']) && (int)$options['liAttch'] == 1) || $isNew) {
                    $options['postType'] = 'A';
                }
            } ?>
        <input class="liPostType<?php echo $ii; ?>" type="radio" name="li[<?php echo $ii; ?>][postType]" value="T" <?php if ($options['postType'] == 'T') {
                echo 'checked="checked"';
        ...
        <input class="liPostType<?php echo $ii; ?>" type="radio" name="li[<?php echo $ii; ?>][postType]" value="I" <?php if ($options['postType'] == 'I') {
                echo 'checked="checked"';
            } ?> /> <?php _e('Image Post', 'social-networks-auto-poster-facebook-twitter-g'); ?> - <i><?php _e('big image with text message (Profiles and Company pages only)', 'social-networks-auto-poster-facebook-twitter-g'); ?></i><br/>  </span> 
        <input class="liPostType<?php echo $ii; ?>" type="radio" name="li[<?php echo $ii; ?>][postType]" value="A" <?php if (empty($options['postType']) || $options['postType'] == 'A') {
                echo 'checked="checked"';
            } ?> /> <?php _e('Add blogpost to LinkedIn message as an attachment', 'social-networks-auto-poster-facebook-twitter-g'); ?><br/>
        
       
        
      <div style="margin-left: 10px;">            
        <strong><?php _e('Attachment Text Format', 'social-networks-auto-poster-facebook-twitter-g'); ?>:</strong><br/> 
      <input value="1"  id="apLIMsgAFrmtA<?php echo $ii; ?>" <?php if (empty($options['msgAFormat']) || trim($options['msgAFormat'])=='') {
                echo "checked";
            } ?> onchange="if (jQuery(this).is(':checked')) { jQuery('#apLIMsgAFrmtDiv<?php echo $ii; ?>').hide(); jQuery('#apLIMsgAFrmt<?php echo $ii; ?>').val(''); }else jQuery('#apLIMsgAFrmtDiv<?php echo $ii; ?>').show();" type="checkbox" name="li[<?php echo $ii; ?>][msgAFormatCB]"/> <strong><?php _e('Auto', 'social-networks-auto-poster-facebook-twitter-g'); ?></strong>
      ...
      <div id="apLIMsgAFrmtDiv<?php echo $ii; ?>" style="<?php  if (empty($options['msgAFormat']) || trim($options['msgAFormat'])=='') {
                echo "display:none;";
            } ?>" >
      <?php $this->elemTitleFormat($ii, 'Title Format', 'msgATFormat', $options['msgATFormat']);
            $this->elemMsgFormat($ii, 'Message Format', 'msgAFormat', $options['msgAFormat']); ?>
  ...
  {
      $otp = array(); //prr($options);
    foreach ($options as $oo => $v) {
        if (isset($v['ck'])) {
            unset($v['ck']);
        }
        if (isset($oo) && $oo!=='' && ((!empty($v['appKey']) && !empty($v['appSec'])) || (!empty($v['appKey2']) && !empty($v['appSec2'])) || (!empty($v['uPass']) && !empty($v['uName'])))) {
            $otp[$oo] = $v;
        }
    }
    ...
    foreach ($post as $ii => $pval) { //  prr($ii, 'II');    prr($pval);
      if ((!empty($pval['appKey']) && !empty($pval['appSec'])) || (!empty($pval['appKey2']) && !empty($pval['appSec2'])) || (!empty($pval['uPass']) && !empty($pval['uName']))) {
          ...
          if (isset($pval['msgCFormat'])) {
              $options[$ii]['msgCFormat'] = trim($pval['msgCFormat']);
          }
          if (isset($pval['msgCTFormat'])) {
              $options[$ii]['msgCTFormat'] = trim($pval['msgCTFormat']);
          }
          if ($options[$ii]['apiToUse']=='liv2') {
              if (isset($pval['appKey2'])) {
                  $options[$ii]['appKey'] = trim($pval['appKey2']);
              }
              if (isset($pval['appSec2'])) {
                  $options[$ii]['appSec'] = trim($pval['appSec2']);
              }
          }
          if (isset($pval['whToPost'])) {
              $options[$ii]['whToPost'] = trim($pval['whToPost']);
          }
          if (isset($pval['pgcID'])) {
              $options[$ii]['pgcID'] = trim($pval['pgcID']);
          }
          if (isset($pval['pggID'])) {
              $options[$ii]['pggID'] = trim($pval['pggID']);
          }
          if (isset($pval['pgID'])) {
      ...
      return $options;
        ...
        public function showEdPostNTSettings($ntOpts, $post)
        {
            ...
            foreach ($ntOpts as $ii=>$ntOpt) {
                ...
                if ($ntOpt['whToPost']!='P') {
                    if (!empty($ntOpt['apiToUse']) && $ntOpt['apiToUse'] !='nx' && !empty($ntOpt['postType']) && $ntOpt['postType'] == 'I') {
                        $ntOpt['postType'] = 'A';
                    } ?>
        ...
        <input type="radio" name="li[<?php echo $ii; ?>][postType]" value="T" <?php if (!empty($ntOpt['postType']) && $ntOpt['postType'] == 'T') {
                        echo 'checked="checked"';
                    } ?> /><?php _e('Text Post', 'social-networks-auto-poster-facebook-twitter-g') ?>  - <i><?php _e('just text message', 'social-networks-auto-poster-facebook-twitter-g') ?></i><br/>       
        <span class="nxs_li_nxapi_<?php echo $ii; ?>" style="display: <?php echo (!empty($ntOpt['apiToUse']) && $ntOpt['apiToUse'] =='nx')?"block":"none"; ?>;">
        <input type="radio" name="li[<?php echo $ii; ?>][postType]" value="I" <?php if (!empty($ntOpt['postType']) && $ntOpt['postType'] == 'I') {
                        echo 'checked="checked"';
                    } ?> onchange="jQuery('#altFormatIMG<?php echo $nt.$ii; ?>').show();" /> <?php _e('Post to LinkedIn as "Image post"', 'social-networks-auto-poster-facebook-twitter-g') ?> - <i><?php _e('big image with text message', 'social-networks-auto-poster-facebook-twitter-g') ?></i><br/> </span>            
        <input type="radio" name="li[<?php echo $ii; ?>][postType]" value="A" <?php if (empty($ntOpt['postType']) || $ntOpt['postType'] == 'A') {
                        echo 'checked="checked"';
                    } ?> onchange="jQuery('#altFormatIMG<?php echo $nt.$ii; ?>').hide();" /><?php _e('Text Post with "attached" blogpost', 'social-networks-auto-poster-facebook-twitter-g') ?>        
     </td></tr>          <?php
                } else {
  ...
  <div class="nxsPostEd_ElemWrap">   
     ...
     <div class="nxsPostEd_Elem">   
         <input type="radio" name="li[<?php echo $ii; ?>][postType]" value="T" class="nxsEdElem nxsImgCtrlCb" data-nt="<?php echo $nt; ?>" data-ii="<?php echo $ii; ?>" <?php if (!empty($ntOpt['postType']) && $ntOpt['postType'] == 'T') {
                    echo 'checked="checked"';
                } ?> /><?php _e('Text Post', 'social-networks-auto-poster-facebook-twitter-g') ?>  - <i><?php _e('just text message', 'social-networks-auto-poster-facebook-twitter-g') ?></i><br/>       
        <span class="nxs_li_nxapi_<?php echo $ii; ?>" style="display: <?php echo (!empty($ntOpt['apiToUse']) && $ntOpt['apiToUse'] =='nx')?"block":"none"; ?>;">
        <input type="radio" name="li[<?php echo $ii; ?>][postType]" value="I" class="nxsEdElem nxsImgCtrlCb" data-nt="<?php echo $nt; ?>" data-ii="<?php echo $ii; ?>" <?php if (!empty($ntOpt['postType']) && $ntOpt['postType'] == 'I') {
                    echo 'checked="checked"';
                } ?> onchange="jQuery('#altFormatIMG<?php echo $nt.$ii; ?>').show();" /> <?php _e('Post to LinkedIn as "Image post"', 'social-networks-auto-poster-facebook-twitter-g') ?> - <i><?php _e('big image with text message', 'social-networks-auto-poster-facebook-twitter-g') ?></i><br/> </span>            
        <input type="radio" name="li[<?php echo $ii; ?>][postType]" value="A" class="nxsEdElem nxsImgCtrlCb" data-nt="<?php echo $nt; ?>" data-ii="<?php echo $ii; ?>" <?php if (empty($ntOpt['postType']) || $ntOpt['postType'] == 'A') {
                    echo 'checked="checked"';
                } ?> onchange="jQuery('#altFormatIMG<?php echo $nt.$ii; ?>').hide();" /><?php _e('Text Post with "attached" blogpost', 'social-networks-auto-poster-facebook-twitter-g') ?> 
     </div>     
   </div>  <?php
        ...
        { //prr($message); prr($options);
            if (!empty($postID)) {
                ...
                if (!empty($options['msgCFormat'])) {
                    $options['msgCFormat'] = nsFormatMessage($options['msgCFormat'], $postID, $addParams);
                }
                if (!empty($options['msgCTFormat'])) {
                    $options['msgCTFormat'] = nsFormatMessage($options['msgCTFormat'], $postID, $addParams);
                }
                if ($options['postType']=='A') {
                    $lng = '';
                    //## AUTO - Get Post Descr from SEO Plugins or make it.
                    if (!empty($options['msgAFormat'])) {
                        $dsc = nsFormatMessage($options['msgAFormat'], $postID, $addParams);
                        $urlTitle = (!empty($options['msgATFormat']))?nsFormatMessage($options['msgATFormat'], $postID, $addParams): nxs_doQTrans($post->post_title, $lng);
                    } else {
                        if (function_exists('aioseop_mrt_fix_meta') && empty($dsc)) {
                            $dsc = trim(get_post_meta($postID, '_aioseop_description', true));
                        }
                        if (function_exists('wpseo_admin_init') && empty($dsc)) {
                    ...
                    $dsc = nsTrnc($dsc, 900, ' ');
                    $message['urlDescr'] = $dsc;
                    if (!empty($urlTitle)) {
                        $message['urlTitle'] = strip_tags(strip_shortcodes($urlTitle));
                    }
                }
            }
        }
        ...
        public function nxsCptCheck()
        {
            ...
            if (!empty($_POST['c'])) {
                ...
                if ($rep['response']['code']=='302' && !empty($rep['headers']['location']) &&  (stripos($rep['headers']['location'], 'linkedin.com/nhome')!==false || stripos($rep['headers']['location'], 'linkedin.com/home')!==false)) {
                    ...
                    if ($_POST['i']!='') {
                        ...
                        if (is_array($options)) {
                            update_option('NS_SNAutoPoster', $options, false);
                        }
                    }
                }
            }
            if (!empty($_POST['s'])) {
                ...
                $advSet = nxs_mkRemOptsArr($hdrsArr, $ck, $flds, $liObj->proxy);
                $rep=nxs_remote_post('https://www.linkedin.com'.$fa, $advSet);
                if (is_nxs_error($rep)) {
                    $badOut = print_r($rep, true)." - ERROR";
                    return $badOut;
                }
                ...
                $advSet = nxs_mkRemOptsArr($hdrsArr, $ck, '', $liObj->proxy);
                $rep = nxs_remote_get('http://www.linkedin.com/profile/edit?trk=tab_pro', $advSet);
                if (is_nxs_error($rep)) {
                    $badOut = print_r($rep, true)." - ERROR";
                    return $badOut;
                ...
                if ($_POST['i']!='') {
                    ...
                    $options = $nxs_SNAP->nxs_options;
                    $nto = $options['li'][$_POST['i']];
                    $opVal = array();
                    $opNm = 'nxs_snap_li_'.sha1('nxs_snap_li'.$nto['uName'].$nto['uPass']);
                    $opVal['ck'] = $ck;
                    nxs_saveOption($opNm, $opVal);
                }
            }
