...
if (!class_exists("nxs_snapClassTR")) {
    class nxs_snapClassTR extends nxs_snapClassNT
    {
        public $ntInfo = array('code'=>'TR', 'lcode'=>'tr', 'name'=>'Tumblr', 'defNName'=>'', 'tstReq' => true, 'instrURL'=>'https://www.nextscripts.com/instructions/tumblr-auto-poster-setup-installation/');
        //#### Update
      ...
      case 340: $ntOptsOut = $this->toLatestVerNTGen($ntOpts); $ntOptsOut['do'] = $ntOpts['do'.$this->ntInfo['code']]; $ntOptsOut['nName'] = $ntOpts['nName']; // prr($ntOpts);
        if (!empty($ntOpts['trMsgFormat'])) {
            $ntOptsOut['msgFormat'] = $ntOpts['trMsgFormat'];
        } if (!empty($ntOpts['trMsgTFormat'])) {
            $ntOptsOut['msgTFormat'] = $ntOpts['trMsgTFormat'];
        }
        if (!empty($ntOpts['trConsKey'])) {
            $ntOptsOut['appKey'] = $ntOpts['trConsKey'];
        }  if (!empty($ntOpts['trConsSec'])) {
            $ntOptsOut['appSec'] = $ntOpts['trConsSec'];
        }
      ...
      break;
        ...
        {
            $defO = array('nName'=>'', 'do'=>'1', 'appKey'=>'', 'appSec'=>'',  'fillSrcURL'=>'1', 'useOrDate'=>'1', 'inclTags'=>'1', 'inclCats'=>'', 'cImgURL'=>'R', 'defImg'=>"", 'msgTFormat'=>"%TITLE%", 'msgFormat'=>"<p>New Post has been published on %URL%</p>\r\n<blockquote><p><strong>%TITLE%</strong></p>\r\n<p><img src=\"%IMG%\"/></p><p>%FULLTEXT%</p></blockquote>",  'trURL'=>'', 'postDate'=>'', 'imgSize'=>'', 'postType'=>'');
            $this->showGNewNTSettings($ii, $defO);
        }
        //#### Show Unit  Settings
        public function checkIfSetupFinished($options)
        {
            return !empty($options['pgID']) && !empty($options['accessToken']);
        }
        ...
        default: echo '<br/><b style="color:red">Could not connect to Tumblr. Refresh the page or try again later.</b>'; die('</div></div>');
            ...
            if (isset($_GET['auth']) && $_GET['auth']=='tra') {
                ...
                $options['accessToken'] = $tum_oauth->getAccessToken($_REQUEST['oauth_verifier']);
                prr($tum_oauth, '**tum_oauth ==1== **');
                prr($options['accessToken'], '**GOT ACCESS TOKEN **');
                $options['accessTokenSec'] =  $options['accessToken']['oauth_token_secret'];
                $options['accessToken'] =  $options['accessToken']['oauth_token'];
                ...
                prr($tum_oauth, '**tum_oauth ==2== **');
                prr($userinfo, '**USERINFO**'); // prr($url); die();
                if ($userinfo->meta->status=='401') {
                    die("<span style='color:red;'>ERROR #1: Authorized USER don't have access to the specified blog: <span style='color:darkred; font-weight: bold;'>".$options['pgID']."</span></span></div>");
                }
                if (is_array($userinfo->response->user->blogs)) {
                    $options['authUser'] = $userinfo->response->user->name;
                    $blogs = '';
                    $opNm = 'nxs_snap_tr_'.sha1('nxs_snap_tr'.$options['authUser'].nxs_gak($options['appKey']));
                    foreach ($userinfo->response->user->blogs as $blog) {
                        if (!empty($blog->uuid)) {
                            $uuid = $blog->uuid;
                        } else {
                            $uuid = rtrim(str_ireplace('http://', '', str_ireplace('https://', '', $blog->url)), '/');
                        }
                ...
                prr($userinfo);
                die("<span style='color:red;'>ERROR #2: Authorized USER don't have access to the specified blog: <span style='color:darkred; font-weight: bold;'>".$options['pgID']."</span></span></div></div>");
            }
        }
  
        public function getListOfBlogs($networks)
        {
            $opVal = array();
            $opNm = 'nxs_snap_tr_'.sha1('nxs_snap_tr'.$_POST['u'].$_POST['p']);
            $opVal = nxs_getOption($opNm);
            $ii = $_POST['ii'];
            $currPstAs = !empty($_POST['cBlog'])?$_POST['cBlog']:(!empty($networks['tr'][$ii])?$networks['tr'][$ii]['pgID']:'');
            if (empty($_POST['force']) && !empty($opVal['blogList'])) {
                $pgs = $opVal['blogList'];
            } else {
                $options = $networks['tr'][$ii];
                require_once('apis/trOAuth.php');
                $tum_oauth = new TumblrOAuth(nxs_gak($options['appKey']), nxs_gas($options['appSec']), $options['accessToken'], $options['accessTokenSec']);
                $userinfo = $tum_oauth->get('http://api.tumblr.com/v2/user/info');// prr($userinfo);
                if ($userinfo->meta->status=='401') {
                    $outMsg = '<b style="color:red;">'.__('Auth Problem').' HTTP-401 (Not Authorized) &nbsp;-&nbsp;</b>';
                ...
                if (is_array($userinfo->response->user->blogs)) {
                    ...
                    foreach ($userinfo->response->user->blogs as $blog) {
                        ...
                        $res = nxs_remote_get("https://api.tumblr.com/v2/blog/".$uuid."/info?api_key=".nxs_gak($options['appKey']), nxs_mkRemOptsArr(nxs_getNXSHeaders()));
                        $replRet = json_decode($res['body'], true);
                        if (!empty($replRet) && !empty($replRet['meta']) && $replRet['meta']['status']=='200') {
                            $pgs .= '<option '.($options['pgID']==$uuid ? 'selected="selected"':'').' value="'.$uuid.'">'.$blog->name.' ('.$uuid.')</option>';
                        }
                    }
                } else {
                    $outMsg = '<b style="color:red;">'.__('Auth Problem').' #2&nbsp;-&nbsp;'.$loginError.'</b>';
                    if (!empty($_POST['isOut'])) {
                        echo $outMsg;
                    }
                    return $outMsg;
            ...
            if (!empty($_POST['isOut'])) {
                echo $pgCust.$pgs.'<option style="color:#BD5200" value="a">'.__('...enter the Blog ID').'</option>';
         ...
         <div style="width:100%;">
          ...
          <select id="trpgID<?php echo $ii; ?>" onchange="nxs_trBlogChange('<?php echo $ii; ?>',jQuery(this));" name="tr[<?php echo $ii; ?>][pgID]">
            <?php $pgi = !empty($options['blogList'])?$options['blogList']:'';
                ...
                echo $pgi; ?><option value="a"><?php _e('.... Enter the Blog ID'); ?></option>
          ...
          </div> <div id="nxsTRMsgDiv<?php echo $ii; ?>"><?php if (!empty($options['uMsg'])) {
                    echo $options['uMsg'];
                } ?><?php if ($isNew) {
                    ?><?php _e('Please authorize your account', 'nxs_snap'); ?><?php
                } ?></div>                                                                                                    
    </div> <?php
            } ?> <input type="hidden" id="trAuthUser<?php echo $ii; ?>" value="<?php echo $options['authUser']; ?>"/> <br/><br/> 
...
<div style="margin-left: 10px;">
    ...
    <input type="radio" name="tr[<?php echo $ii; ?>][postType]" value="T" <?php if ($options['postType'] != 'I') {
    ...
    <div style="width:100%; margin-left: 15px;"><strong id="altFormatText"><?php _e('Post Title Format', 'social-networks-auto-poster-facebook-twitter-g'); ?>:</strong> (<a href="#" id="apTRTMsgFrmt<?php echo $ii; ?>HintInfo" onclick="mxs_showHideFrmtInfo('apTRTMsgFrmt<?php echo $ii; ?>'); return false;"><?php _e('Show format info', 'social-networks-auto-poster-facebook-twitter-g'); ?></a>)  </div><div onblur="jQuery('#apTRMsgFrmt<?php echo $ii; ?>Hint').hide();">
              <input name="tr[<?php echo $ii; ?>][msgTFormat]" id="apTRMsgTFrmt<?php echo $ii; ?>" style="margin-left: 15px; width: 50%;" value="<?php if ($options['msgTFormat']!='') {
                _e(apply_filters('format_to_edit', htmlentities($options['msgTFormat'], ENT_COMPAT, "UTF-8")), 'social-networks-auto-poster-facebook-twitter-g');
            } else {
                echo "New Post has been published on %SITENAME%";
            } ?>"  onfocus="jQuery('#apTRTMsgFrmt<?php echo $ii; ?>Hint').show();"  <?php if ($options['postType'] == 'I') {
                echo 'disabled="disabled"';
            } ?>  /><br/>
              <?php nxs_doShowHint("apTRTMsgFrmt".$ii); ?>
            </div>
            
<input type="radio" name="tr[<?php echo $ii; ?>][postType]" value="I" <?php if ($options['postType'] == 'I') {
                echo 'checked="checked"';
...
<input type="radio" name="tr[<?php echo $ii; ?>][cImgURL]" value="R" <?php if (empty($options['cImgURL']) || $options['cImgURL'] == 'R') {
                echo 'checked="checked"';
            } ?> /> Regular Post URL&nbsp;&nbsp;
<input type="radio" name="tr[<?php echo $ii; ?>][cImgURL]" value="S" <?php if (!empty($options['cImgURL']) && $options['cImgURL'] == 'S') {
                echo 'checked="checked"';
            } ?> /> Shortened Post URL&nbsp;&nbsp;
<input type="radio" name="tr[<?php echo $ii; ?>][cImgURL]" value="N" <?php if (!empty($options['cImgURL']) && $options['cImgURL'] == 'N') {
                echo 'checked="checked"';
            } ?> /> No Clickthrough URL&nbsp;&nbsp;
            ...
            </div><input name="tr[<?php echo $ii; ?>][defImg]" id="apTRDefImg<?php echo $ii; ?>" style=" margin-left: 15px; width: 30%;" <?php if ($options['postType'] != 'I') {
                echo 'disabled="disabled"';
            } ?> value="<?php _e(apply_filters('format_to_edit', htmlentities($options['defImg'], ENT_COMPAT, "UTF-8")), 'social-networks-auto-poster-facebook-twitter-g') ?>" /> 
<br/>            
<input type="radio" name="tr[<?php echo $ii; ?>][postType]" value="U" <?php if ($options['postType'] == 'U') {
                echo 'checked="checked"';
            } ?> /> Audio Post<br/>
<input type="radio" name="tr[<?php echo $ii; ?>][postType]" value="V" <?php if ($options['postType'] == 'V') {
                echo 'checked="checked"';
            } ?> /> Video Post<br/>            
<i style="">Tip: Your post must contain link to Audio or Video file if you select "Audio Post" or "Video Post" , otherwise it will reverted to the "Text Post"</i>
    ...
    <div style="margin-bottom: 20px;margin-top: 5px;">
              <input value="1" type="checkbox" name="tr[<?php echo $ii; ?>][fillSrcURL]"  <?php if ((int)$options['fillSrcURL'] == 1) {
                echo "checked";
            } ?> /> 
              <strong>Fill "Source URL"</strong> Will fill Tumblr's "Source URL" with post URL or defined URL.
              
              <br/><input value="1" type="checkbox" name="tr[<?php echo $ii; ?>][useOrDate]"  <?php if ((int)$options['useOrDate'] == 1) {
                echo "checked";
            } ?> /> 
              <strong>Keep Original Post Date</strong> Will post to Tumblr with original date of the post 
              
              <br/><input value="1" type="checkbox" name="tr[<?php echo $ii; ?>][inclTags]"  <?php if ((int)$options['inclTags'] == 1) {
                echo "checked";
            } ?> /> 
              <strong>Post with tags.</strong> Tags from the blogpost will be auto posted to Tumblr                                
              
              <br/><input value="1" type="checkbox" name="tr[<?php echo $ii; ?>][inclCats]"  <?php if ((int)$options['inclCats'] == 1) {
                echo "checked";
            } ?> /> 
      ...
      if ($options['appSec']=='') {
          ...
          if (!empty($options['accessToken'])) {
            ...
            <a href="<?php echo $nxs_snapSetPgURL.(stripos($nxs_snapSetPgURL, '?')!==false?'&':'?'); ?>auth=tr&acc=<?php echo $ii; ?>">Authorize Your Tumblr Account</a> 
              <?php if (empty($options['accessTokenSec'])) {
  ...
  { //######################## STOPPED HERE
    foreach ($post as $ii => $pval) {
        if (!empty($pval['appKey']) && !empty($pval['appKey'])) {
            if (!isset($options[$ii])) {
                $options[$ii] = array();
            }
            ...
            if (isset($pval['defImg'])) {
                $options[$ii]['defImg'] = trim($pval['defImg']);
            } else {
                $options[$ii]['defImg'] = "";
            }
            ...
            if (isset($pval['inclCats'])) {
                $options[$ii]['inclCats'] = $pval['inclCats'];
            } else {
                $options[$ii]['inclCats'] = 0;
            }
        
            if (isset($pval['pgID'])) {
                $options[$ii]['pgID'] = trim($pval['pgID']);
            } elseif (isset($pval['trURL'])) {
                $options[$ii]['trURL'] = trim($pval['trURL']);
                ...
                $options[$ii]['pgID'] = $trPgID; //echo $fbPgID;
            }
        } elseif (count($pval)==1) {
            if (isset($pval['do'])) {
      ...
      return $options;
        ...
        <tr style="<?php echo !empty($ntOpt['do'])?'display:table-row;':'display:none;'; ?>" class="nxstbldo nxstbldo<?php echo strtoupper($nt).$ii; ?>" style=""><th scope="row" style="text-align:right; width:60px; padding-right:10px;">
                <input type="radio" name="tr[<?php echo $ii; ?>][postType]" value="T" <?php if ($ntOpt['postType'] != 'I') {
                    echo 'checked="checked"';
                } ?>  /> <br/>                
                </th>
                ...
                <tr style="<?php echo !empty($ntOpt['do'])?'display:table-row;':'display:none;'; ?>" class="nxstbldo nxstbldo<?php echo strtoupper($nt).$ii; ?>" style=""><th scope="row" style="text-align:right; width:60px; padding-right:10px;">
                <input type="radio" name="tr[<?php echo $ii; ?>][postType]" value="I" <?php if ($ntOpt['postType'] == 'I') {
                    echo 'checked="checked"';
                } ?>  />  <br/>                
                </th>
                <td><b>Image Post</b>&nbsp;&nbsp;&nbsp;&nbsp;
                <input type="radio" name="tr[<?php echo $ii; ?>][postType]" value="V" <?php if ($ntOpt['postType'] == 'V') {
                    echo 'checked="checked"';
                } ?>  /> <b>Video Post</b> <?php nxs_doShowHint("apTRTMsgFrmt".$ii); ?> &nbsp;&nbsp;&nbsp;&nbsp;
                <input type="radio" name="tr[<?php echo $ii; ?>][postType]" value="U" <?php if ($ntOpt['postType'] == 'U') {
                    echo 'checked="checked"';
                } ?>  /> <b>Audio Post</b> <?php nxs_doShowHint("apTRTMsgFrmt".$ii); ?> 
                </td></tr>
   ...
   <div class="nxsPostEd_ElemWrap"> <div class="nxsPostEd_ElemLabel"><?php _e('Post Type:', 'social-networks-auto-poster-facebook-twitter-g'); ?></div>       
     <div class="nxsPostEd_Elem">   
         <input type="radio" name="tr[<?php echo $ii; ?>][postType]" value="T" class="nxsEdElem" data-ii="<?php echo $ii; ?>" data-nt="<?php echo $nt; ?>" <?php if ($ntOpt['postType'] != 'I') {
           echo 'checked="checked"';
       } ?>  /><?php _e('Text Post', 'social-networks-auto-poster-facebook-twitter-g'); ?>                
         <?php $this->elemEdTitleFormat($ii, __('Title Format:', 'social-networks-auto-poster-facebook-twitter-g'), $msgTFormat); ?>                
         <input type="radio" name="tr[<?php echo $ii; ?>][postType]" value="I" class="nxsEdElem" data-ii="<?php echo $ii; ?>" data-nt="<?php echo $nt; ?>" <?php if ($ntOpt['postType'] == 'I') {
           echo 'checked="checked"';
       } ?>  /> <b>Image Post</b><br/>
         <input type="radio" name="tr[<?php echo $ii; ?>][postType]" value="V" class="nxsEdElem" data-ii="<?php echo $ii; ?>" data-nt="<?php echo $nt; ?>" <?php if ($ntOpt['postType'] == 'V') {
           echo 'checked="checked"';
       } ?>  /> <b>Video Post</b><br/>
         <input type="radio" name="tr[<?php echo $ii; ?>][postType]" value="U" class="nxsEdElem" data-ii="<?php echo $ii; ?>" data-nt="<?php echo $nt; ?>" <?php if ($ntOpt['postType'] == 'U') {
           echo 'checked="checked"';
       } ?>  /> <b>Audio Post</b> 
     </div>
   </div><?php
