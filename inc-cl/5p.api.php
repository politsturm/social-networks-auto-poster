...
if (!class_exists("nxs_class_SNAP_5P")) {
    class nxs_class_SNAP_5P
    {
        public $ntCode = 'XI';
        public $ntLCode = 'xi';
      ...
      $tum_oauth = new nxs_OAuthBaseCl(nxs_gak($options['appKey']), nxs_gas($options['appSec']), $options['accessToken'], $options['accessTokenSec']);
            ...
            $uplk = $postinfo['upload_key'];
            $upid = $postinfo['photo']['id'];
            $url = 'https://upload.500px.com/v1/upload';
            $fields = array( 'photo_id' => $upid, 'upload_key' => $uplk, 'consumer_key' => nxs_gak($options['appKey']), 'access_key' => $options['accessToken'] );
            ...
            if (!empty($imgRes) && !empty($imgRes['body']) && stripos($imgRes['body'], '"error":"None')!==false) {
                ...
                */
                ...
                $badOut['Error'] .= print_r($imgRes, true)." <br/> Code: ".print_r($postinfo, true);
            }
            return $badOut;
        }
    }
