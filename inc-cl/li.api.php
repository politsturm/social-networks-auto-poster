...
if (!class_exists("nxs_class_SNAP_LI")) {
    class nxs_class_SNAP_LI
    {
        ...
        public function postShare($tkn, $msg, $pgID='', $title='', $url='', $imgURL='', $dsc='')
        {
            $nURL = 'https://api.linkedin.com/v1/'.(empty($pgID)?'people/~':'companies/'.$pgID).'/shares?format=json&oauth2_access_token='.$tkn;
            ...
            if (!empty($url)) {
                $toPost['content'] = array('submitted-url'=>$url, 'title'=>htmlspecialchars($title, ENT_NOQUOTES, "UTF-8"), 'description'=>htmlspecialchars($dsc, ENT_NOQUOTES, "UTF-8"));
            }
            if (!empty($imgURL)) {
                $toPost['content']['submitted-image-url'] = $imgURL;
            }
            $hdrsArr['Content-Type']='application/json';
            $hdrsArr['x-li-format']='json';
            $toPost = json_encode($toPost);
            $advSet = nxs_mkRemOptsArr($hdrsArr, '', $toPost);
            $response  = nxs_remote_post($nURL, $advSet);
            if (is_nxs_error($response) || empty($response['body'])) {
        ...
        public function doPostToNT($options, $message)
        {
            ...
            $liPostID = ''; // prr($message); prr($options);
            ...
            if ((!isset($options['uName']) || empty($options['uPass'])) && (empty($options['accessToken']))) {
            ...
            if (empty($options['msgTFormat'])) {
                $options['msgTFormat'] = '%TITLE%';
            }
            if (empty($options['msgATFormat'])) {
                $options['msgATFormat'] = '%TITLE%';
            }
            //## Format
            ...
            if (!empty($message['pTitle'])) {
                ...
                $msgT = nxs_doFormatMsg($options['msgTFormat'], $message);
            }
      
            if (!empty($message['urlTitle'])) {
                $msgAT = $message['urlTitle'];
            } else {
                $msgAT = nxs_doFormatMsg($options['msgATFormat'], $message);
            }
            if (!empty($message['urlDescr'])) {
                $msgA = $message['urlDescr'];
            } else {
                $msgA = nxs_doFormatMsg($options['msgAFormat'], $message);
            }
      
            if ($options['whToPost']=='P') {
                $title = nxs_doFormatMsg($options['msgCTFormat'], $message);
                $html = nxs_doFormatMsg($options['msgCFormat'], $message);
            }
            ...
            if ($options['postType'] == 'A' || $options['postType'] == 'I') {
                ...
                if (!empty($msgAT)) {
                    $msgAT = nxs_html_to_utf8($msgAT);
                }
                $msgA = strip_tags($msgA);
                $msgA = nxs_decodeEntitiesFull($msgA);
                if (!empty($msgA)) {
                    $msgA = nxs_html_to_utf8($msgA);
                }
                $msgA = nsTrnc($msgA, 300);
            }
            $msg  = strip_tags($msg);
            if (!empty($msg)) {
        ...
        $uname = $options['uName'];
                ...
                $ck = !empty($options['ck'])?maybe_unserialize($options['ck']):'';
                if (!empty($ck)) {
                    $ck = nxsClnCookies($ck);
                }
                $nt = new nxsAPI_LI();
                ...
                if (is_array($loginErr) && !empty($loginErr['out'])) {
                    if (function_exists('update_option')) {
                        update_option('nxs_li_ctp_save', $loginErr['ser'], false);
                    }
                    $text = $loginErr['out'];
                    echo "#2. LinkedIn asked you to enter verification code. Please check your email or phone, enter the code and click \"Confirm Code\"";
                    $text = str_ireplace('This login attempt seems suspicious. ', '', $text);
                    echo $text;
                    echo '<br/><input type="hidden" id="nxsLiNum" name="nxsLiNum" value="'.$options['ii'].'" /><input type="button" value="Click to Confirm Code" onclick="doCtpSave(); return false;" id="results_ok_button" name="nxs_go" class="button" /><br/><br/><div style="color:red;" id="nxsLITestResults"></div>'; ?><script type="text/javascript"> function doCtpSave(){ var u = jQuery('#verification-code').val(); var ii = jQuery('#nxsLiNum').val(); //alert(ii);                               
            jQuery.post(ajaxurl,{s:u, i:ii, action: 'nxs_snap_aj',"nxsact":"getItFromNT", "fName":"nxsCptCheck", nt:"LI", id: 0, _wpnonce: jQuery('input#nxsSsPageWPN_wpnonce').val()}, function(j){
              jQuery('#nxsLITestResults').html(j);
                ...
                if ($loginErr) {
                    $badOut['Error'] .= 'Can\'t Connect - '.print_r($loginErr, true);
                    return $badOut;
                }
                $opVal['ck'] = $nt->ck;
                nxs_saveOption($opNm, $opVal); //$options['ck'] = $nt->ck; if (function_exists('nxs_save_glbNtwrks')) nxs_save_glbNtwrks('li', $options['ii'], $nt->ck, 'ck');
                if ($options['whToPost']=='C' && !empty($options['pgcID'])) {
                    $to = 'https://www.linkedin.com/company/'.$options['pgcID'];
                } elseif ($options['whToPost']=='G' && !empty($options['pggID'])) {
                    $to = 'https://www.linkedin.com/groups/'.$options['pggID'];
                } elseif ($options['whToPost']=='PR') {
                ...
                if ($options['whToPost']=='P') {
                    $ret = $nt->postToPulse($msg, $title, $html, $imgURL);
                } else {
                    $ret = $nt->post($msg, $lnk, $to);
                }
                ...
                if (!empty($options['apiToUse']) && $options['apiToUse']=='liv2') {
                    if (empty($options['pgID'])||$options['pgID']=='p') {
                        $options['pgID'] = '';
                    } //## V2
                    if ($options['postType'] == 'A') {
                        $ret = $this->postShare($options['accessToken'], $msg, $options['pgID'], nsTrnc($msgAT, 200), $urlToGo, $imgURL, $msgA);
                    } else {
                        $ret = $this->postShare($options['accessToken'], $msg, $options['pgID']);
                    }
                } else {  //## V1
                    require_once('apis/liOAuth.php');
                    $linkedin = new nsx_LinkedIn(nxs_gak($options['appKey']), nxs_gas($options['appSec']));
                    $linkedin->oauth_verifier = $options['oAuthVerifier'];
                    ...
                    try {
                        if ($options['postType'] == 'A') {
                            $ret = $linkedin->postShare($msg, nsTrnc($msgAT, 200), str_replace('&', '&amp;', $urlToGo), $imgURL, $msgA);
                        } else {
