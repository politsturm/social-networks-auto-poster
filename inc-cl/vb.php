...
if (!class_exists("nxs_snapClassVB")) {
    class nxs_snapClassVB extends nxs_snapClassNT
    {
        public $ntInfo = array('code'=>'VB', 'lcode'=>'vb', 'name'=>'vBulletin', 'defNName'=>'uName', 'tstReq' => false, 'instrURL'=>'https://www.nextscripts.com/setup-installation-vbulletin-social-networks-auto-poster-wordpress/');
        public $defO = array('nName'=>'', 'do'=>'1', 'uName'=>'', 'uPass'=>'', 'vbURL'=>'', 'inclTags'=>'1', 'msgTFormat'=>'%TITLE%', 'msgFormat'=>"%FULLTEXT%");
  ...
  <div style="width:100%;"><strong id="altFormatText">vBulletin URL:</strong> <span style="font-size: 11px; margin: 0px;">Could be Forum URL or Thread URL. Either new thread of new post will be created.</span></div>
    ...
    <div style="margin-bottom: 20px;margin-top: 5px;"><input value="1"  id="vbInclTags<?php echo $ii; ?>" type="checkbox" name="vb[<?php echo $ii; ?>][inclTags]"  <?php if ((int)$options['inclTags'] == 1) {
                echo "checked";
            } ?> /> 
        ...
        public function setNTSettings($post, $options)
        {
            foreach ($post as $ii => $pval) {
                if (!empty($pval['uPass']) && !empty($pval['uPass'])) {
                    ...
                    if (isset($pval['vbURL'])) {
                        $options[$ii]['vbURL'] = trim($pval['vbURL']);
                    }
                } elseif (count($pval)==1) {
                    if (isset($pval['do'])) {
